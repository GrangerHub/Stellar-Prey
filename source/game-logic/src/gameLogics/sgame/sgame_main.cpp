...
static sint gameCvarTableSize = sizeof( gameCvarTable ) / sizeof( gameCvarTable[ 0 ] );
...
void idSGameMain::SpawnClients( team_t team )
{
    ...
    if( GetSpawnQueueLength( sq ) > 0 && numSpawns > 0 )
    {
        ...
        if( ( spawn = idSGameClient::SelectTremulousSpawnPoint( team, ent->client->pers.lastDeathLocation, spawn_origin, spawn_angles ) ) )
        {
...
void idSGameMain::CalculateBuildPoints( void )
{
    ...
    gentity_t*   ent;
    sint         localHTP = level.humanBuildPoints = g_humanBuildPoints.integer,
    ...
    //may as well pump the stages here too
    {
        ...
        if( g_alienStage.integer < g_alienMaxStage.integer )
            alienNextStageThreshold = ( sint )( ceil( ( float32 )g_alienStageThreshold.integer * ( g_alienStage.integer + 1 ) * alienPlayerCountMod ) );
        else if( g_humanStage.integer > S1 )
            alienNextStageThreshold = ( sint )( ceil( ( float32 )level.alienStagedownCredits + g_alienStageThreshold.integer * alienPlayerCountMod ) );
        else
            alienNextStageThreshold = -1;
            
        if( g_humanStage.integer < g_humanMaxStage.integer )
            humanNextStageThreshold = ( sint )( ceil( ( float32 )g_humanStageThreshold.integer * ( g_humanStage.integer + 1 ) * humanPlayerCountMod ) );
        else if( g_alienStage.integer > S1 )
            humanNextStageThreshold = ( sint )( ceil( ( float32 )level.humanStagedownCredits + g_humanStageThreshold.integer * humanPlayerCountMod ) );
        else
        ...
        if( alienNextStageThreshold > 0 )
            alienNextStageThreshold = ceil( ( float32 )alienNextStageThreshold / 100 ) * 100;
        if( humanNextStageThreshold > 0 )
            humanNextStageThreshold = ceil( ( float32 )humanNextStageThreshold / 100 ) * 100;
...
{
    float32         alienPlayerCountMod     = level.averageNumAlienClients / PLAYER_COUNT_MOD;
    float32         humanPlayerCountMod     = level.averageNumHumanClients / PLAYER_COUNT_MOD;
    ...
    if( g_alienCredits.integer >=
            ( sint )( ceil( ( float32 )g_alienStageThreshold.integer * S2 * alienPlayerCountMod ) ) &&
    ...
    if( g_alienCredits.integer >=
            ( sint )( ceil( ( float32 )g_alienStageThreshold.integer * S3 * alienPlayerCountMod ) ) &&
    ...
    if( g_humanCredits.integer >=
            ( sint )( ceil( ( float32 )g_humanStageThreshold.integer * S2 * humanPlayerCountMod ) ) &&
    ...
    if( g_humanCredits.integer >=
            ( sint )( ceil( ( float32 )g_humanStageThreshold.integer * S3 * humanPlayerCountMod ) ) &&
    ...
    {
        idSGameTrigger::Checktrigger_stages( TEAM_ALIENS, ( stage_t )g_alienMaxReachedStage.integer );
    ...
    {
        idSGameTrigger::Checktrigger_stages( TEAM_HUMANS, ( stage_t )g_humanMaxReachedStage.integer );
...
void idSGameMain::CalculateRanks( void )
{
    ...
    qsort( level.sortedClients, level.numConnectedClients, sizeof( level.sortedClients[ 0 ] ), &idSGameMain::SortRanks );
...
void idSGameMain::FindIntermissionPoint( void )
{
    ...
    ent = idSGameUtils::Find( nullptr, FOFS( classname ), "info_player_intermission" );
    
    if( !ent )
    {
        // the map creator forgot to put in an intermission point...
        idSGameClient::SelectSpawnPoint( vec3_origin, level.intermission_origin, level.intermission_angle );
    }
    else
    {
...
void idSGameMain::CheckVote( void )
{
    ...
    if( level.voteYes + level.voteNo > 0 )
        voteYesPercent = ( sint )( 100 * ( level.voteYes ) / ( level.voteYes + level.voteNo ) );
    else
        voteYesPercent = 0;
