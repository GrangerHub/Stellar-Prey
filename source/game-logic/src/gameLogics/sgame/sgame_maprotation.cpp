...
===============
*/
bool idSGameMaprotation::ParseMapCommandSection( mapRotationEntry_t* mre, valueType** text_p )
{
    valueType* token;
    ...
    while( 1 )
    {
        ...
        {
            idSGameMain::Printf( S_COLOR_RED "ERROR: maximum number of map commands (%d) reached\n", MAX_MAP_COMMANDS );
            return false;
        }
...
===============
*/
bool idSGameMaprotation::ParseMapRotation( mapRotation_t* mr, valueType** text_p )
{
    valueType* token;
    bool mnSet = false;
    ...
    while( 1 )
    {
        ...
        if( !Q_stricmp( token, "{" ) )
        {
            if( !mnSet )
            {
                idSGameMain::Printf( S_COLOR_RED "ERROR: map settings section with no name\n" );
                return false;
            }
            
            if( !ParseMapCommandSection( mre, text_p ) )
            {
                idSGameMain::Printf( S_COLOR_RED "ERROR: failed to parse map command section\n" );
                return false;
            }
        ...
        else if( !Q_stricmp( token, "goto" ) )
        {
            ...
            {
                idSGameMain::Printf( S_COLOR_RED "ERROR: maximum number of conditions for one map (%d) reached\n", MAX_MAP_ROTATION_CONDS );
                return false;
            }
        ...
        else if( !Q_stricmp( token, "if" ) )
        {
            ...
            if( !Q_stricmp( token, "numClients" ) )
            {
                ...
                {
                    idSGameMain::Printf( S_COLOR_RED "ERROR: invalid operator in expression: %s\n", token );
                    return false;
                }
            ...
            else if( !Q_stricmp( token, "lastWin" ) )
            {
                ...
                {
                    idSGameMain::Printf( S_COLOR_RED "ERROR: invalid right hand side in expression: %s\n", token );
                    return false;
                }
            }
            ...
            {
                idSGameMain::Printf( S_COLOR_RED "ERROR: invalid left hand side in expression: %s\n", token );
                return false;
            }
        ...
        {
            idSGameMain::Printf( S_COLOR_RED "ERROR: maximum number of maps in one rotation (%d) reached\n", MAX_MAP_ROTATION_MAPS );
            return false;
        }
        ...
        if( !Q_stricmp( token, "{" ) )
        {
            if( mrNameSet )
            {
                ...
                for( i = 0; i < mapRotations.numRotations; i++ )
                {
                    ...
                    {
                        idSGameMain::Printf( S_COLOR_RED "ERROR: a map rotation is already named %s\n", mrName );
                        return false;
                    }
                }
                ...
                {
                    idSGameMain::Printf( S_COLOR_RED "ERROR: maximum number of map rotations (%d) reached\n",
                                         MAX_MAP_ROTATIONS );
                    return false;
                }
                ...
                {
                    idSGameMain::Printf( S_COLOR_RED "ERROR: %s: failed to parse map rotation %s\n", fileName, mrName );
                    return false;
                }
    ...
    for( i = 0; i < mapRotations.numRotations; i++ )
    {
        for( j = 0; j < mapRotations.rotations[ i ].numMaps; j++ )
        {
            ...
            for( k = 0; k < mapRotations.rotations[ i ].maps[ j ].numConditions; k++ )
            {
                ...
                {
                    idSGameMain::Printf( S_COLOR_RED "ERROR: conditional destination \"%s\" doesn't exist\n", mapRotations.rotations[ i ].maps[ j ].conditions[ k ].dest );
                    return false;
                }
...
void idSGameMaprotation::IssueMapChange( sint rotation )
{
    ...
    idSGameMain::MapConfigs( mapRotations.rotations[ rotation ].maps[ map ].name );
...
===============
*/
mapConditionType_t idSGameMaprotation::ResolveConditionDestination( sint* n, valueType* name )
{
    sint i;
...
===============
*/
bool idSGameMaprotation::EvaluateMapCondition( mapRotationCondition_t* mrc )
{
    switch( mrc->lhs )
    {
        case MCV_RANDOM:
    ...
    for( i = 0; i < mre->numConditions; i++ )
    {
        ...
        if( mrc->unconditional || EvaluateMapCondition( mrc ) )
        {
            ...
            switch( mct )
            {
                ...
                default:
                case MCT_ERR:
                    idSGameMain::Printf( S_COLOR_YELLOW "WARNING: map switch destination could not be resolved: %s\n", mrc->dest );
                    break;
            }
        }
    }
...
===============
*/
bool idSGameMaprotation::StartMapRotation( valueType* name, bool changeMap )
{
    sint i;
