...
================
*/
void idSGameBuildable::SetBuildableAnim( gentity_t* ent, buildableAnimNumber_t anim, bool force )
{
...
================
*/
void idSGameBuildable::SetIdleBuildableAnim( gentity_t* ent, buildableAnimNumber_t anim )
{
...
===============
*/
gentity_t* idSGameBuildable::CheckSpawnPoint( sint spawnNum, vec3_t origin, vec3_t normal, buildable_t spawn, vec3_t spawnOrigin )
{
    ...
    {
        bggame->ClassBoundingBox( PCL_HUMAN, cmins, cmaxs, nullptr, nullptr, nullptr );
    ...
    trap_Trace( &tr, origin, nullptr, nullptr, localOrigin, spawnNum, MASK_SHOT );
...
bool idSGameBuildable::FindCreep( gentity_t* self )
{
    ...
    if( self->client || self->parentNode == nullptr || !self->parentNode->inuse ||
...
================
*/
void idSGameBuildable::nullDieFunction( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, sint damage, sint mod )
{
}
...
void idSGameBuildable::AGeneric_Blast( gentity_t* self )
{
    ...
    idSGameCombat::SelectiveRadiusDamage( self->s.pos.trBase, self, self->splashDamage, self->splashRadius, self, self->splashMethodOfDeath, TEAM_ALIENS );
...
================
*/
void idSGameBuildable::AGeneric_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, sint damage, sint mod )
{
...
void idSGameBuildable::AGeneric_CreepCheck( gentity_t* self )
{
    ...
    if( !FindCreep( self ) )
    {
        if( spawn && self->killedBy != ENTITYNUM_NONE )
            idSGameCombat::Damage( self, nullptr, g_entities + self->killedBy, nullptr, nullptr, self->health, 0, MOD_NOCREEP );
        else
            idSGameCombat::Damage( self, nullptr, nullptr, nullptr, nullptr, self->health, 0, MOD_NOCREEP );
        return;
    }
    CreepSlow( self );
}
...
void idSGameBuildable::AGeneric_Think( gentity_t* self )
{
    ...
    self->nextthink = level.time + bggame->Buildable( ( buildable_t )self->s.modelindex )->nextthink;
...
================
*/
void idSGameBuildable::AGeneric_Pain( gentity_t* self, gentity_t* attacker, sint damage )
{
    if( self->health <= 0 )
        return;
...
================
*/
void idSGameBuildable::ASpawn_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, sint damage, sint mod )
{
    sint i;
...
void idSGameBuildable::ASpawn_Think( gentity_t* self )
{
    ...
    if( self->spawned )
    {
        ...
        {
            if( ( ent = CheckSpawnPoint( self->s.number, self->s.origin, self->s.origin2, BA_A_SPAWN, nullptr ) ) != nullptr )
            {
                // If the thing blocking the spawn is a buildable, kill it.
                // If it's part of the map, kill self.
                if( ent->s.eType == ET_BUILDABLE )
                {
                    idSGameCombat::Damage( ent, nullptr, nullptr, nullptr, nullptr, 10000, 0, MOD_SUICIDE );
                    SetBuildableAnim( self, BANIM_SPAWN1, true );
                }
                else if( ent->s.number == ENTITYNUM_WORLD || ent->s.eType == ET_MOVER )
                {
                    idSGameCombat::Damage( self, nullptr, nullptr, nullptr, nullptr, 10000, 0, MOD_SUICIDE );
                    return;
                }
...
void idSGameBuildable::AOvermind_Think( gentity_t* self )
{
    ...
    {
        //do some damage
        if( idSGameCombat::SelectiveRadiusDamage( self->s.pos.trBase, self, self->splashDamage, self->splashRadius, self, MOD_OVERMIND, TEAM_ALIENS ) )
        {
            self->timestamp = level.time;
...
================
*/
void idSGameBuildable::ABarricade_Pain( gentity_t* self, gentity_t* attacker, sint damage )
{
    if( self->health <= 0 )
        return;
    if( !self->shrunkTime )
...
void idSGameBuildable::ABarricade_Shrink( gentity_t* self, bool shrink )
{
    ...
    else
    {
        ...
        {
            SetIdleBuildableAnim( self, ( buildableAnimNumber_t )bggame->Buildable( BA_A_BARRICADE )->idleAnim );
...
================
*/
void idSGameBuildable::ABarricade_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, sint damage, sint mod )
{
    AGeneric_Die( self, inflictor, attacker, damage, mod );
...
void idSGameBuildable::ABarricade_Touch( gentity_t* self, gentity_t* other, trace_t* trace )
{
        ...
        for( i = 0; i < num; i++ )
        {
            ...
            if( enemy->client && enemy->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
            {
                ...
                if( level.time >= self->timestamp + ACIDTUBE_REPEAT_ANIM )
                {
                    self->timestamp = level.time;
                    SetBuildableAnim( self, BANIM_ATTACK1, false );
                    idSGameUtils::AddEvent( self, EV_ALIEN_ACIDTUBE, DirToByte( self->s.origin2 ) );
                }
                
                idSGameCombat::SelectiveRadiusDamage( self->s.pos.trBase, self, ACIDTUBE_DAMAGE, ACIDTUBE_RANGE, self, MOD_ATUBE, TEAM_ALIENS );
...
================
*/
bool idSGameBuildable::AHive_CheckTarget( gentity_t* self, gentity_t* enemy )
{
    trace_t trace;
...
================
*/
void idSGameBuildable::AHive_Pain( gentity_t* self, gentity_t* attacker, sint damage )
{
...
================
*/
void idSGameBuildable::AHive_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, sint damage, sint mod )
{
...
================
*/
bool idSGameBuildable::AHovel_Blocked( gentity_t* hovel, gentity_t* player, bool provideExit )
{
    ...
    bggame->ClassBoundingBox( ( class_t )player->client->ps.stats[ STAT_CLASS ], mins, maxs, nullptr, nullptr, nullptr );
    ...
    trap_Trace( &tr, start, nullptr, nullptr, end, player->s.number, MASK_PLAYERSOLID );
    if( tr.fraction < 1.0f )
        return true;
...
bool idSGameBuildable::APropHovel_Blocked( vec3_t origin, vec3_t angles, vec3_t normal, gentity_t* player )
{
    gentity_t hovel;
...
================
*/
void idSGameBuildable::AHovel_Use( gentity_t* self, gentity_t* other, gentity_t* activator )
{
    ...
    if( self->spawned && FindOvermind( self ) )
    {
        if( self->active )
        {
            ...
            idSGameUtils::TriggerMenu( activator->client->ps.clientNum, MN_A_HOVEL_OCCUPIED );
        }
                 ...
                 activator->health > 0 && self->health > 0 )
        {
            if( AHovel_Blocked( self, activator, false ) )
            {
                ...
                idSGameUtils::TriggerMenu( activator->client->ps.clientNum, MN_A_HOVEL_BLOCKED );
                return;
            }
...
================
*/
void idSGameBuildable::AHovel_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, sint damage, sint mod )
{
...
================
*/
void idSGameBuildable::ABooster_Touch( gentity_t* self, gentity_t* other, trace_t* trace )
{
    gclient_t* client = other->client;
...
================
*/
void idSGameBuildable::ATrapper_FireOnEnemy( gentity_t* self, sint firespeed, float32 range )
{
    gentity_t* enemy = self->enemy;
    vec3_t dirToTarget;
    vec3_t halfAcceleration, thirdJerk;
    float32 distanceToTarget = bggame->Buildable( ( buildable_t )self->s.modelindex )->turretRange;
    sint lowMsec = 0;
    sint highMsec = ( sint )( ( ( ( distanceToTarget * LOCKBLOB_SPEED ) + ( distanceToTarget * bggame->Class( ( class_t )enemy->client->ps.stats[ STAT_CLASS ] )->speed ) ) / ( LOCKBLOB_SPEED * LOCKBLOB_SPEED ) ) * 1000.0f );
...
================
*/
bool idSGameBuildable::ATrapper_CheckTarget( gentity_t* self, gentity_t* target, sint range )
{
    vec3_t    distance;
    trace_t   trace;
    ...
    trap_Trace( &trace, self->s.pos.trBase, nullptr, nullptr, target->s.pos.trBase, self->s.number, MASK_SHOT );
...
{
    sint range = bggame->Buildable( ( buildable_t )self->s.modelindex )->turretRange;
    sint firespeed = bggame->Buildable( ( buildable_t )self->s.modelindex )->turretFireSpeed;
...
void idSGameBuildable::HRepeater_Think( gentity_t* self )
{
    ...
    if( NumberOfDependants( self ) == 0 )
    {
        ...
        if( self->count < 0 )
            ...
            idSGameCombat::Damage( self, nullptr, nullptr, nullptr, nullptr, self->health, 0, MOD_SUICIDE );
    }
    else
        self->count = -1;
...
================
*/
void idSGameBuildable::HRepeater_Use( gentity_t* self, gentity_t* other, gentity_t* activator )
{
...
void idSGameBuildable::HReactor_Think( gentity_t* self )
{
    ...
    if( self->spawned && ( self->health > 0 ) )
    {
        ...
        if( fired )
        {
            self->timestamp = level.time;
            if( self->dcc )
                idSGameCombat::SelectiveRadiusDamage( self->s.pos.trBase, self, REACTOR_ATTACK_DCC_DAMAGE, REACTOR_ATTACK_DCC_RANGE, self, MOD_REACTOR, TEAM_HUMANS );
            else
                idSGameCombat::SelectiveRadiusDamage( self->s.pos.trBase, self, REACTOR_ATTACK_DAMAGE, REACTOR_ATTACK_RANGE, self, MOD_REACTOR, TEAM_HUMANS );
        }
    }
...
================
*/
void idSGameBuildable::HArmoury_Activate( gentity_t* self, gentity_t* other, gentity_t* activator )
{
    if( self->spawned )
    {
        ...
        if( self->powered )
            ...
            idSGameUtils::TriggerMenu( activator->client->ps.clientNum, MN_H_NOTPOWERED );
    }
}
...
================
*/
void idSGameBuildable::HMedistat_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, sint damage, sint mod )
{
    ...
    if( self->spawned )
    {
        ...
        if( !occupied )
        {
            ...
            for( i = 0; i < num; i++ )
            {
                ...
                if( player->client && player->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
                {
                    ...
                    else if( !bggame->InventoryContainsUpgrade( UP_MEDKIT, player->client->ps.stats ) )
        ...
        else if( self->enemy && self->enemy->client )  //heal!
        {
            ...
            if( self->enemy->health >= self->enemy->client->ps.stats[ STAT_MAX_HEALTH ] )
            {
                ...
                if( !bggame->InventoryContainsUpgrade( UP_MEDKIT, self->enemy->client->ps.stats ) )
...
================
*/
bool idSGameBuildable::HMGTurret_CheckTarget( gentity_t* self, gentity_t* target, bool los_check )
{
    trace_t   tr;
    vec3_t    dir, end;
    ...
    trap_Trace( &tr, self->s.pos.trBase, nullptr, nullptr, end, self->s.number, MASK_SHOT );
...
void idSGameBuildable::HMGTurret_Think( gentity_t* self )
{
    ...
    idSGameWeapons::FireWeapon( self );
    self->s.eFlags |= EF_FIRING;
    self->count = level.time + bggame->Buildable( ( buildable_t )self->s.modelindex )->turretFireSpeed;
...
void idSGameBuildable::HSpawn_Blast( gentity_t* self )
{
    ...
    idSGameCombat::RadiusDamage( self->s.pos.trBase, self, self->splashDamage, self->splashRadius, self, self->splashMethodOfDeath );
...
================
*/
void idSGameBuildable::HSpawn_Die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, sint damage, sint mod )
{
    idSGameCombat::RewardAttackers( self );
    SetBuildableAnim( self, BANIM_DESTROY1, true );
    SetIdleBuildableAnim( self, BANIM_DESTROYED );
...
void idSGameBuildable::HSpawn_Think( gentity_t* self )
{
    ...
    if( self->spawned )
    {
        ...
        if( self->s.groundEntityNum )
        {
            if( ( ent = CheckSpawnPoint( self->s.number, self->s.origin, self->s.origin2, BA_H_SPAWN, nullptr ) ) != nullptr )
            {
                // If the thing blocking the spawn is a buildable, kill it.
                // If it's part of the map, kill self.
                if( ent->s.eType == ET_BUILDABLE )
...
void idSGameBuildable::QueueBuildPoints( gentity_t* self )
{
    ...
    if( self->killedBy != ENTITYNUM_NONE && killer->client &&  //there's a tiny chance that a turret could hit a tube
            //and kill it
...
void idSGameBuildable::BuildableThink( gentity_t* ent, sint msec )
{
    ...
    if( ent->health <= 0 )
        ...
        ent->s.generic1 = ( uchar8 )( ( B_HEALTH_MASK * ent->health + bHealth - 1 ) / bHealth );
...
===============
*/
bool idSGameBuildable::BuildableRange( vec3_t origin, float32 r, buildable_t buildable )
{
    sint       entityList[ MAX_GENTITIES ];
    vec3_t    range;
    vec3_t    mins, maxs;
...
===============
*/
bool idSGameBuildable::BuildablesIntersect( buildable_t a, vec3_t originA, buildable_t b, vec3_t originB )
{
    vec3_t minsA, maxsA;
    vec3_t minsB, maxsB;
...
===============
*/
sint idSGameBuildable::CompareBuildablesForRemoval( const void* a, const void* b )
{
    sint       precedence[ ] =
    {
        BA_NONE,
...
void idSGameBuildable::FreeMarkedBuildables( gentity_t* deconner )
{
    ...
    for( i = 0; i < level.numBuildablesForRemoval; i++ )
    {
        ...
        idSGameCombat::Damage( ent, nullptr, deconner, nullptr, nullptr, ent->health, 0, MOD_DECONSTRUCT );
...
===============
*/
itemBuildError_t idSGameBuildable::SufficientBPAvailable( buildable_t buildable, vec3_t origin )
{
    sint               i;
    ...
    for( i = MAX_CLIENTS, ent = g_entities + i; i < level.num_entities; i++, ent++ )
    {
        ...
        collision = BuildablesIntersect( buildable, origin, ( buildable_t )ent->s.modelindex, ent->s.origin );
        ...
        if( ent->deconstruct )
        {
            ...
            if( collision || repeaterInRange )
            {
                ...
                pointsYielded += bggame->Buildable( ( buildable_t )ent->s.modelindex )->buildPoints;
    ...
    qsort( level.markedBuildables, numBuildables, sizeof( level.markedBuildables[ 0 ] ), CompareBuildablesForRemoval );
...
================
*/
itemBuildError_t idSGameBuildable::CanBuild( gentity_t* ent, buildable_t buildable, sint distance, vec3_t origin )
{
    vec3_t            angles;
    ...
    bggame->PositionBuildableRelativeToPlayer( ps, mins, maxs, trap_Trace, entity_origin, angles, &tr1 );
    trap_Trace( &tr2, entity_origin, mins, maxs, entity_origin, ent->s.number, MASK_PLAYERSOLID );
    trap_Trace( &tr3, ps->origin, nullptr, nullptr, entity_origin, ent->s.number, MASK_PLAYERSOLID );
    ...
    if( ent->client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
    {
        ...
        if( buildable == BA_A_HOVEL )
        {
            ...
            bggame->ClassBoundingBox( PCL_ALIEN_BUILDER0_UPG, builderMins, builderMaxs, nullptr, nullptr, nullptr );
    ...
    else if( ent->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
    {
        ...
        if( buildable == BA_H_REPEATER )
        {
            ...
            else if( !g_markDeconstruct.integer && RepeaterEntityForPoint( entity_origin ) )
                reason = IBE_RPTPOWERHERE;
        }
    ...
    if( reason == IBE_NONE )
    {
        ...
        if( CheckSpawnPoint( ENTITYNUM_NONE, origin, normal, buildable, nullptr ) != nullptr )
            reason = IBE_NORMAL;
...
================
*/
gentity_t* idSGameBuildable::Build( gentity_t* builder, buildable_t buildable, vec3_t origin, vec3_t angles )
{
    gentity_t* built;
    vec3_t    normal;
    ...
    built->buildableTeam = ( team_t )( built->s.modelindex2 = bggame->Buildable( buildable )->team );
    ...
    if( bggame->Buildable( ( buildable_t )built->s.modelindex )->team == TEAM_ALIENS )
        built->powered = true;
    ...
    SetIdleBuildableAnim( built, ( buildableAnimNumber_t )bggame->Buildable( buildable )->idleAnim );
...
=================
*/
bool idSGameBuildable::BuildIfValid( gentity_t* ent, buildable_t buildable )
{
    float32         dist;
    vec3_t        origin;
...
void idSGameBuildable::FinishSpawningBuildable( gentity_t* ent )
{
    ...
    if( tr.startsolid )
    {
        idSGameMain::Printf( S_COLOR_YELLOW "idSGameBuildable::FinishSpawningBuildable: %s startsolid at %s\n", built->classname, idSGameUtils::vtos( built->s.origin ) );
...
============
*/
void idSGameBuildable::SpawnBuildable( gentity_t* ent, buildable_t buildable )
{
    ent->s.modelindex = buildable;
...
sint idSGameBuildable::LayoutList( pointer map, valueType* list, sint len )
{
    ...
    if( count != numFiles )
    {
        idSGameMain::Printf( S_COLOR_YELLOW "WARNING: layout list was truncated to %d "
...
void idSGameBuildable::LayoutSelect( void )
{
    ...
    while( 1 )
    {
        ...
        if( trap_FS_FOpenFile( fileName, nullptr, FS_READ ) > 0 )
        {
            ...
            idSGameMain::Printf( S_COLOR_YELLOW "idSGameBuildable::LayoutSelect: WARNING: layout \"%s\" does not exist\n", s );
    }
    if( !cnt )
    {
        idSGameMain::Printf( S_COLOR_RED "idSGameBuildable::LayoutSelect: ERROR: none of the specified layouts could be "
...
============
*/
void idSGameBuildable::LayoutBuildItem( buildable_t buildable, vec3_t origin, vec3_t angles, vec3_t origin2, vec3_t angles2 )
{
    gentity_t* builder;
...
void idSGameBuildable::LayoutLoad( void )
{
    ...
    *( layout + len ) = '\0';
    ...
    while( *layout )
    {
        if( i >= sizeof( line ) - 1 )
        {
            idSGameMain::Printf( S_COLOR_RED "ERROR: line overflow in %s before \"%s\"\n",
        ...
        if( *layout == '\n' )
        {
            ...
            if( buildable > BA_NONE && buildable < BA_NUM_BUILDABLES )
                ...
                idSGameMain::Printf( S_COLOR_YELLOW "WARNING: bad buildable number (%d) in layout.  skipping\n", buildable );
        }
        layout++;
    }
}
