...
static sint adminNumCmds = sizeof( g_admin_cmds ) / sizeof( g_admin_cmds[ 0 ] );
...
===============
*/
bool idAdminLocal::AdminNameCheck( gentity_t* ent, valueType* name, valueType* err, sint len )
{
    sint i;
    gclient_t* client;
    ...
    for( i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[ i ]; i++ )
    {
        ...
        idSGameCmds::SanitiseString( g_admin_admins[ i ]->name, testName, sizeof( testName ) );
...
===============
*/
bool idAdminLocal::AdminHigherGuid( valueType* admin_guid, valueType* victim_guid )
{
    sint i, alevel = 0;
...
bool idAdminLocal::AdminHigher( gentity_t* admin, gentity_t* victim )
{
    ...
    return AdminHigherGuid( admin->client->pers.guid, victim->client->pers.guid );
}
...
void idAdminLocal::AdminWriteConfig( void )
{
    ...
    if( !g_admin.string[ 0 ] )
    {
        idSGameMain::Printf( S_COLOR_YELLOW "WARNING: g_admin is not set. configuration will not be saved to a file.\n" );
        return;
    }
    ...
    for( i = 0; i < MAX_ADMIN_COMMANDS && g_admin_commands[ i ]; i++ )
    {
        ...
        {
            Q_strcat( levels, sizeof( levels ), va( "%i ", g_admin_commands[ i ]->levels[ j ] ) );
        }
...
===============
*/
void idAdminLocal::AdminReadConfigString( valueType** cnf, valueType* s, sint size )
{
    valueType* t;
...
===============
*/
bool idAdminLocal::AdminCommandPermission( gentity_t* ent, valueType* command )
{
    sint i, j, level;
...
===============
*/
void idAdminLocal::AdminLog( gentity_t* admin, valueType* cmd, sint skiparg )
{
    fileHandle_t f;
...
===============
*/
sint idAdminLocal::AdminListAdmins( gentity_t* ent, sint start, valueType* search )
{
    sint drawn = 0;
    ...
    for( i = start; i < MAX_ADMIN_ADMINS && g_admin_admins[ i ] && drawn < MAX_ADMIN_LISTITEMS; i++ )
    {
        if( search[ 0 ] )
        {
            ...
            for( j = 0; j < level.maxclients; j++ )
            {
                ...
                idSGameCmds::SanitiseString( vic->client->pers.netname, name2, sizeof( name2 ) );
...
===============
*/
void idAdminLocal::AdminDuration( sint secs, valueType* duration, sint dursize )
{
...
===============
*/
bool idAdminLocal::AdminBanCheck( valueType* userinfo, valueType* reason, sint rlen )
{
    valueType* guid, *ip;
    sint i, t;
...
bool idAdminLocal::AdminCmdCheck( gentity_t* ent, bool say )
{
    ...
    if( !Q_stricmp( command, "say" ) || ( !Q_stricmp( command, "say_team" ) && AdminPermission( ent, ADMF_TEAMCHAT_CMD ) ) )
    {
        skip = 1;
...
void idAdminLocal::AdminNamelogUpdate( gclient_t* client, bool disconnect )
{
    ...
    for( i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[ i ]; i++ )
    {
        ...
        if( !Q_stricmp( client->pers.ip, g_admin_namelog[ i ]->ip ) && !Q_stricmp( client->pers.guid, g_admin_namelog[ i ]->guid ) )
        {
            ...
            {
                idSGameCmds::SanitiseString( g_admin_namelog[ i ]->name[ j ], n2, sizeof( n2 ) );
...
bool idAdminLocal::AdminReadconfig( gentity_t* ent, sint skiparg )
{
    ...
    *( cnf + len ) = '\0';
    ...
    bggame->Free( cnf2 );
    adminLocal.ADMP( va( "^3!readconfig: ^7loaded %d levels, %d admins, %d bans, %d commands\n", lc, ac, bc, cc ) );
...
bool idAdminLocal::AdminSetlevel( gentity_t* ent, sint skiparg )
{
    ...
    else
    {
        ...
        for( i = 0; i < level.maxclients && matches < 2; i++ )
        {
            ...
            idSGameCmds::SanitiseString( level.clients[ i ].pers.netname, testname, sizeof( testname ) );
...
===============
*/
bool idAdminLocal::AdminCreateBan( gentity_t* ent, valueType* netname, valueType* guid, valueType* ip, sint seconds, valueType* reason )
{
    g_admin_ban_t* b = nullptr;
    qtime_t qt;
    sint t, i;
...
bool idAdminLocal::AdminKick( gentity_t* ent, sint skiparg )
{
    ...
    if( ( found = idSGameCmds::ClientNumbersFromString( name, pids, MAX_CLIENTS ) ) != 1 )
    {
    ...
    adminLocal.AdminCreateBan( ent, vic->client->pers.netname, vic->client->pers.guid, vic->client->pers.ip,
                               adminLocal.AdminParseTime( va( "1s%s", g_adminTempBan.string ) ), ( *reason ) ? reason : "kicked by admin" );
    ...
    trap_SendServerCommand( pids[ 0 ], va( "disconnect \"You have been kicked.\n%s^7\nreason:\n%s\"",
                                           ( ent ) ? va( "admin:\n%s", ent->client->pers.netname ) : "", ( *reason ) ? reason : "kicked by admin" ) );
                                           
    trap_DropClient( pids[ 0 ], va( "has been kicked%s^7. reason: %s",
                                    ( ent ) ? va( " by %s", ent->client->pers.netname ) : "", ( *reason ) ? reason : "kicked by admin" ), 0 );
...
bool idAdminLocal::AdminBan( gentity_t* ent, sint skiparg )
{
    ...
    if( adminLocal.AdminPermission( ent, ADMF_CAN_PERM_BAN ) && adminLocal.AdminPermission( ent, ADMF_UNACCOUNTABLE ) )
    {
        minargc = 2 + skiparg;
    }
    else if( adminLocal.AdminPermission( ent, ADMF_CAN_PERM_BAN ) || adminLocal.AdminPermission( ent, ADMF_UNACCOUNTABLE ) )
    {
        minargc = 3 + skiparg;
    }
    else
        ...
        for( i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[ i ]; i++ )
        {
                    ...
                    g_admin_namelog[ i ]->name[ j ][ 0 ]; j++ )
            {
                ...
                if( strstr( n2, s2 ) )
                {
                    ...
                    adminLocal.ADMBP( va( "%-2s (*%s) %15s ^7'%s^7'\n", ( g_admin_namelog[ i ]->slot > -1 ) ? va( "%d", g_admin_namelog[ i ]->slot ) : "-",
    ...
    if( ent && !adminLocal.AdminHigherGuid( ent->client->pers.guid, g_admin_namelog[ logmatch ]->guid ) )
    {
    ...
    adminLocal.AdminDuration( ( seconds ) ? seconds : -1, duration, sizeof( duration ) );
    adminLocal.AdminCreateBan( ent, g_admin_namelog[ logmatch ]->name[ 0 ], g_admin_namelog[ logmatch ]->guid, g_admin_namelog[ logmatch ]->ip, seconds, reason );
    ...
    if( g_admin_namelog[ logmatch ]->slot == -1 )
    {
        ...
        AP( va( "print \"^3!ban:^7 %s^7 has been banned by %s^7, duration: %s, reason: %s\n\"", g_admin_namelog[ logmatch ]->name[ 0 ],
                ( ent ) ? ent->client->pers.netname : "console", duration, ( *reason ) ? reason : "banned by admin" ) );
        return true;
    }
    
    trap_SendServerCommand( g_admin_namelog[ logmatch ]->slot, va( "disconnect \"You have been banned.\n admin:\n%s^7\nduration:\n%s\nreason:\n%s\"",
                            ( ent ) ? ent->client->pers.netname : "console", duration, ( *reason ) ? reason : "banned by admin" ) );
                            
    trap_DropClient( g_admin_namelog[ logmatch ]->slot, va( "has been banned by %s^7, duration: %s, reason: %s", ( ent ) ? ent->client->pers.netname : "console",
...
bool idAdminLocal::AdminUnBan( gentity_t* ent, sint skiparg )
{
    ...
    if( g_admin_bans[ bnum - 1 ]->expires == 0 && !adminLocal.AdminPermission( ent, ADMF_CAN_PERM_BAN ) )
    {
    ...
    AP( va( "print \"^3!unban: ^7ban #%d for %s^7 has been removed by %s\n\"", bnum, g_admin_bans[ bnum - 1 ]->name, ( ent ) ? ent->client->pers.netname : "console" ) );
...
bool idAdminLocal::AdminAdjustBan( gentity_t* ent, sint skiparg )
{
    ...
    if( ban->expires == 0 && !adminLocal.AdminPermission( ent, ADMF_CAN_PERM_BAN ) )
    {
    ...
    else
    {
        ...
        ban->expires = expires;
        adminLocal.AdminDuration( ( expires ) ? expires - time : -1, duration, sizeof( duration ) );
    }
    reason = idSGameCmds::SayConcatArgs( 3 + skiparg );
    ...
    AP( va( "print \"^3!adjustban: ^7ban #%d for %s^7 has been updated by %s^7 %s%s%s%s%s\n\"", bnum, ban->name,
            ( ent ) ? ent->client->pers.netname : "console", ( length >= 0 ) ? "duration: " : "",
            duration, ( length >= 0 && *reason ) ? ", " : "", ( *reason ) ? "reason: " : "", reason ) );
...
bool idAdminLocal::AdminMap( gentity_t* ent, sint skiparg )
{
    ...
    if( idSGameCmds::SayArgc( ) > 2 + skiparg )
    {
        ...
        if( !Q_stricmp( layout, "*BUILTIN*" ) || trap_FS_FOpenFile( va( "layouts/%s/%s.dat", map, layout ), nullptr, FS_READ ) > 0 )
        {
...
bool idAdminLocal::AdminMute( gentity_t* ent, sint skiparg )
{
    ...
    if( !adminLocal.AdminHigher( ent, &g_entities[ pids[ 0 ] ] ) )
    {
        adminLocal.ADMP( va( "^3!%s: ^7sorry, but your intended victim has a higher admin level than you\n", cmd ) );
        return false;
    }
...
bool idAdminLocal::AdminListAdmins( gentity_t* ent, sint skiparg )
{
    ...
    {
        adminLocal.ADMBP_begin();
        adminLocal.ADMBP( va( "^3!listadmins:^7 showing admin %d - %d of %d.  ", ( found ) ? ( start + 1 ) : 0, ( ( start + MAX_ADMIN_LISTITEMS ) > found ) ?
                              found : ( start + MAX_ADMIN_LISTITEMS ), found ) );
                              
        if( ( start + MAX_ADMIN_LISTITEMS ) < found )
        {
            adminLocal.ADMBP( va( "run '!listadmins %d' to see more", ( start + MAX_ADMIN_LISTITEMS + 1 ) ) );
        }
        adminLocal.ADMBP( "\n" );
        adminLocal.ADMBP_end();
    }
...
bool idAdminLocal::AdminListPlayers( gentity_t* ent, sint skiparg )
{
    ...
    for( i = 0; i < level.maxclients; i++ )
    {
        ...
        {
            adminLocal.ADMBP( va( "%2i %s%s^7 %-2i %s^7 (*%s) ^1%1s%1s^7 %s^7 %s%s^7%s\n", i, c, t, l, lname, guid_stub, muted, denied, p->pers.netname,
                                  ( *n ) ? "(a.k.a. " : "", n, ( *n ) ? ")" : "" ) );
        }
        else
        {
...
bool idAdminLocal::AdminShowBans( gentity_t* ent, sint skiparg )
{
    ...
    adminLocal.ADMBP( va( "^3!showbans:^7 showing bans %d - %d of %d (%d total).", ( found ) ? ( start + 1 ) : 0, i, max + 1, found ) );
...
bool idAdminLocal::AdminAdminTest( gentity_t* ent, sint skiparg )
{
    ...
    AP( va( "print \"^3!admintest: ^7%s^7 is a level %d admin %s%s^7%s\n\"", ent->client->pers.netname, l,
            ( lname ) ? "(" : "", ( lname ) ? g_admin_levels[ i ]->name : "", ( lname ) ? ")" : "" ) );
...
bool idAdminLocal::AdminRestart( gentity_t* ent, sint skiparg )
{
    ...
    AP( va( "print \"^3restart: ^7map restarted by %s %s %s\n\"", ( ent ) ? ent->client->pers.netname : "console",
            ( layout[0] ) ? va( "^7(forcing layout '%s^7')", layout ) : "", ( teampref[0] ) ? va( "^7(with teams option: '%s^7')", teampref ) : "" ) );
...
bool idAdminLocal::AdminNextmap( gentity_t* ent, sint skiparg )
{
    ...
    idSGameMain::LogExit( va( "nextmap was run by %s", ( ent ) ? ent->client->pers.netname : "console" ) );
...
bool idAdminLocal::AdminNameLog( gentity_t* ent, sint skiparg )
{
    ...
    for( i = 0; i < MAX_ADMIN_NAMELOGS && g_admin_namelog[ i ]; i++ )
    {
        ...
        adminLocal.ADMBP( va( "%-2s (*%s) %15s^7", ( g_admin_namelog[ i ]->slot > -1 ) ?
                              va( "%d", g_admin_namelog[ i ]->slot ) : "-", guid_stub, g_admin_namelog[ i ]->ip ) );
