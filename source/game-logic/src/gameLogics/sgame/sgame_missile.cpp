...
void idSGameMissile::ExplodeMissile(gentity_t *ent) {
    ...
    // splash damage
    if(ent->splashDamage) {
        idSGameCombat::RadiusDamage(ent->r.currentOrigin, ent->parent,
...
void idSGameMissile::MissileImpact(gentity_t *ent, trace_t *trace) {
    ...
    // check for bounce
    if(!other->takedamage && (ent->bounceCount > 0 ||
                              ent->bounceCount == -5) &&
    ...
    if(other->takedamage) {
        ...
        if(ent->damage) {
            ...
            idSGameCombat::Damage(other, ent, attacker, velocity, ent->s.origin,
                                  ent->damage, DAMAGE_NO_LOCDAMAGE, ent->methodOfDeath);
        }
    }
    ...
    if(other->takedamage && (other->s.eType == ET_PLAYER ||
        ...
        idSGameUtils::AddEvent(ent, EV_MISSILE_HIT,
                               DirToByte(trace->plane.normal));
        ent->s.otherEntityNum = other->s.number;
    } else if(trace->surfaceFlags & SURF_METALSTEPS) {
        idSGameUtils::AddEvent(ent, EV_MISSILE_MISS_METAL,
                               DirToByte(trace->plane.normal));
    } else {
        idSGameUtils::AddEvent(ent, EV_MISSILE_MISS,
                               DirToByte(trace->plane.normal));
    }
    ...
    idSGameWeapons::SnapVectorTowards(trace->endpos,
                                      ent->s.pos.trBase);    // save net bandwidth

    idSGameUtils::SetOrigin(ent, trace->endpos);

    // splash damage (doesn't apply to person directly hit)
    if(ent->splashDamage) {
        idSGameCombat::RadiusDamage(trace->endpos, ent->parent, ent->splashDamage,
...
=================
*/
gentity_t *idSGameMissile::fire_flamer(gentity_t *self, vec3_t start,
                                       vec3_t dir) {
    gentity_t *bolt;
    vec3_t    pvel;
    ...
    bolt->s.pos.trTime = level.time -
                         MISSILE_PRESTEP_TIME;   // move a bit on the very first frame
...
=================
*/
gentity_t *idSGameMissile::fire_blaster(gentity_t *self, vec3_t start,
                                        vec3_t dir) {
    gentity_t *bolt;
...
=================
*/
gentity_t *idSGameMissile::fire_pulseRifle(gentity_t *self, vec3_t start,
        vec3_t dir) {
    gentity_t *bolt;
...
=================
*/
gentity_t *idSGameMissile::fire_luciferCannon(gentity_t *self,
        vec3_t start, vec3_t dir, sint damage, sint radius, sint speed) {
    gentity_t *bolt;
    float32 charge;
...
=================
*/
gentity_t *idSGameMissile::launch_grenade(gentity_t *self, vec3_t start,
        vec3_t dir) {
    gentity_t *bolt;
...
void idSGameMissile::AHive_SearchAndDestroy(gentity_t *self) {
    ...
    nearest = DistanceSquared(self->r.currentOrigin,
    ...
    VectorSubtract(self->target_ent->r.currentOrigin, self->r.currentOrigin,
                   dir);
    VectorNormalize(dir);
...
=================
*/
gentity_t *idSGameMissile::fire_hive(gentity_t *self, vec3_t start,
                                     vec3_t dir) {
    gentity_t *bolt;
...
=================
*/
gentity_t *idSGameMissile::fire_lockblob(gentity_t *self, vec3_t start,
        vec3_t dir) {
    gentity_t *bolt;
...
=================
*/
gentity_t *idSGameMissile::fire_slowBlob(gentity_t *self, vec3_t start,
        vec3_t dir) {
    gentity_t *bolt;
...
=================
*/
gentity_t *idSGameMissile::fire_bounceBall(gentity_t *self, vec3_t start,
        vec3_t dir) {
    gentity_t *bolt;
