...
void trap_LocateGameData(gentity_t *gEnts, uint64 numGEntities,
    ...
    imports->serverGameSystem->LocateGameData(gEnts, numGEntities,
...
sint trap_EntitiesInBox(const vec3_t mins, const vec3_t maxs, sint *list,
                        sint maxcount) {
    return imports->serverWorldSystem->AreaEntities(mins, maxs, list,
            maxcount);
}

bool trap_EntityContact(const vec3_t mins, const vec3_t maxs,
                        const gentity_t *ent) {
    return imports->serverGameSystem->EntityContact(mins, maxs,
            (sharedEntity_t *)ent, TT_AABB);
}

bool trap_EntityContactCapsule(const vec3_t mins, const vec3_t maxs,
                               const gentity_t *ent) {
    return imports->serverGameSystem->EntityContact(mins, maxs,
            (sharedEntity_t *)ent, TT_CAPSULE);
}

void trap_Trace(trace_t *results, const vec3_t start, const vec3_t mins,
                const vec3_t maxs, const vec3_t end, sint passEntityNum,
                sint contentmask) {
    imports->serverWorldSystem->Trace(results, start, mins, maxs, end,
                                      passEntityNum, contentmask, TT_AABB);
}
...
void trap_TraceNoEnts(trace_t *results, const vec3_t start,
                      ...
                      sint contentmask) {
    imports->serverWorldSystem->Trace(results, start, mins, maxs, end, -2,
                                      contentmask, TT_AABB);
}
...
void trap_TraceCapsule(trace_t *results, const vec3_t start,
                       ...
                       sint contentmask) {
    imports->serverWorldSystem->Trace(results, start, mins, maxs, end,
                                      passEntityNum, contentmask, TT_CAPSULE);
}
...
bool trap_GetTag(sint clientNum, sint tagFileNumber, valueType *tagname,
                 orientation_t *_or) {
    return imports->serverGameSystem->GetTag(clientNum, tagFileNumber, tagname,
            _or);
}
