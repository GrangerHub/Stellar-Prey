...
void idSGameActive::DamageFeedback( gentity_t* player )
{
    ...
    if( ( level.time > player->pain_debounce_time ) && !( player->flags & FL_GODMODE ) )
    {
        ...
        idSGameUtils::AddEvent( player, EV_PAIN, player->health > 255 ? 255 : player->health );
...
void idSGameActive::WorldEffects( gentity_t* ent )
{
    ...
    if( waterlevel == 3 )
    {
        ...
        if( ent->client->airOutTime < level.time )
        {
            ...
            if( ent->health > 0 )
            {
                ...
                {
                    idSGameUtils::Sound( ent, CHAN_VOICE, idSGameUtils::SoundIndex( "*drown.wav" ) );
                }
                else if( rand() & 1 )
                {
                    idSGameUtils::Sound( ent, CHAN_VOICE, idSGameUtils::SoundIndex( "sound/player/gurp1.wav" ) );
                }
                else
                {
                    idSGameUtils::Sound( ent, CHAN_VOICE, idSGameUtils::SoundIndex( "sound/player/gurp2.wav" ) );
                }
                
                // don't play a normal pain sound
                ent->pain_debounce_time = level.time + 200;
                
                idSGameCombat::Damage( ent, nullptr, nullptr, nullptr, nullptr, ent->damage, DAMAGE_NO_ARMOR, MOD_WATER );
            }
        }
    }
    else
    ...
    if( waterlevel && ( ent->watertype & ( CONTENTS_LAVA | CONTENTS_SLIME ) ) )
    {
        if( ent->health > 0 && ent->pain_debounce_time <= level.time )
        {
            ...
            {
                idSGameCombat::Damage( ent, nullptr, nullptr, nullptr, nullptr, 30 * waterlevel, 0, MOD_LAVA );
            }
            
            if( ent->watertype & CONTENTS_SLIME )
            {
                idSGameCombat::Damage( ent, nullptr, nullptr, nullptr, nullptr, 10 * waterlevel, 0, MOD_SLIME );
            }
        }
    }
}
...
sint idSGameActive::GetClientMass( gentity_t* ent )
{
    ...
    {
        if( bggame->InventoryContainsUpgrade( UP_BATTLESUIT, ent->client->ps.stats ) )
        {
            entMass *= 2;
        }
    }
...
void idSGameActive::ClientShove( gentity_t* ent, gentity_t* victim )
{
    ...
    if( !ent->client->pers.cmd.rightmove && !ent->client->pers.cmd.forwardmove && !ent->client->pers.cmd.upmove )
    {
        return;
    }
    
    // Cannot push enemy players unless they are walking on the player
    if( !idSGameTeam::OnSameTeam( ent, victim ) && victim->client->ps.groundEntityNum != ent - g_entities )
    {
        return;
    }
    ...
    if( !victim->client->ps.pm_time )
    {
        ...
        if( ( victim->client->ps.groundEntityNum == ent - g_entities ) && ( victim->client->ps.stats[STAT_STATE] & SS_WALLCLIMBING ) )
        {
...
void idSGameActive::TouchTriggers( gentity_t* ent )
{
    ...
    bggame->ClassBoundingBox( ( class_t )ent->client->ps.stats[ STAT_CLASS ], pmins, pmaxs, nullptr, nullptr, nullptr );
    ...
    for( i = 0; i < num; i++ )
    {
        ...
        if( ( ent->client->sess.spectatorState != SPECTATOR_NOT ) || ( ent->client->ps.stats[ STAT_STATE ] & SS_HOVELING ) )
        {
            // this is ugly but adding a new ET_? type will
            // most likely cause network incompatibilities
            if( hit->s.eType != ET_TELEPORT_TRIGGER && hit->touch != idSGameMover::Touch_DoorTrigger )
            {
...
void idSGameActive::SpectatorThink( gentity_t* ent, usercmd_t* ucmd )
{
    ...
    attack1 = ( client->buttons & BUTTON_ATTACK ) && !( client->oldbuttons & BUTTON_ATTACK );
    attack3 = ( client->buttons & BUTTON_USE_HOLDABLE ) && !( client->oldbuttons & BUTTON_USE_HOLDABLE );
    ...
    if( following )
    {
        ...
        if( clientNum < 0 || clientNum > level.maxclients || ( !g_entities[clientNum].client && !level.clients[clientNum].pers.demoClient ) || level.clients[clientNum].sess.spectatorState != SPECTATOR_NOT )
        {
            following = false;
        }
    }
    ...
    {
        queued = idSGameMain::SearchSpawnQueue( &level.alienSpawnQueue, ent - g_entities );
    }
    else if( client->pers.teamSelection == TEAM_HUMANS )
    {
        queued = idSGameMain::SearchSpawnQueue( &level.humanSpawnQueue, ent - g_entities );
    }
    else
    {
        queued = false;
    ...
    if( attack1 && queued )
    {
        ...
        {
            idSGameMain::RemoveFromSpawnQueue( &level.alienSpawnQueue, client->ps.clientNum );
        }
        else if( client->ps.stats[STAT_TEAM] == TEAM_HUMANS )
        {
            idSGameMain::RemoveFromSpawnQueue( &level.humanSpawnQueue, client->ps.clientNum );
        }
    ...
    if( !following )
    {
        ...
        if( client->ps.pm_flags & PMF_QUEUED )
        {
            if( client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
            {
                client->ps.persistant[ PERS_QUEUEPOS ] = idSGameMain::GetPosInSpawnQueue( &level.alienSpawnQueue, client->ps.clientNum );
                client->ps.persistant[ PERS_SPAWNS ] = level.numAlienSpawns;
            }
            else if( client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
            {
                client->ps.persistant[ PERS_QUEUEPOS ] = idSGameMain::GetPosInSpawnQueue( &level.humanSpawnQueue, client->ps.clientNum );
...
void idSGameActive::ClientTimerActions( gentity_t* ent, sint msec )
{
    ...
    bool walking = false, stopped = false, crouched = false, jumping = false, strafing = false;
    ...
    while( client->time100 >= 100 )
    {
        ...
        // Update build timer
        if( weapon == WP_ABUILD || weapon == WP_ABUILD2 || bggame->InventoryContainsWeapon( WP_HBUILD, client->ps.stats ) )
        {
        ...
        switch( weapon )
        {
            ...
            case WP_HBUILD:
                ...
                {
                    sint dist = bggame->Class( ( class_t )ent->client->ps.stats[STAT_CLASS] )->buildDist;
                    vec3_t dummy;
                    
                    if( idSGameBuildable::CanBuild( ent, ( buildable_t )( client->ps.stats[STAT_BUILDABLE] & ~SB_VALID_TOGGLEBIT ), dist, dummy ) == IBE_NONE )
                    {
        ...
        if( ent->client->pers.teamSelection == TEAM_HUMANS && ( client->ps.stats[ STAT_STATE ] & SS_HEALING_2X ) )
        {
            sint remainingStartupTime = MEDKIT_STARTUP_TIME - ( level.time - client->lastMedKitTime );
            
            if( remainingStartupTime < 0 )
            {
                if( ent->health < ent->client->ps.stats[ STAT_MAX_HEALTH ] && ent->client->medKitHealthToRestore && ent->client->ps.pm_type != PM_DEAD )
                {
    ...
    while( client->time1000 >= 1000 )
    {
        ...
        if( client->ps.stats[ STAT_STATE ] & SS_POISONED )
        {
            ...
            idSGameCombat::Damage( ent, client->lastPoisonClient, client->lastPoisonClient, nullptr, 0, damage, 0, MOD_POISON );
        }
        
        // turn off life support when a team admits defeat
        if( client->ps.stats[ STAT_TEAM ] == TEAM_ALIENS && level.surrenderTeam == TEAM_ALIENS )
        {
            idSGameCombat::Damage( ent, nullptr, nullptr, nullptr, nullptr, bggame->Class( ( class_t )client->ps.stats[ STAT_CLASS ] )->regenRate, DAMAGE_NO_ARMOR, MOD_SUICIDE );
        }
        else if( client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS && level.surrenderTeam == TEAM_HUMANS )
        {
            idSGameCombat::Damage( ent, nullptr, nullptr, nullptr, nullptr, 5, DAMAGE_NO_ARMOR, MOD_SUICIDE );
        }
    ...
    for( i = oldEventSequence; i < client->ps.eventSequence; i++ )
    {
        ...
        switch( _event )
        {
            ...
            case EV_FALL_FAR:
                ...
                fallDistance = ( ( float32 )client->ps.stats[ STAT_FALLDIST ] - MIN_FALL_DISTANCE ) / ( MAX_FALL_DISTANCE - MIN_FALL_DISTANCE );
                ...
                damage = ( sint )( ( float32 )bggame->Class( _class )->health * bggame->Class( _class )->fallDamage * fallDistance );
                ...
                idSGameCombat::Damage( ent, nullptr, nullptr, dir, point, damage, DAMAGE_NO_LOCDAMAGE, MOD_FALLING );
                break;
...
void idSGameActive::UnlaggedCalc( sint time, gentity_t* rewindEnt )
{
    ...
    if( frameMsec > 0 )
    {
        lerp = ( float32 )( time - level.unlaggedTimes[ startIndex ] ) / ( float32 )frameMsec;
    }
...
void idSGameActive::UnlaggedOn( gentity_t* attacker, vec3_t muzzle, float32 range )
{
    sint i = 0;
    gentity_t* ent;
    unlagged_t* calc;
...
void idSGameActive::UnlaggedDetectCollisions( gentity_t* ent )
{
    ...
    trap_Trace( &tr, ent->client->oldOrigin, ent->r.mins, ent->r.maxs, ent->client->ps.origin, ent->s.number,  MASK_PLAYERSOLID );
...
void idSGameActive::ClientThink_real( gentity_t* ent )
{
    ...
    else if( client->ps.stats[STAT_STATE] & SS_BLOBLOCKED || client->ps.stats[STAT_STATE] & SS_GRABBED )
    {
        client->ps.pm_type = PM_GRABBED;
    }
    else if( bggame->InventoryContainsUpgrade( UP_JETPACK, client->ps.stats ) && bggame->UpgradeIsActive( UP_JETPACK, client->ps.stats ) )
    {
    ...
    if( ( client->ps.stats[STAT_STATE] & SS_GRABBED ) && client->grabExpiryTime < level.time )
    {
        client->ps.stats[STAT_STATE] &= ~SS_GRABBED;
    }
    
    if( ( client->ps.stats[STAT_STATE] & SS_BLOBLOCKED ) && client->lastLockTime + LOCKBLOB_LOCKTIME < level.time )
    {
        client->ps.stats[STAT_STATE] &= ~SS_BLOBLOCKED;
    }
    
    if( ( client->ps.stats[STAT_STATE] & SS_SLOWLOCKED ) && client->lastSlowTime + ABUILDER_BLOB_TIME < level.time )
    {
    ...
    if( ( client->ps.eFlags & EF_POISONCLOUDED ) && bggame->PlayerPoisonCloudTime( &client->ps ) - level.time + client->lastPoisonCloudedTime <= 0 )
    {
        client->ps.eFlags &= ~EF_POISONCLOUDED;
    }
    
    if( client->ps.stats[STAT_STATE] & SS_POISONED && client->lastPoisonTime + ALIEN_POISON_TIME < level.time )
    {
    ...
    if( bggame->InventoryContainsUpgrade( UP_MEDKIT, client->ps.stats ) && bggame->UpgradeIsActive( UP_MEDKIT, client->ps.stats ) )
    {
        //if currently using a medkit or have no need for a medkit now
        if( client->ps.stats[ STAT_STATE ] & SS_HEALING_2X || ( client->ps.stats[ STAT_HEALTH ] == client->ps.stats[ STAT_MAX_HEALTH ] && !( client->ps.stats[ STAT_STATE ] & SS_POISONED ) ) )
        {
    ...
    {
        float32 regenRate = bggame->Class( ( class_t )ent->client->ps.stats[ STAT_CLASS ] )->regenRate;
        ...
        else
        {
            ...
            for( i = 0; i < num; i++ )
            {
                ...
                if( modifier < BOOSTER_REGEN_MOD && boost->s.eType == ET_BUILDABLE && boost->s.modelindex == BA_A_BOOSTER && boost->spawned && boost->health > 0 && level.overmindPresent )
                {
                    modifier = BOOSTER_REGEN_MOD;
                    continue;
                }
                
                if( boost->s.eType == ET_PLAYER && boost->client && boost->client->pers.teamSelection == ent->client->pers.teamSelection && boost->health > 0 )
                {
                    ...
                    else if( _class == PCL_ALIEN_LEVEL1_UPG && modifier < LEVEL1_UPG_REGEN_MOD )
                    {
    ...
    if( bggame->InventoryContainsUpgrade( UP_GRENADE, client->ps.stats ) && bggame->UpgradeIsActive( UP_GRENADE, client->ps.stats ) )
    {
    ...
    {
        client->ps.speed = g_speed.value * bggame->Class( ( class_t )client->ps.stats[STAT_CLASS] )->speed;
    }
    ...
    if( bggame->InventoryContainsUpgrade( UP_JETPACK, client->ps.stats ) && bggame->UpgradeIsActive( UP_JETPACK, client->ps.stats ) )
    {
    ...
    {
        bggame->PlayerStateToEntityStateExtraPolate( &ent->client->ps, &ent->s, ent->client->ps.commandTime, true );
    }
    else
    {
    ...
    if( ( client->buttons & BUTTON_USE_EVOLVE ) && !( client->oldbuttons & BUTTON_USE_EVOLVE ) && client->ps.stats[ STAT_HEALTH ] > 0 )
    {
        trace_t trace;
        vec3_t view, point;
        gentity_t* traceEnt;
...
#define USE_OBJECT_RANGE 64
            ...
            trap_Trace( &trace, client->ps.origin, nullptr, nullptr, point, ent->s.number, MASK_SHOT );
            ...
            else
            {
                ...
                {
                    if( bggame->AlienCanEvolve( ( class_t )client->ps.stats[ STAT_CLASS ], client->ps.persistant[ PERS_CREDIT ], g_alienStage.integer ) )
                    {
