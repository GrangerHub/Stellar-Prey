...
void idSGamePhysics::Bounce(gentity_t *ent, trace_t *trace) {
    ...
    if((trace->plane.normal[2] >= minNormal || (invert &&
...
void idSGamePhysics::Physics(gentity_t *ent, sint msec) {
    ...
    if(ent->s.groundEntityNum == -1) {
        if(ent->s.eType == ET_BUILDABLE) {
            if(ent->s.pos.trType != bggame->Buildable((buildable_t)
                    ent->s.modelindex)->traj) {
                ent->s.pos.trType = bggame->Buildable((buildable_t)
                                                      ent->s.modelindex)->traj;
                ent->s.pos.trTime = level.time;
            }
