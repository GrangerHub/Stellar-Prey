...
void idSGameTeam::PrintMsg( gentity_t* ent, pointer fmt, ... )
{
    ...
    trap_SendServerCommand( ( ( ent == nullptr ) ? -1 : ent - g_entities ), va( "print \"%s\"", msg ) );
}
...
void idSGameTeam::LeaveTeam( gentity_t* self )
{
    ...
    if( team == TEAM_ALIENS )
    {
        idSGameMain::RemoveFromSpawnQueue( &level.alienSpawnQueue, self->client->ps.clientNum );
    }
    else if( team == TEAM_HUMANS )
    {
        idSGameMain::RemoveFromSpawnQueue( &level.humanSpawnQueue, self->client->ps.clientNum );
    }
    else
    {
...
void idSGameTeam::ChangeTeam( gentity_t* ent, team_t newTeam )
{
    ...
    if( adminLocal.AdminPermission( ent, ADMF_TEAMCHANGEFREE ) || ( ( oldTeam == TEAM_HUMANS || oldTeam == TEAM_ALIENS ) && ( level.time - ent->client->pers.teamChangeTime ) > 60000 ) )
    {
        ...
        else if( oldTeam == TEAM_HUMANS )
        {
            ...
            ent->client->ps.persistant[ PERS_CREDIT ] *= ( ALIEN_MAX_CREDITS / ( float32 )HUMAN_MAX_CREDITS );
        }
        ...
        else if( newTeam == TEAM_HUMANS )
        {
            ...
            ent->client->ps.persistant[ PERS_CREDIT ] *= ( HUMAN_MAX_CREDITS / ( float32 )ALIEN_MAX_CREDITS );
        }
    }
    else
    {
...
============
*/
bool idSGameTeam::Team_GetLocationMsg( gentity_t* ent, valueType* loc, sint loclen )
{
    gentity_t* best;
