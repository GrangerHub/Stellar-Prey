...
void idSGameCmds::Cmd_Team_f(gentity_t *ent) {
    ...
    if(oldteam != TEAM_NONE &&
        ...
        if(ent->client->pers.teamSelection == TEAM_ALIENS) {
            idSGameUtils::TriggerMenu(ent->client->ps.clientNum,
...
void idSGameCmds::Cmd_VSay_f(gentity_t *ent) {
    ...
    track = bggame->VoiceTrackFind(cmd->tracks,
                                   ent->client->pers.teamSelection,
                                   ent->client->pers.classSelection, weapon,
                                   (sint)ent->client->voiceEnthusiasm,
                                   &trackNum);
...
void idSGameCmds::Cmd_CallVote_f(gentity_t *ent) {
    ...
    if(!Q_stricmp(arg1, "kick")) {
        ...
        Q_vsprintf_s(level.voteString, sizeof(level.voteString),
                     sizeof(level.voteString),
        ...
        Q_vsprintf_s(level.voteString, sizeof(level.voteString),
                     sizeof(level.voteString), "%s", arg1);
        Q_vsprintf_s(level.voteDisplayString, sizeof(level.voteDisplayString),
        ...
        if(!g_suddenDeathVotePercent.integer) {
            ...
            Q_vsprintf_s(level.voteString, sizeof(level.voteString),
                         sizeof(level.voteString), "suddendeath");
            Q_vsprintf_s(level.voteDisplayString, sizeof(level.voteDisplayString),
...
void idSGameCmds::Cmd_CallTeamVote_f(gentity_t *ent) {
    ...
    if(!Q_stricmp(arg1, "kick")) {
        ...
        // use ip in case this player disconnects before the vote ends
        Q_vsprintf_s(level.teamVoteString[ cs_offset ],
                     sizeof(level.teamVoteString[cs_offset]),
                     sizeof(level.teamVoteString[ cs_offset ]),
                     "!ban %s \"1s%s\" team vote kick", level.clients[ clientNum ].pers.ip,
                     g_adminTempBan.string);
        Q_vsprintf_s(level.teamVoteDisplayString[ cs_offset ],
...
bool idSGameCmds::RoomForClassChange(gentity_t *ent, class_t _class,
                                     vec3_t newOrigin) {
    ...
    bggame->ClassBoundingBox(oldClass, fromMins, fromMaxs, nullptr, nullptr,
                             nullptr);
    bggame->ClassBoundingBox(_class, toMins, toMaxs, nullptr, nullptr,
                             nullptr);
    ...
    if(ent->client->sess.spectatorState != SPECTATOR_NOT) {
        ...
        if(ent->client->pers.teamSelection == TEAM_ALIENS) {
            if(newClass != PCL_ALIEN_BUILDER0 &&
                ...
                idSGameUtils::TriggerMenu2(ent->client->ps.clientNum, MN_A_CLASSNOTSPAWN,
                                           newClass);
                return;
            }

            if(!bggame->ClassIsAllowed(newClass)) {
                idSGameUtils::TriggerMenu2(ent->client->ps.clientNum, MN_A_CLASSNOTALLOWED,
                                           newClass);
                return;
            }

            if(!bggame->ClassAllowedInStage(newClass, (stage_t)g_alienStage.integer)) {
                idSGameUtils::TriggerMenu2(ent->client->ps.clientNum, MN_A_CLASSNOTATSTAGE,
                                           newClass);
                return;
            }
    ...
    if(ent->client->pers.teamSelection == TEAM_ALIENS) {
        ...
        if(ent->client->pers.classSelection != PCL_NONE) {
            ...
            if(RoomForClassChange(ent, newClass, infestOrigin)) {
                if(cost >= 0) {
                    ent->client->pers.evolveHealthFraction = (float32)
                    ...
                    idSGameClient::ClientSpawn(ent, ent, ent->s.pos.trBase,
                                               ent->s.apos.trBase);
...
void idSGameCmds::Cmd_Destroy_f(gentity_t *ent) {
    ...
    if(ent->client->ps.stats[ STAT_STATE ] & SS_HOVELING) {
        idSGameCombat::Damage(ent->client->hovel, ent, ent, forward, ent->s.origin,
                              10000, 0, MOD_SUICIDE);
    }
...
void idSGameCmds::Cmd_ActivateItem_f(gentity_t *ent) {
    ...
    if(!Q_stricmp("weapon", s)) {
        if(ent->client->ps.weapon == WP_BLASTER &&
                bggame->PlayerCanChangeWeapon(&ent->client->ps)) {
    ...
    if(upgrade != UP_NONE &&
            bggame->InventoryContainsUpgrade(upgrade, ent->client->ps.stats)) {
...
void idSGameCmds::Cmd_ToggleItem_f(gentity_t *ent) {
    ...
    if(weapon != WP_NONE) {
        ...
        idSGameWeapons::ForceWeaponChange(ent, (weapon_t)weapon);
    } else if(bggame->InventoryContainsUpgrade(upgrade,
              ent->client->ps.stats)) {
...
void idSGameCmds::Cmd_Buy_f(gentity_t *ent) {
    ...
    if(idSGameBuildable::BuildableRange(ent->client->ps.origin, 100,
                                        BA_H_ARMOURY)) {
        energyOnly = false;
    } else if(upgrade == UP_AMMO &&
              bggame->Weapon((weapon_t)ent->client->ps.stats[ STAT_WEAPON ])->usesEnergy
              &&
              (idSGameBuildable::BuildableRange(ent->client->ps.origin, 100,
                      BA_H_REACTOR) ||
               idSGameBuildable::BuildableRange(ent->client->ps.origin, 100,
                       BA_H_REPEATER))) {
        energyOnly = true;
    ...
    if(weapon != WP_NONE) {
        ...
        //are we /allowed/ to buy this?
        if(!bggame->WeaponAllowedInStage(weapon, (stage_t)g_humanStage.integer) ||
                !bggame->WeaponIsAllowed(weapon)) {
            trap_SendServerCommand(ent - g_entities,
                                   "print \"You can't buy this item\n\"");
            return;
        }
        ...
        //can afford this?
        if(bggame->Weapon(weapon)->price > (schar16)
                ent->client->ps.persistant[ PERS_CREDIT ]) {
        ...
        if(bggame->Weapon(weapon)->slots & bggame->CalculateSlotsForInventory(
                    ent->client->ps.stats)) {
        ...
        //subtract from funds
        idSGameClient::AddCreditToClient(ent->client,
                                         -(schar16)bggame->Weapon(weapon)->price, false);
        ...
        //can afford this?
        if(bggame->Upgrade(upgrade)->price > (schar16)
                ent->client->ps.persistant[ PERS_CREDIT ]) {
            idSGameUtils::TriggerMenu(ent->client->ps.clientNum, MN_H_NOFUNDS);
            return;
        }
        ...
        //have space to carry this?
        if(bggame->Upgrade(upgrade)->slots & bggame->CalculateSlotsForInventory(
                    ent->client->ps.stats)) {
            idSGameUtils::TriggerMenu(ent->client->ps.clientNum, MN_H_NOSLOTS);
            return;
        }
        ...
        //subtract from funds
        idSGameClient::AddCreditToClient(ent->client,
                                         -(schar16)bggame->Upgrade(upgrade)->price, false);
    } else {
...
void idSGameCmds::Cmd_Sell_f(gentity_t *ent) {
    ...
    if(weapon != WP_NONE) {
        ...
        if(bggame->InventoryContainsWeapon(weapon, ent->client->ps.stats)) {
            ...
            //add to funds
            idSGameClient::AddCreditToClient(ent->client,
                                             (schar16)bggame->Weapon((weapon_t)weapon)->price, false);
        }
        ...
        if(bggame->InventoryContainsUpgrade(upgrade, ent->client->ps.stats)) {
            ...
            //add to funds
            idSGameClient::AddCreditToClient(ent->client,
                                             (schar16)bggame->Upgrade((upgrade_t)upgrade)->price, false);
        }
        ...
        for(i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++) {
            ...
            if(bggame->InventoryContainsWeapon(i, ent->client->ps.stats) &&
                ...
                //add to funds
                idSGameClient::AddCreditToClient(ent->client,
                                                 (schar16)bggame->Weapon((weapon_t)i)->price, false);
            }
        ...
        for(i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++) {
            ...
            if(bggame->InventoryContainsUpgrade(i, ent->client->ps.stats) &&
                ...
                //add to funds
                idSGameClient::AddCreditToClient(ent->client,
                                                 (schar16)bggame->Upgrade((upgrade_t)i)->price, false);
            }
        }
    } else {
        idSGameUtils::TriggerMenu(ent->client->ps.clientNum, MN_H_UNKNOWNITEM);
...
void idSGameCmds::Cmd_Build_f(gentity_t *ent) {
    ...
    if(buildable != BA_NONE &&
            ...
            ((team == TEAM_ALIENS &&
              bggame->BuildableAllowedInStage(buildable,
                      (stage_t)g_alienStage.integer)) ||
             (team == TEAM_HUMANS &&
              bggame->BuildableAllowedInStage(buildable,
                      (stage_t)g_humanStage.integer)))) {
...
void idSGameCmds::StopFollowing(gentity_t *ent) {
    ...
    if(ent->client->pers.teamSelection == TEAM_NONE) {
        ent->client->sess.spectatorState = (spectatorState_t)(
...
void idSGameCmds::Cmd_PTRCRestore_f(gentity_t *ent) {
    ...
    if(connection && connection->ptrCode == code) {
        ...
        idSGameClient::AddCreditToClient(ent->client, connection->clientCredit,
                                         true);

        if(connection->oldClient != ent - g_entities) {
            idSGameClient::AddCreditToClient(&level.clients[ connection->oldClient ],
                                             -connection->clientCredit, true);
        }
...
void idSGameCmds::Cmd_Damage_f(gentity_t *ent) {
    ...
    point[ 1 ] += dy;
    point[ 2 ] += dz;
    idSGameCombat::Damage(ent, nullptr, nullptr, nullptr, point, damage,
...
void idSGameCmds::EditPlayerInventory(gentity_t *ent) {
    ...
    if(Q_stricmp(itstr, "dmg") == 0) {
        trap_Argv(2, arg, sizeof(arg));
        idSGameCombat::Damage(ent, nullptr, nullptr, nullptr, nullptr, atoi(arg),
                              0, MOD_UNKNOWN);
...
void idSGameCmds::Share_f(gentity_t *ent) {
    ...
    if((team == TEAM_HUMANS) &&
            (creds > HUMAN_MAX_CREDITS -
             level.clients[clientNum].ps.persistant[PERS_CREDIT])) {
        creds = HUMAN_MAX_CREDITS -
                level.clients[clientNum].ps.persistant[PERS_CREDIT];
    } else if((team == TEAM_ALIENS) &&
              (creds > ALIEN_MAX_KILLS -
               level.clients[clientNum].ps.persistant[PERS_CREDIT])) {
        creds = ALIEN_MAX_KILLS -
                level.clients[clientNum].ps.persistant[PERS_CREDIT];
    }
