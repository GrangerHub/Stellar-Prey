...
==================
*/
void idSGameCombat::LookAtKiller( gentity_t* self, gentity_t* inflictor, gentity_t* attacker )
{
...
==================
*/
void idSGameCombat::player_die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, sint damage, sint meansOfDeath )
{
    gentity_t* ent;
    sint       anim;
    sint       killer;
    ...
    if( attacker && attacker->client )
    {
        ...
        if( attacker == self || idSGameTeam::OnSameTeam( self, attacker ) )
        {
            ...
            {
                idSGameClient::AddCreditToClient( attacker->client, -ALIEN_TK_SUICIDE_PENALTY, true );
                AddScore( attacker, -ALIEN_TK_SUICIDE_PENALTY );
            }
            else if( attacker->client->ps.stats[ STAT_TEAM ] == TEAM_HUMANS )
            {
                idSGameClient::AddCreditToClient( attacker->client, -HUMAN_TK_SUICIDE_PENALTY, true );
    ...
    
    {
        ...
        idSGameUtils::AddEvent( self, EV_DEATH1 + i, ( killer < MAX_CLIENTS ) ? killer : self - g_entities );
...
===============
*/
sint idSGameCombat::ParseDmgScript( damageRegion_t* regions, valueType* buf )
{
    valueType*  token;
...
============
*/
float32 idSGameCombat::GetRegionDamageModifier( gentity_t* targ, sint _class, sint piece )
{
...
============
*/
float32 idSGameCombat::GetNonLocDamageModifier( gentity_t* targ, sint _class )
{
...
============
*/
float32 idSGameCombat::GetPointDamageModifier( gentity_t* targ, damageRegion_t* regions, sint len, float32 angle, float32 height )
{
...
============
*/
float32 idSGameCombat::CalcDamageModifier( vec3_t point, gentity_t* targ, gentity_t* attacker, sint _class, sint dflags )
{
...
void idSGameCombat::InitDamageLocations( void )
{
    ...
    for( i = PCL_NONE + 1; i < PCL_NUM_CLASSES; i++ )
    {
        ...
        {
            idSGameMain::Printf( S_COLOR_RED "file too large: %s is %i, max allowed is %i", filename, len, MAX_DAMAGE_REGION_TEXT );
...
void idSGameCombat::Damage( gentity_t* targ, gentity_t* inflictor, gentity_t* attacker, vec3_t dir, vec3_t point, sint damage, sint dflags, sint mod )
{
    gclient_t* client;
    sint     take;
    sint     save;
    ...
    {
        knockback = ( sint )( ( float32 )knockback *
                              bggame->Class( ( class_t )targ->client->ps.stats[ STAT_CLASS ] )->knockbackScale );
    }
    ...
    if( !( dflags & DAMAGE_NO_PROTECTION ) )
    {
        ...
        if( targ->s.eType == ET_BUILDABLE && attacker->client )
        {
            ...
            if( targ->buildableTeam == TEAM_HUMANS && idSGameBuildable::FindDCC( targ ) && level.time > level.humanBaseAttackTimer && mod != MOD_DECONSTRUCT && mod != MOD_SUICIDE )
            {
    ...
    if( client )
    {
        ...
        take = ( sint )( take * CalcDamageModifier( point, targ, attacker, client->ps.stats[ STAT_CLASS ], dflags ) + 0.5f );
    ...
    if( take )
    {
        ...
        if( attacker->client && attacker != targ && !idSGameTeam::OnSameTeam( targ, attacker ) )
...
============
*/
bool idSGameCombat::SelectiveRadiusDamage( vec3_t origin, gentity_t* attacker, float32 damage, float32 radius, gentity_t* ignore, sint mod, sint team )
{
    float32     points, dist;
    gentity_t* ent;
    ...
    for( e = 0; e < numListedEntities; e++ )
    {
        ...
        if( CanDamage( ent, origin ) && ent->client && ent->client->ps.stats[ STAT_TEAM ] != team )
        {
            ...
            dir[ 2 ] += 24;
            hitClient = true;
            Damage( ent, nullptr, attacker, dir, origin, ( sint )points, DAMAGE_RADIUS | DAMAGE_NO_LOCDAMAGE, mod );
        }
    }
...
============
*/
bool idSGameCombat::RadiusDamage( vec3_t origin, gentity_t* attacker, float32 damage, float32 radius, gentity_t* ignore, sint mod )
{
    float32     points, dist;
    gentity_t* ent;
    sint       entityList[ MAX_GENTITIES ];
...
================
*/
void idSGameCombat::LogDestruction( gentity_t* self, gentity_t* actor, sint mod )
{
