...
================
*/
void idSGameMover::CreateRotationMatrix( vec3_t angles, vec3_t matrix[ 3 ] )
{
...
================
*/
void idSGameMover::TransposeMatrix( vec3_t matrix[ 3 ], vec3_t transpose[ 3 ] )
{
    sint i, j;
...
==================
*/
bool idSGameMover::TryPushingEntity( gentity_t* check, gentity_t* pusher, vec3_t move, vec3_t amove )
{
...
============
*/
bool idSGameMover::MoverPush( gentity_t* pusher, vec3_t move, vec3_t amove, gentity_t** obstacle )
{
    sint       i, e;
    gentity_t* check;
    vec3_t    mins, maxs;
    ...
    *obstacle = nullptr;
    ...
    for( e = 0 ; e < listedEntities ; e++ )
    {
        ...
        {
            idSGameCombat::Damage( check, pusher, pusher, nullptr, nullptr, 99999, 0, MOD_CRUSH );
            continue;
        }
...
void idSGameMover::MoverTeam( gentity_t* ent )
{
    ...
    if( part )
    {
        ...
        for( part = ent; part; part = part->teamchain )
        {
            ...
            bggame->EvaluateTrajectory( &part->s.pos, level.time, part->r.currentOrigin );
            bggame->EvaluateTrajectory( &part->s.apos, level.time, part->r.currentAngles );
...
===============
*/
void idSGameMover::SetMoverState( gentity_t* ent, moverState_t moverState, sint time )
{
    vec3_t delta;
    float32  f;
    ...
    if( moverState >= ROTATOR_POS1 && moverState <= ROTATOR_2TO1 )
        bggame->EvaluateTrajectory( &ent->s.apos, level.time, ent->r.currentAngles );
...
================
*/
void idSGameMover::MatchTeam( gentity_t* teamLeader, sint moverState, sint time )
{
    gentity_t*   slave;
...
================
*/
void idSGameMover::Use_BinaryMover( gentity_t* ent, gentity_t* other, gentity_t* activator )
{
    sint total, partial;
...
void idSGameMover::Blocked_Door( gentity_t* ent, gentity_t* other )
{
    ...
    if( ent->damage )
        idSGameCombat::Damage( other, ent, ent, nullptr, nullptr, ent->damage, 0, MOD_CRUSH );
...
================
*/
void idSGameMover::Touch_DoorTriggerSpectator( gentity_t* ent, gentity_t* other, trace_t* trace )
{
    sint     i, axis;
...
================
*/
void idSGameMover::manualDoorTriggerSpectator( gentity_t* door, gentity_t* player )
{
    gentity_t* other;
...
================
*/
void idSGameMover::manualTriggerSpectator( gentity_t* trigger, gentity_t* player )
{
    gentity_t* t = nullptr;
    ...
    while( ( t = idSGameUtils::Find( t, FOFS( targetname ), trigger->target ) ) != nullptr )
    {
    ...
    if( i > 0 )
    {
        ...
        for( j = 0; j < i; j++ )
        {
            float32 d = Distance( player->r.currentOrigin, targets[ j ]->r.currentOrigin );
...
================
*/
void idSGameMover::Touch_DoorTrigger( gentity_t* ent, gentity_t* other, trace_t* trace )
{
...
void idSGameMover::SP_func_door( gentity_t* ent )
{
    ...
    idSGameSpawn::SpawnString( "sound2to1", "sound/movers/doors/dr1_strt.wav", &s );
    ent->sound2to1 = idSGameUtils::SoundIndex( s );
    idSGameSpawn::SpawnString( "sound1to2", "sound/movers/doors/dr1_strt.wav", &s );
    ent->sound1to2 = idSGameUtils::SoundIndex( s );
    
    idSGameSpawn::SpawnString( "soundPos2", "sound/movers/doors/dr1_end.wav", &s );
    ent->soundPos2 = idSGameUtils::SoundIndex( s );
    idSGameSpawn::SpawnString( "soundPos1", "sound/movers/doors/dr1_end.wav", &s );
...
void idSGameMover::SP_func_door_model( gentity_t* ent )
{
    ...
    // for drawing, but clip against the brushes
    if( !ent->model2 )
        idSGameMain::Printf( S_COLOR_YELLOW "WARNING: func_door_model %d spawned with no model2 key\n", ent->s.number );
    else
    ...
    ent->s.weapon = abs( ( sint )ent->animation[ 1 ] );       //number of frames
...
===============
*/
void idSGameMover::Touch_Plat( gentity_t* ent, gentity_t* other, trace_t* trace )
{
    // DONT_WAIT
    if( ent->spawnflags & 1 )
        return;
...
===============
*/
void idSGameMover::Touch_PlatCenterTrigger( gentity_t* ent, gentity_t* other, trace_t* trace )
{
    if( !other->client )
        return;
...
void idSGameMover::SP_func_plat( gentity_t* ent )
{
    ...
    idSGameSpawn::SpawnString( "sound2to1", "sound/movers/plats/pt1_strt.wav", &s );
    ent->sound2to1 = idSGameUtils::SoundIndex( s );
    idSGameSpawn::SpawnString( "sound1to2", "sound/movers/plats/pt1_strt.wav", &s );
    ent->sound1to2 = idSGameUtils::SoundIndex( s );
    
    idSGameSpawn::SpawnString( "soundPos2", "sound/movers/plats/pt1_end.wav", &s );
    ent->soundPos2 = idSGameUtils::SoundIndex( s );
    idSGameSpawn::SpawnString( "soundPos1", "sound/movers/plats/pt1_end.wav", &s );
    ent->soundPos1 = idSGameUtils::SoundIndex( s );
...
===============
*/
void idSGameMover::Touch_Button( gentity_t* ent, gentity_t* other, trace_t* trace )
{
    if( !other->client )
        return;
...
void idSGameMover::SP_func_button( gentity_t* ent )
{
    ...
    idSGameSpawn::SpawnString( "sound1to2", "sound/movers/switches/button1.wav", &s );
    ent->sound1to2 = idSGameUtils::SoundIndex( s );
...
================
*/
void idSGameMover::Start_Train( gentity_t* ent, gentity_t* other, gentity_t* activator )
{
    vec3_t  move;
...
================
*/
void idSGameMover::Stop_Train( gentity_t* ent, gentity_t* other, gentity_t* activator )
{
    vec3_t  origin;
...
================
*/
void idSGameMover::Use_Train( gentity_t* ent, gentity_t* other, gentity_t* activator )
{
    if( ent->spawnflags & TRAIN_START_OFF )
    {
...
void idSGameMover::Think_SetupTrainTargets( gentity_t* ent )
{
    ...
    ent->nextTrain = idSGameUtils::Find( nullptr, FOFS( targetname ), ent->target );
...
void idSGameMover::Blocked_Train( gentity_t* self, gentity_t* other )
{
    ...
    else
    {
        if( !other->client )
        {
            ...
            //KILL!!1!!!
            idSGameCombat::Damage( other, self, self, nullptr, nullptr, 10000, 0, MOD_CRUSH );
            ...
            if( other->s.eType == ET_BUILDABLE && other->spawned )
            {
                ...
                if( other->buildableTeam == TEAM_ALIENS )
                {
                    ...
                    tent = idSGameUtils::TempEntity( other->s.origin, EV_ALIEN_BUILDABLE_EXPLOSION );
                    tent->s.eventParm = DirToByte( dir );
                }
                else if( other->buildableTeam == TEAM_HUMANS )
                {
                    VectorSet( dir, 0.0f, 0.0f, 1.0f );
                    tent = idSGameUtils::TempEntity( other->s.origin, EV_HUMAN_BUILDABLE_EXPLOSION );
                    tent->s.eventParm = DirToByte( dir );
                }
            }
