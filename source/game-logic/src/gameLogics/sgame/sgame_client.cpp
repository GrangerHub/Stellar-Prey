...
===============
*/
void idSGameClient::AddCreditToClient(gclient_t *client, schar16 credit,
                                      bool cap) {
    if(!client) {
        return;
    }
...
gentity_t *idSGameClient::SelectNearestDeathmatchSpawnPoint(vec3_t from) {
    ...
    while((spot = idSGameUtils::Find(spot, FOFS(classname),
                                     "info_player_deathmatch")) != nullptr) {
...
gentity_t *idSGameClient::SelectRandomDeathmatchSpawnPoint(void) {
    ...
    if(!count) {  // no spots that won't telefrag
        return idSGameUtils::Find(nullptr, FOFS(classname),
...
============
*/
gentity_t *idSGameClient::SelectRandomFurthestSpawnPoint(vec3_t avoidPoint,
    ...
    if(!numSpots) {
        spot = idSGameUtils::Find(nullptr, FOFS(classname),
                                  "info_player_deathmatch");
...
gentity_t *idSGameClient::SelectAlienSpawnPoint(vec3_t preference) {
    ...
    while((spot = idSGameUtils::Find(spot, FOFS(classname),
                                     bggame->Buildable(BA_A_SPAWN)->entityName)) != nullptr) {
        if(!spot->spawned) {
            continue;
        }
        ...
        if(idSGameBuildable::CheckSpawnPoint(spot->s.number, spot->s.origin,
                                             spot->s.origin2, BA_A_SPAWN, nullptr) != nullptr) {
            continue;
        }
...
gentity_t *idSGameClient::SelectHumanSpawnPoint(vec3_t preference) {
    ...
    while((spot = idSGameUtils::Find(spot, FOFS(classname),
                                     bggame->Buildable(BA_H_SPAWN)->entityName)) != nullptr) {
        if(!spot->spawned) {
            continue;
        }
        ...
        if(idSGameBuildable::CheckSpawnPoint(spot->s.number, spot->s.origin,
                                             spot->s.origin2, BA_H_SPAWN, nullptr) != nullptr) {
            continue;
        }
...
============
*/
gentity_t *idSGameClient::SelectSpawnPoint(vec3_t avoidPoint,
        vec3_t origin, vec3_t angles) {
...
============
*/
gentity_t *idSGameClient::SelectTremulousSpawnPoint(team_t team,
    ...
    if(team == TEAM_ALIENS) {
        idSGameBuildable::CheckSpawnPoint(spot->s.number, spot->s.origin,
...
============
*/
gentity_t *idSGameClient::SelectInitialSpawnPoint(vec3_t origin,
        vec3_t angles) {
    gentity_t *spot;
...
============
*/
gentity_t *idSGameClient::SelectSpectatorSpawnPoint(vec3_t origin,
        vec3_t angles) {
...
============
*/
gentity_t *idSGameClient::SelectAlienLockSpawnPoint(vec3_t origin,
        vec3_t angles) {
    gentity_t *spot;

    spot = nullptr;
    spot = idSGameUtils::Find(spot, FOFS(classname),
...
============
*/
gentity_t *idSGameClient::SelectHumanLockSpawnPoint(vec3_t origin,
        vec3_t angles) {
    gentity_t *spot;
...
void idSGameClient::SpawnCorpse(gentity_t *ent) {
    ...
    body->nonSegModel = ent->client->ps.persistant[ PERS_STATE ] &
                        PS_NONSEGMODEL;
    ...
    bggame->ClassBoundingBox((class_t)ent->client->ps.stats[ STAT_CLASS ],
                             nullptr, nullptr, nullptr, body->r.mins, body->r.maxs);
    ...
    trap_Trace(&tr, origin, body->r.mins, body->r.maxs, dest, body->s.number,
               body->clipmask);
...
void idSGameLocal::ClientUserinfoChanged(sint clientNum) {
    ...
    if(client->pers.classSelection == PCL_NONE) {
        ...
        Q_vsprintf_s(buffer, MAX_QPATH, MAX_QPATH, "%s/%s",
                     bggame->ClassConfig(PCL_HUMAN_BSUIT)->modelName,
...
void idSGameLocal::ClientBegin(sint clientNum) {
    ...
    if(Q_stricmp(Info_ValueForKey(userinfo, "cg_version"), PRODUCT_NAME)) {
        trap_SendServerCommand(clientNum,
                               "disconnect \"Your client is missing files.\n\n"
...
============
*/
void idSGameClient::ClientSpawn(gentity_t *ent, gentity_t *spawn,
                                vec3_t origin, vec3_t angles) {
    sint                 index;
    ...
    if(client->sess.spectatorState != SPECTATOR_NOT) {
        if(teamLocal == TEAM_NONE) {
            ...
            spawnPoint = idSGameClient::SelectHumanLockSpawnPoint(spawn_origin,
                         spawn_angles);
        }
    } else {
        if(spawn == nullptr) {
    ...
    bggame->ClassBoundingBox(ent->client->pers.classSelection, ent->r.mins,
                             ent->r.maxs, nullptr, nullptr, nullptr);
...
void idSGameLocal::ClientDisconnect(sint clientNum) {
    ...
    if(ent->client->pers.connected == CON_CONNECTED &&
        ...
        tent = idSGameUtils::TempEntity(ent->client->ps.origin,
