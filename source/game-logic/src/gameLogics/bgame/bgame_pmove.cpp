...
==================
*/
void idBothGamesLocal::ClipVelocity( vec3_t in, vec3_t normal, vec3_t out, float32 overbounce )
{
    ...
    if( pm->waterlevel <= 1 )
    {
        if( ( pml.walking || pml.ladder ) && !( pml.groundTrace.surfaceFlags & SURF_SLICK ) )
        {
            ...
            if( !( pm->ps->pm_flags & PMF_TIME_KNOCKBACK ) )
            {
                ...
                {
                    friction *= 1.f + HUMAN_LAND_FRICTION * pm->ps->pm_time / HUMAN_DODGE_TIMEOUT;
                }
...
==============
*/
void idBothGamesLocal::Accelerate( vec3_t wishdir, float32 wishspeed, float32 accel )
{
...
float32 idBothGamesLocal::CmdScale( usercmd_t* cmd )
{
    ...
    if( pm->ps->stats[STAT_TEAM] == TEAM_HUMANS && pm->ps->pm_type == PM_NORMAL )
    {
        ...
        {
            if( InventoryContainsUpgrade( UP_LIGHTARMOUR, pm->ps->stats ) || InventoryContainsUpgrade( UP_BATTLESUIT, pm->ps->stats ) )
            {
    ...
    {
        modifier *= 1.0f + ( pm->ps->stats[STAT_MISC] * ( LEVEL4_TRAMPLE_SPEED - 1.0f ) / LEVEL4_TRAMPLE_DURATION );
    }
...
void idBothGamesLocal::SetMovementDir( void )
{
    ...
    if( ( pm->cmd.forwardmove || pm->cmd.rightmove ) && ( pm->ps->groundEntityNum != ENTITYNUM_NONE ) && ( speed = VectorLength( moved ) ) && ( speed > pml.frametime * 5 ) )
    {
...
void idBothGamesLocal::CheckCharge( void )
{
    ...
    if( pm->cmd.buttons & BUTTON_ATTACK2 && !( pm->ps->stats[STAT_STATE] & SS_CHARGING ) )
    {
...
bool idBothGamesLocal::CheckPounce( void )
{
    ...
    if( ( pm->ps->pm_flags & PMF_CHARGE ) || pm->ps->stats[STAT_MISC] < LEVEL3_POUNCE_TIME_MIN || pm->ps->groundEntityNum == ENTITYNUM_NONE )
    {
        return false;
    }
...
bool idBothGamesLocal::CheckWallJump( void )
{
    ...
    if( !( Class( ( class_t )pm->ps->stats[STAT_CLASS] )->abilities & SCA_WALLJUMPER ) )
    {
        return false;
    }
    ...
    if( trace.fraction < 1.0f && !( trace.surfaceFlags & ( SURF_SKY | SURF_SLICK ) ) && trace.plane.normal[2] < MIN_WALK_NORMAL )
    {
    ...
    VectorMA( pm->ps->velocity, Class( ( class_t )pm->ps->stats[STAT_CLASS] )->jumpMagnitude, dir, pm->ps->velocity );
...
bool idBothGamesLocal::CheckJump( void )
{
    ...
    if( ( pm->ps->stats[STAT_TEAM] == TEAM_HUMANS ) && ( pm->ps->stats[STAT_STAMINA] < 0 ) )
    {
        return false;
    }
    ...
    VectorMA( pm->ps->velocity, Class( ( class_t )pm->ps->stats[STAT_CLASS] )->jumpMagnitude, normal, pm->ps->velocity );
...
bool idBothGamesLocal::CheckDodge( void )
{
    ...
    // Landed a dodge
    if( ( pm->ps->pm_flags & PMF_CHARGE ) && pm->ps->groundEntityNum != ENTITYNUM_NONE )
    {
    ...
    if( pm->cmd.forwardmove <= 0 || pm->cmd.upmove < 0 || pm->ps->pm_type != PM_NORMAL || pm->cmd.buttons & BUTTON_WALKING )
    {
        pm->ps->stats[STAT_STATE] &= ~SS_SPEEDBOOST;
    }
    
    // Reasons why we can't start a dodge or sprint
    if( pm->ps->pm_type != PM_NORMAL || pm->ps->stats[STAT_STAMINA] < 0 || ( pm->ps->pm_flags & PMF_CROUCH_HELD ) )
    {
        return false;
    }
    
    // Start a sprint instead of forward leaps
    if( pm->cmd.forwardmove > 0 && ( ( pm->cmd.buttons & BUTTON_DODGE ) || ( pm->ps->persistant[PERS_STATE] & PS_ALWAYSSPRINT ) ) )
    {
    ...
    if( pm->ps->pm_flags & ( PMF_TIME_LAND | PMF_CHARGE ) || pm->ps->groundEntityNum == ENTITYNUM_NONE || !( pm->cmd.buttons & BUTTON_DODGE ) )
    {
        return false;
    }
    ...
    for( i = 0; i < 3; i++ )
    {
        if( ( velocity[i] < 0.f && pm->ps->velocity[i] > velocity[i] ) || ( velocity[i] > 0.f && pm->ps->velocity[i] < velocity[i] ) )
        {
    ...
    pm->ps->legsAnim = ( ( pm->ps->legsAnim & ANIM_TOGGLEBIT ) ^ ANIM_TOGGLEBIT ) | LEGS_JUMP;
    AddEvent( EV_JUMP );
...
void idBothGamesLocal::AirMove( void )
{
    ...
    Accelerate( wishdir, wishspeed, Class( ( class_t )pm->ps->stats[STAT_CLASS] )->airAcceleration );
...
void idBothGamesLocal::CheckLadder( void )
{
    ...
    if( !ClassHasAbility( ( class_t )pm->ps->stats[STAT_CLASS], SCA_CANUSELADDERS ) )
    {
        pml.ladder = false;
        return;
    }
...
void idBothGamesLocal::GroundTraceMissed( void )
{
    ...
    if( pm->ps->groundEntityNum != ENTITYNUM_NONE )
    {
        ...
        pm->trace( &trace, pm->ps->origin, nullptr, nullptr, point, pm->ps->clientNum, pm->tracemask );
    ...
    if( ClassHasAbility( ( class_t )pm->ps->stats[STAT_CLASS], SCA_TAKESFALLDAMAGE ) )
    {
...
void idBothGamesLocal::GroundClimbTrace( void )
{
    ...
    float32 traceDOTsurf, traceDOTref, surfDOTref, rTtDOTrTsTt, traceANGsurf, traceANGref, surfANGref;
...
{
    pm->ps->viewheight = ( pm->ps->pm_flags & PMF_DUCKED ) ? ClassConfig( ( class_t )pm->ps->stats[STAT_CLASS] )->crouchViewheight : ClassConfig( ( class_t )pm->ps->stats[STAT_CLASS] )->viewheight;
}
...
void idBothGamesLocal::CheckDuck( void )
{
    ...
    ClassBoundingBox( ( class_t )pm->ps->stats[STAT_CLASS], PCmins, PCmaxs, PCcmaxs, nullptr, nullptr );
    ...
    if( ( pm->cmd.upmove < 0 ) && !VectorCompare( PCmaxs, PCcmaxs ) && pm->ps->pm_type != PM_JETPACK )
    {
        // duck
...
void idBothGamesLocal::Footsteps( void )
{
    ...
    if( ClassHasAbility( ( class_t )pm->ps->stats[STAT_CLASS], SCA_WALLCLIMBER ) && ( pml.groundPlane ) )
    {
...
void idBothGamesLocal::Weapon( void )
{
    ...
    if( pm->ps->persistant[PERS_SPECSTATE] != SPECTATOR_NOT || pm->ps->stats[STAT_STATE] & SS_HOVELING )
    {
        return;
    }
    ...
    if( pm->ps->weapon == WP_ALEVEL4 )
    {
        ...
        {
            // Charge button held
            if( pm->ps->stats[STAT_MISC] < LEVEL4_TRAMPLE_CHARGE_TRIGGER && ( pm->cmd.buttons & BUTTON_ATTACK2 ) )
            {
            ...
            else if( !( pm->ps->stats[STAT_STATE] & SS_CHARGING ) )
            {
                if( pm->ps->stats[STAT_MISC] > LEVEL4_TRAMPLE_CHARGE_MIN )
                {
                    ...
                    pm->ps->stats[STAT_MISC] = pm->ps->stats[STAT_MISC] * LEVEL4_TRAMPLE_DURATION / LEVEL4_TRAMPLE_CHARGE_MAX;
                    pm->ps->stats[STAT_STATE] |= SS_CHARGING;
    ...
    if( pm->ps->weapon == WP_LUCIFER_CANNON )
    {
        ...
        if( !pm->ps->weaponTime && pm->ps->weaponstate != WEAPON_NEEDS_RESET && ( pm->cmd.buttons & BUTTON_ATTACK ) )
        {
            ...
            if( pm->ps->stats[STAT_MISC] > pm->ps->ammo * ( LCANNON_CHARGE_TIME_MAX / LCANNON_CHARGE_AMMO ) )
            {
                pm->ps->stats[STAT_MISC] = pm->ps->ammo * ( LCANNON_CHARGE_TIME_MAX / LCANNON_CHARGE_AMMO );
            }
        }
        ...
        if( pm->cmd.buttons & BUTTON_USE_HOLDABLE )
        {
            if( !( pm->ps->pm_flags & PMF_USE_ITEM_HELD ) )
            {
                ...
                else
                {
                    ...
                    if( InventoryContainsUpgrade( pm->cmd.weapon - 32, pm->ps->stats ) )  //sanity check
                    {
    ...
    if( !pm->ps->ammo && !pm->ps->clips && !Weapon( ( weapon_t )pm->ps->weapon )->infiniteAmmo )
    {
        if( attack1 || ( Weapon( ( weapon_t )pm->ps->weapon )->hasAltMode && attack2 ) || ( Weapon( ( weapon_t )pm->ps->weapon )->hasThirdMode && attack3 ) )
        {
            AddEvent( EV_NOAMMO );
    ...
    if( pm->ps->weaponstate == WEAPON_RELOADING )
    {
        ...
        if( Weapon( ( weapon_t )pm->ps->weapon )->usesEnergy && InventoryContainsUpgrade( UP_BATTPACK, pm->ps->stats ) )
        {
    ...
    if( !Weapon( ( weapon_t )pm->ps->weapon )->infiniteAmmo && ( pm->ps->ammo <= 0 || ( pm->ps->pm_flags & PMF_WEAPON_RELOAD ) ) && pm->ps->clips > 0 )
    {
        pm->ps->pm_flags &= ~PMF_WEAPON_RELOAD;
    ...
    if( !Weapon( ( weapon_t )pm->ps->weapon )->infiniteAmmo || ( pm->ps->weapon == WP_ALEVEL3_UPG && attack3 ) )
    {
        // Special case for lcannon
        if( pm->ps->weapon == WP_LUCIFER_CANNON && attack1 && !attack2 )
        {
            pm->ps->ammo -= ( pm->ps->stats[STAT_MISC] * LCANNON_CHARGE_AMMO + LCANNON_CHARGE_TIME_MAX - 1 ) / LCANNON_CHARGE_TIME_MAX;
        }
        else
        {
            pm->ps->ammo--;
    ...
    {
        if( pm->ps->pm_flags & PMF_DUCKED || InventoryContainsUpgrade( UP_BATTLESUIT, pm->ps->stats ) )
        {
            pm->ps->delta_angles[PITCH] -= ANGLE2SHORT( ( ( random() * 0.5 ) - 0.125 ) * ( 30 / ( float32 )addTime ) );
            pm->ps->delta_angles[YAW] -= ANGLE2SHORT( ( ( random() * 0.5 ) - 0.25 ) * ( 30.0 / ( float32 )addTime ) );
        }
        else
        {
            pm->ps->delta_angles[PITCH] -= ANGLE2SHORT( ( ( random() * 8 ) - 2 ) * ( 30.0 / ( float32 )addTime ) );
            pm->ps->delta_angles[YAW] -= ANGLE2SHORT( ( ( random() * 8 ) - 4 ) * ( 30.0 / ( float32 )addTime ) );
        }
    }
...
================
*/
void idBothGamesLocal::UpdateViewAngles( playerState_t* ps, const usercmd_t* cmd )
{
    schar16 temp[3];
    sint i;
    ...
    if( !( ps->stats[STAT_STATE] & SS_WALLCLIMBING ) || !RotateAxis( ps->grapplePoint, axis, rotaxis, false, ps->eFlags & EF_WALLCLIMBCEILING ) )
    {
    ...
    if( ps->pm_type == PM_GRABBED && !InventoryContainsUpgrade( UP_BATTLESUIT, ps->stats ) )
    {
        vec3_t  dir, angles;
...
void idBothGamesLocal::PmoveSingle( pmove_t* pmove )
{
    ...
    if( !( pm->ps->pm_flags & PMF_RESPAWNED ) && pm->ps->pm_type != PM_INTERMISSION && ( pm->cmd.buttons & BUTTON_ATTACK ) && ( ( pm->ps->ammo > 0 || pm->ps->clips > 0 ) || Weapon( ( weapon_t )pm->ps->weapon )->infiniteAmmo ) )
    {
    ...
    // set the firing flag for continuous beam weapons
    if( !( pm->ps->pm_flags & PMF_RESPAWNED ) && pm->ps->pm_type != PM_INTERMISSION && ( pm->cmd.buttons & BUTTON_ATTACK2 ) && ( ( pm->ps->ammo > 0 || pm->ps->clips > 0 ) || Weapon( ( weapon_t )pm->ps->weapon )->infiniteAmmo ) )
    {
    ...
    // set the firing flag for continuous beam weapons
    if( !( pm->ps->pm_flags & PMF_RESPAWNED ) && pm->ps->pm_type != PM_INTERMISSION && ( pm->cmd.buttons & BUTTON_USE_HOLDABLE ) && ( ( pm->ps->ammo > 0 || pm->ps->clips > 0 ) || Weapon( ( weapon_t )pm->ps->weapon )->infiniteAmmo ) )
    {
    ...
    {
        if( ClassHasAbility( ( class_t )pm->ps->stats[STAT_CLASS], SCA_WALLCLIMBER ) && ( pm->ps->stats[STAT_STATE] & SS_WALLCLIMBING ) )
        {
