    ...
    for(i = 0; i < numBindings; i++) {
        if(!Q_stricmp(command, bindings[ i ].command)) {
            if(bindings[ i ].keys[ 0 ] != K_NONE) {
                trap_Key_KeynumToStringBuf(bindings[ i ].keys[ 0 ], buffer,
                                           MAX_STRING_CHARS);
                ...
                Q_vsprintf_s(buffer, MAX_STRING_CHARS, MAX_STRING_CHARS,
                             "\"%s\" (unbound)", bindings[ i ].humanName);
            }
...
===============
*/
entityState_t *idCGameTutorial::BuildableInRange(playerState_t *ps,
        float32 *healthFraction) {
    vec3_t view, point;
    trace_t trace;
    entityState_t *es;
    sint health;
    ...
    if(es->eType == ET_BUILDABLE &&
            ps->stats[STAT_TEAM] == bggame->Buildable((buildable_t)
                    es->modelindex)->team) {
        return es;
    } else {
        return nullptr;
...
===============
*/
void idCGameTutorial::AlienBuilderText(valueType *text,
                                       playerState_t *ps) {
    buildable_t buildable = (buildable_t)(ps->stats[ STAT_BUILDABLE ] &
                                          ~SB_VALID_TOGGLEBIT);
    entityState_t *es;
...
void idCGameTutorial::HumanText(valueType *text, playerState_t *ps) {
    ...
    if(!ps->ammo && !ps->clips &&
            !bggame->Weapon((weapon_t)ps->weapon)->infiniteAmmo) {
        //no ammo
        switch(ps->weapon) {
            case WP_MACHINEGUN:
            case WP_CHAINGUN:
