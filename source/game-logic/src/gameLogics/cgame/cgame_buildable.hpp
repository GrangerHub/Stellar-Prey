...
public:
    ...
    static bool ParseBuildableAnimationFile(pointer filename,
                                            buildable_t buildable);
    static bool ParseBuildableSoundFile(pointer filename,
                                        buildable_t buildable);
    static void InitBuildables(void);
    static void SetBuildableLerpFrameAnimation(buildable_t buildable,
            lerpFrame_t *lf, sint newAnimation);
    static void RunBuildableLerpFrame(centity_t *cent);
    static void BuildableAnimation(centity_t *cent, sint *old, sint *now,
                                   float32 *backLerp);
    static void PositionAndOrientateBuildable(const vec3_t angles,
