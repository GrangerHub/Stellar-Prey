...
================
*/
sfxHandle_t idCGamePlayers::CustomSound( sint clientNum, pointer soundName )
{
    clientInfo_t* ci;
    sint i;
...
======================
*/
bool idCGamePlayers::ParseAnimationFile( pointer filename, clientInfo_t* ci )
{
...
==========================
*/
bool idCGamePlayers::RegisterClientSkin( clientInfo_t* ci, pointer modelName, pointer skinName )
{
...
==========================
*/
bool idCGamePlayers::RegisterClientModelname( clientInfo_t* ci, pointer modelName, pointer skinName )
{
...
void idCGamePlayers::LoadClientInfo( clientInfo_t* ci )
{
    ...
    {
        Error( "idCGameLocal::RegisterClientModelname( %s, %s ) failed", ci->modelName, ci->skinName );
    }
    ...
    if( ci->footsteps == FOOTSTEP_CUSTOM )
    {
        for( i = 0; i < 4; i++ )
        {
            ci->customFootsteps[ i ] = trap_S_RegisterSound( va( "sound/player/%s/step%d.ogg", dir, i + 1 ) );
            if( !ci->customFootsteps[i] )
            {
                ci->customFootsteps[i] = trap_S_RegisterSound( va( "sound/player/footsteps/step%d.ogg", i + 1 ) );
            }
            
            ci->customMetalFootsteps[ i ] = trap_S_RegisterSound( va( "sound/player/%s/clank%d.ogg", dir, i + 1 ) );
            if( !ci->customMetalFootsteps[i] )
            {
                ci->customMetalFootsteps[i] = trap_S_RegisterSound( va( "sound/player/footsteps/clank%d.ogg", i + 1 ) );
            }
        }
    }
...
======================
*/
void idCGamePlayers::CopyClientInfoModel( clientInfo_t* from, clientInfo_t* to )
{
...
======================
*/
void idCGamePlayers::PrecacheClientInfo( class_t _class, valueType* model, valueType* skin )
{
...
=============
*/
void idCGamePlayers::TeamJoinMessage( clientInfo_t* newInfo, clientInfo_t* ci )
{
    sint team, oldteam;
    ...
    if( team == TEAM_NONE )
    {
        Printf( "%s" S_COLOR_WHITE " left the %ss\n", playerName, bggame->TeamName( ( team_t )oldteam ) );
    }
    else if( oldteam == TEAM_NONE )
    {
        Printf( "%s" S_COLOR_WHITE " joined the %ss\n", playerName, bggame->TeamName( ( team_t )team ) );
    }
    else
    {
        Printf( "%s" S_COLOR_WHITE " left the %ss and joined the %ss\n", playerName, bggame->TeamName( ( team_t )oldteam ), bggame->TeamName( ( team_t )team ) );
    }
}
...
void idCGamePlayers::NewClientInfo( sint clientNum )
{
    ...
    if( clientNum == cg.clientNum && ( !ci->infoValid || ci->team != newInfo.team ) )
    {
        ...
        trap_Cvar_VariableStringBuffer( va( "cg_%sConfig", bggame->TeamName( newInfo.team ) ), config, sizeof( config ) );
        if( config[0] )
        {
...
===============
*/
void idCGamePlayers::SetLerpFrameAnimation( clientInfo_t* ci, lerpFrame_t* lf, sint newAnimation )
{
    animation_t* anim;
...
===============
*/
void idCGamePlayers::RunPlayerLerpFrame( clientInfo_t* ci, lerpFrame_t* lf, sint newAnimation, float32 speedScale )
{
    sint f, numFrames;
    animation_t* anim;
...
===============
*/
void idCGamePlayers::ClearLerpFrame( clientInfo_t* ci, lerpFrame_t* lf, sint animationNumber )
{
...
===============
*/
void idCGamePlayers::PlayerAnimation( centity_t* cent, sint* legsOld, sint* legs, float32* legsBackLerp, sint* torsoOld, sint* torso, float32* torsoBackLerp )
{
    clientInfo_t* ci;
    sint clientNum;
    ...
    {
        RunPlayerLerpFrame( ci, &cent->pe.legs, cent->currentState.legsAnim, speedScale );
    }
    ...
    *legsBackLerp = cent->pe.legs.backlerp;
    ...
    RunPlayerLerpFrame( ci, &cent->pe.torso, cent->currentState.torsoAnim, speedScale );
...
===============
*/
void idCGamePlayers::PlayerNonSegAnimation( centity_t* cent, sint* nonSegOld, sint* nonSeg, float32* nonSegBackLerp )
{
    clientInfo_t* ci;
    sint clientNum;
    float32 speedScale = 1.0f;
    ...
    {
        RunPlayerLerpFrame( ci, &cent->pe.nonseg, cent->currentState.legsAnim, speedScale );
    }
...
==================
*/
void idCGamePlayers::SwingAngles( float32 destination, float32 swingTolerance, float32 clampTolerance, float32 speed, float32* angle, bool* swinging )
{
...
===============
*/
void idCGamePlayers::PlayerAngles( centity_t* cent, vec3_t srcAngles, vec3_t legs[ 3 ], vec3_t torso[ 3 ], vec3_t head[ 3 ] )
{
    ...
    if( cent->currentState.eFlags & EF_DEAD )
    {
        SwingAngles( torsoAngles[ YAW ], 0, 0, cg_swingSpeed.value, &cent->pe.torso.yawAngle, &cent->pe.torso.yawing );
        SwingAngles( legsAngles[ YAW ], 0, 0, cg_swingSpeed.value, &cent->pe.legs.yawAngle, &cent->pe.legs.yawing );
    }
    else
    {
        SwingAngles( torsoAngles[ YAW ], 25, 90, cg_swingSpeed.value, &cent->pe.torso.yawAngle, &cent->pe.torso.yawing );
        SwingAngles( legsAngles[ YAW ], 40, 90, cg_swingSpeed.value, &cent->pe.legs.yawAngle, &cent->pe.legs.yawing );
    }
    ...
    SwingAngles( dest, 15, 30, 0.1f, &cent->pe.torso.pitchAngle, &cent->pe.torso.pitching );
...
===============
*/
void idCGamePlayers::PlayerWWSmoothing( centity_t* cent, vec3_t in[3], vec3_t out[3] )
{
...
===============
*/
void idCGamePlayers::PlayerNonSegAngles( centity_t* cent, vec3_t srcAngles, vec3_t nonSegAxis[ 3 ] )
{
    ...
    if( cent->currentState.eFlags & EF_DEAD )
    {
        SwingAngles( localAngles[ YAW ], 0, 0, cg_swingSpeed.value, &cent->pe.nonseg.yawAngle, &cent->pe.nonseg.yawing );
    }
    else
    {
        SwingAngles( localAngles[ YAW ], 40, 90, cg_swingSpeed.value, &cent->pe.nonseg.yawAngle, &cent->pe.nonseg.yawing );
    }
...
void idCGamePlayers::PlayerUpgrades( centity_t* cent, refEntity_t* torso )
{
    ...
    if( held & ( 1 << UP_JETPACK ) )
    {
        ...
        idCGameEnts::PositionRotatedEntityOnTag( &jetpack, torso, torso->hModel, "tag_head" );
        ...
        if( active & ( 1 << UP_JETPACK ) )
        {
            if( es->pos.trDelta[ 2 ] > 10.0f )
            {
                if( cent->jetPackState != JPS_ASCENDING )
                {
                    ...
                    cent->jetPackPS = idCGameParticles::SpawnNewParticleSystem( cgs.media.jetPackAscendPS );
            ...
            else if( es->pos.trDelta[ 2 ] < -10.0f )
            {
                if( cent->jetPackState != JPS_DESCENDING )
                {
                    ...
                    cent->jetPackPS = idCGameParticles::SpawnNewParticleSystem( cgs.media.jetPackDescendPS );
            ...
            else
            {
                if( cent->jetPackState != JPS_HOVERING )
                {
                    ...
                    cent->jetPackPS = idCGameParticles::SpawnNewParticleSystem( cgs.media.jetPackHoverPS );
            ...
            idCGameEnts::PositionRotatedEntityOnTag( &flash, &jetpack, jetpack.hModel, "tag_flash" );
            trap_R_AddRefEntityToScene( &flash );
            
            if( idCGameParticles::IsParticleSystemValid( &cent->jetPackPS ) )
            {
                idCGameAttachment::SetAttachmentTag( &cent->jetPackPS->attachment, jetpack, jetpack.hModel, "tag_flash" );
    ...
    if( held & ( 1 << UP_BATTPACK ) )
    {
        ...
        //FIXME: change to tag_back when it exists
        idCGameEnts::PositionRotatedEntityOnTag( &battpack, torso, torso->hModel, "tag_head" );
    ...
    if( es->eFlags & EF_BLOBLOCKED )
    {
        ...
        idCGamePredict::Trace( &tr, es->pos.trBase, nullptr, nullptr, temp, es->number, MASK_SOLID );
        ...
        if( size > 0.0f )
        {
            idCGameMarks::ImpactMark( cgs.media.creepShader, origin, up, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, false, size, true );
        }
    }
}
...
bool idCGamePlayers::PlayerShadow( centity_t* cent, float32* shadowPlane, class_t _class )
{
    ...
    *shadowPlane = 0;
    ...
    idCGameMarks::ImpactMark( cgs.media.shadowMarkShader, trace.endpos, trace.plane.normal, cent->pe.legs.yawAngle, 0.0f, 0.0f, 0.0f, alpha,
...
void idCGamePlayers::PlayerSplash( centity_t* cent, class_t _class )
{
    ...
    idCGameMarks::ImpactMark( cgs.media.wakeMarkShader, trace.endpos, trace.plane.normal, cent->pe.legs.yawAngle, 1.0f, 1.0f, 1.0f, 1.0f,
...
=================
*/
sint idCGamePlayers::LightVerts( vec3_t normal, sint numVerts, polyVert_t* verts )
{
    sint i, j;
    float32 incoming;
...
void idCGamePlayers::Player( centity_t* cent )
{
    ...
    if( es->eFlags & EF_WALLCLIMB && bggame->RotateAxis( es->angles2, tempAxis, tempAxis2, true, es->eFlags & EF_WALLCLIMBCEILING ) )
    {
    ...
    if( es->eFlags & EF_WALLCLIMB && bggame->RotateAxis( es->angles2, legs.axis, tempAxis, false, es->eFlags & EF_WALLCLIMBCEILING ) )
    {
    ...
    if( !ci->nonsegmented )
    {
        PlayerAnimation( cent, &legs.oldframe, &legs.frame, &legs.backlerp, &torso.oldframe, &torso.frame, &torso.backlerp );
    }
    else
    {
    ...
    // add the shadow
    if( ( es->number == cg.snap->ps.clientNum && cg.renderingThirdPerson ) || es->number != cg.snap->ps.clientNum )
    {
    ...
    if( es->eFlags & EF_WALLCLIMB && !( es->eFlags & EF_DEAD ) && !( cg.intermissionStarted ) )
    {
        ...
        idCGamePredict::CapTrace( &tr, start, mins, maxs, end, es->number, MASK_PLAYERSOLID );
    ...
    VectorMA( legs.origin, bggame->ClassConfig( _class )->zOffset, surfNormal, legs.origin );
    VectorCopy( legs.origin, legs.lightingOrigin );
    VectorCopy( legs.origin, legs.oldorigin ); // don't positionally lerp at all
    ...
    if( !ci->nonsegmented )
    {
        ...
        idCGameEnts::PositionRotatedEntityOnTag( &torso, &legs, ci->legsModel, "tag_torso" );
        ...
        idCGameEnts::PositionRotatedEntityOnTag( &head, &torso, ci->torsoModel, "tag_head" );
        ...
        if( ( es->eFlags & EF_POISONCLOUDED ) && ( es->number != cg.snap->ps.clientNum || cg.renderingThirdPerson ) )
        {
            if( !idCGameParticles::IsParticleSystemValid( &cent->poisonCloudedPS ) )
            {
                cent->poisonCloudedPS = idCGameParticles::SpawnNewParticleSystem( cgs.media.poisonCloudedPS );
            }
            idCGameAttachment::SetAttachmentTag( &cent->poisonCloudedPS->attachment, head, head.hModel, "tag_head" );
            idCGameAttachment::SetAttachmentCent( &cent->poisonCloudedPS->attachment, cent );
            idCGameAttachment::AttachToTag( &cent->poisonCloudedPS->attachment );
        }
        else if( idCGameParticles::IsParticleSystemValid( &cent->poisonCloudedPS ) )
        {
...
void idCGamePlayers::Corpse( centity_t* cent )
{
    ...
    bggame->ClassBoundingBox( ( class_t )es->clientNum, liveZ, nullptr, nullptr, deadZ, nullptr );
...
===============
*/
bool idCGameLocal::GetTag( sint clientNum, valueType* tagname, orientation_t* _or )
{
    clientInfo_t* ci;
    centity_t* cent;
    refEntity_t* refent;
...
void idCGamePlayers::ResetPlayerEntity( centity_t* cent )
{
    ...
    ClearLerpFrame( &cgs.clientinfo[ cent->currentState.clientNum ], &cent->pe.legs, cent->currentState.legsAnim );
    ClearLerpFrame( &cgs.clientinfo[ cent->currentState.clientNum ], &cent->pe.torso, cent->currentState.torsoAnim );
    ClearLerpFrame( &cgs.clientinfo[ cent->currentState.clientNum ], &cent->pe.nonseg, cent->currentState.legsAnim );
    
    bggame->EvaluateTrajectory( &cent->currentState.pos, cg.time, cent->lerpOrigin );
    bggame->EvaluateTrajectory( &cent->currentState.apos, cg.time, cent->lerpAngles );
...
void idCGamePlayers::Bleed( vec3_t origin, vec3_t normal, sint entityNum )
{
    ...
    if( idCGameParticles::IsParticleSystemValid( &ps ) )
    {
        ...
        idCGameAttachment::SetAttachmentCent( &ps->attachment, &cg_entities[ entityNum ] );
        idCGameAttachment::AttachToPoint( &ps->attachment );
...
void idCGamePlayers::TeamStatusDisplay( centity_t* cent )
{
    ...
    // trace for top
    if( ( es->legsAnim & ~ANIM_TOGGLEBIT ) == LEGS_WALKCR || ( es->legsAnim & ~ANIM_TOGGLEBIT ) == LEGS_IDLECR )
    {
...
void idCGamePlayers::DrawTeamStatus( void )
{
    ...
    if( cg_drawTeamStatus.integer )
    {
        for( i = 0; i < cg.snap->numEntities; i++ )
        {
            ...
            if( es->eType == ET_PLAYER && ( cent->currentState.misc & 0x00FF ) == cg.predictedPlayerState.stats[STAT_TEAM] )
            {
