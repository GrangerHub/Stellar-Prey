...
===============
*/
void idCGameParticles::DestroyParticle( particle_t* p, vec3_t impactNormal )
{
...
===============
*/
particle_t* idCGameParticles::SpawnNewParticle( baseParticle_t* bp, particleEjector_t* parent )
{
    sint i, j;
    ...
    for( i = 0; i < MAX_PARTICLES; i++ )
    {
        ...
        if( !p->valid && cg.clientFrame > p->frameWhenInvalidated + 1 )
        {
            ...
            p->birthTime = cg.time;
            p->lifeTime = ( sint )RandomiseValue( ( float32 )bp->lifeTime, bp->lifeTimeRandFrac );
            
            p->radius.delay = ( sint )RandomiseValue( ( float32 )bp->radius.delay, bp->radius.delayRandFrac );
            ...
            p->alpha.delay = ( sint )RandomiseValue( ( float32 )bp->alpha.delay, bp->alpha.delayRandFrac );
            ...
            p->rotation.delay = ( sint )RandomiseValue( ( float32 )bp->rotation.delay, bp->rotation.delayRandFrac );
            ...
            p->dLightRadius.delay = ( sint )RandomiseValue( ( float32 )bp->dLightRadius.delay, bp->dLightRadius.delayRandFrac );
            ...
            p->bounceMarkCount = rint( RandomiseValue( ( float32 )bp->bounceMarkCount, bp->bounceMarkCountRandFrac ) );
            p->bounceSoundCount = rint( RandomiseValue( ( float32 )bp->bounceSoundCount, bp->bounceSoundCountRandFrac ) );
            ...
            if( idCGameAttachment::AttachmentVelocity( &ps->attachment, attachmentVelocity ) )
            {
            ...
            {
                trailSystem_t* ts = idCGameTrails::SpawnNewTrailSystem( bp->childTrailSystemHandle );
        ...
        if( pe->valid )
        {
            ...
            if( !ps->lazyRemove )
            {
                while( pe->nextEjectionTime <= cg.time && ( pe->count > 0 || pe->totalParticles == PARTICLES_INFINITE ) )
                {
                    ...
                    pe->nextEjectionTime = cg.time + ( sint )RandomiseValue( LerpValues( pe->ejectPeriod.initial, pe->ejectPeriod.final, lerpFrac ), pe->ejectPeriod.randFrac );
                }
            }
...
===============
*/
particleEjector_t* idCGameParticles::SpawnNewParticleEjector( baseParticleEjector_t* bpe, particleSystem_t* parent )
{
    sint i;
    ...
    for( i = 0; i < MAX_PARTICLE_EJECTORS; i++ )
    {
        ...
        if( !pe->valid )
        {
            ...
            pe->nextEjectionTime = cg.time + ( sint )RandomiseValue( ( float32 )bpe->eject.delay, bpe->eject.delayRandFrac );
            pe->count = pe->totalParticles = ( sint )rint( RandomiseValue( ( float32 )bpe->totalParticles, bpe->totalParticlesRandFrac ) );
...
===============
*/
particleSystem_t* idCGameParticles::SpawnNewParticleSystem( qhandle_t psHandle )
{
    sint i, j;
        ...
        if( !Q_stricmpn( bps->name, name, MAX_QPATH ) )
        {
            ...
            for( j = 0; j < bps->numEjectors; j++ )
            {
                ...
                for( l = 0; l < bpe->numParticles; l++ )
                {
                    ...
                    if( bp->childTrailSystemName[0] != '\0' )
                    {
                        bp->childTrailSystemHandle = idCGameTrails::RegisterTrailSystem( bp->childTrailSystemName );
                    }
                }
            }
...
===============
*/
void idCGameParticles::ParseValueAndVariance( valueType* token, float32* value, float32* variance, bool allowNegative )
{
...
===============
*/
bool idCGameParticles::ParseParticle( baseParticle_t* bp, valueType** text_p )
{
    valueType* token;
...
===============
*/
bool idCGameParticles::ParseParticleEjector( baseParticleEjector_t* bpe, valueType** text_p )
{
    valueType* token;
    float32 number, randFrac;
...
===============
*/
bool idCGameParticles::ParseParticleSystem( baseParticleSystem_t* bps, valueType** text_p, pointer name )
{
    valueType* token;
    baseParticleEjector_t* bpe;
...
===============
*/
void idCGameParticles::SetParticleSystemNormal( particleSystem_t* ps, vec3_t normal )
{
    if( ps == nullptr || !ps->valid )
    {
        Printf( S_COLOR_YELLOW "WARNING: tried to modify a nullptr particle system\n" );
        return;
    }
...
void idCGameParticles::DestroyParticleSystem( particleSystem_t** ps )
{
    ...
    if( *ps == nullptr || !( *ps )->valid )
    {
        Printf( S_COLOR_YELLOW "WARNING: tried to destroy a nullptr particle system\n" );
        return;
    }
...
bool idCGameParticles::IsParticleSystemInfinite( particleSystem_t* ps )
{
    ...
    if( ps == nullptr )
    {
        Printf( S_COLOR_YELLOW "WARNING: tried to test a nullptr particle system\n" );
        return false;
    }
...
void idCGameParticles::GarbageCollectParticleSystems( void )
{
    ...
    for( i = 0; i < MAX_PARTICLE_SYSTEMS; i++ )
    {
        ...
        if( ( centNum = idCGameAttachment::AttachmentCentNum( &ps->attachment ) ) >= 0 && centNum != cg.snap->ps.clientNum )
        {
...
===============
*/
float32 idCGameParticles::CalculateTimeFrac( sint birth, sint life, sint delay )
{
    float32 frac;
    
    frac = ( ( float32 )cg.time - ( float32 )( birth + delay ) ) / ( float32 )( life - delay );
...
#define MAX_ACC_RADIUS 1000.0f
    ...
    idCGamePredict::Trace( &trace, p->origin, mins, maxs, newOrigin, idCGameAttachment::AttachmentCentNum( &ps->attachment ), CONTENTS_SOLID );
    ...
    if( trace.plane.normal[2] > 0.5f && ( p->velocity[2] < 40.0f || p->velocity[2] < -cg.frametime * p->velocity[2] ) )
    {
        p->atRest = true;
    }
    ...
    {
        idCGameMarks::ImpactMark( bp->bounceMark, trace.endpos, trace.plane.normal, random( ) * 360, 1, 1, 1, 1, true, bp->bounceMarkRadius, false );
        p->bounceMarkCount--;
    }
...
===============
*/
void idCGameParticles::Radix( sint bits, sint size, particle_t** source, particle_t** dest )
{
...
===============
*/
void idCGameParticles::RadixSort( particle_t** source, particle_t** temp, sint size )
{
...
void idCGameParticles::RenderParticle( particle_t* p )
{
    ...
    if( bp->numFrames )       //shader based
    {
        ...
        re.shaderRGBA[ 3 ] = ( uchar8 )( ( float32 )0xFF * LerpValues( p->alpha.initial, p->alpha.final, CalculateTimeFrac( p->birthTime, p->lifeTime, p->alpha.delay ) ) );
        ...
        {
            //looping animation
            index = ( sint )( bp->framerate * timeFrac * p->lifeTime * 0.001 ) % bp->numFrames;
            re.customShader = bp->shaders[ index ];
        }
    ...
    if( bps->thirdPersonOnly && idCGameAttachment::AttachmentCentNum( &ps->attachment ) == cg.snap->ps.clientNum && !cg.renderingThirdPerson )
    {
    ...
    if( bp->dynamicLight && !( re.renderfx & RF_THIRD_PERSON ) )
    {
        trap_R_AddLightToScene( p->origin, LerpValues( p->dLightRadius.initial, p->dLightRadius.final, CalculateTimeFrac( p->birthTime, p->lifeTime, p->dLightRadius.delay ) ),
                                ( float32 )bp->dLightColor[ 0 ] / ( float32 )0xFF, ( float32 )bp->dLightColor[ 1 ] / ( float32 )0xFF, ( float32 )bp->dLightColor[ 2 ] / ( float32 )0xFF );
    }
...
void idCGameParticles::ParticleSystemEntity( centity_t* cent )
{
    ...
    if( !IsParticleSystemValid( &cent->entityPS ) && !cent->entityPSMissing )
    {
        ...
        {
            idCGameAttachment::SetAttachmentPoint( &cent->entityPS->attachment, cent->lerpOrigin );
