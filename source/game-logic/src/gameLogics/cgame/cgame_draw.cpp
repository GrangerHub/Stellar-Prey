...
===============
*/
void idCGameDraw::AlignText( rectDef_t* rect, pointer text, float32 scale, float32 w, float32 h, sint align, sint valign, float32* x, float32* y )
{
    float32 tx, ty;
...
==============
*/
void idCGameDraw::DrawFieldPadded( sint x, sint y, sint width, sint cw, sint ch, sint value )
{
    ...
    while( *ptr && l )
    {
        if( width > orgL )
        {
            idCGameDrawTools::DrawPic( x, y, charWidth, charHeight, cgs.media.numberShaders[ 0 ] );
            width--;
            x += charWidth;
            continue;
        }
        ...
        idCGameDrawTools::DrawPic( x, y, charWidth, charHeight, cgs.media.numberShaders[ frame ] );
        x += charWidth;
        ptr++;
        l--;
    }
...
==============
*/
void idCGameDraw::DrawField( float32 x, float32 y, sint width, float32 cw, float32 ch, sint value )
{
    valueType num[ 16 ], *ptr;
    sint l, frame;
...
===============
*/
void idCGameDraw::DrawProgressBar( rectDef_t* rect, vec4_t color, float32 scale, sint align, sint textalign, sint textStyle, sint special, float32 progress )
{
    ...
    {
        idCGameDrawTools::DrawPic( rect->x, rect->y, rimWidth, rect->h, cgs.media.whiteShader );
        idCGameDrawTools::DrawPic( rect->x + rimWidth, rect->y, leftWidth, rimWidth, cgs.media.whiteShader );
        idCGameDrawTools::DrawPic( rect->x + rimWidth, rect->y + rect->h - rimWidth, leftWidth, rimWidth, cgs.media.whiteShader );
        idCGameDrawTools::DrawPic( rect->x + rimWidth + leftWidth, rect->y, rimWidth + doneWidth, rect->h, cgs.media.whiteShader );
    }
    else
    {
        idCGameDrawTools::DrawPic( rect->x, rect->y, rimWidth + doneWidth, rect->h, cgs.media.whiteShader );
        idCGameDrawTools::DrawPic( rimWidth + rect->x + doneWidth, rect->y, leftWidth, rimWidth, cgs.media.whiteShader );
        idCGameDrawTools::DrawPic( rimWidth + rect->x + doneWidth, rect->y + rect->h - rimWidth, leftWidth, rimWidth, cgs.media.whiteShader );
        idCGameDrawTools::DrawPic( rect->x + rect->w - rimWidth, rect->y, rimWidth, rect->h, cgs.media.whiteShader );
    }
...
void idCGameDraw::DrawPlayerCreditsValue( rectDef_t* rect, vec4_t color, bool padding )
{
    sint value;
    playerState_t* ps;
    centity_t* cent;
...
==============
*/
void idCGameDraw::DrawPlayerStamina( sint ownerDraw, rectDef_t* rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader )
{
...
==============
*/
void idCGameDraw::DrawPlayerStaminaBolt( rectDef_t* rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader )
{
...
==============
*/
void idCGameDraw::DrawPlayerClipsRing( rectDef_t* rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader )
{
    playerState_t* ps = &cg.snap->ps;
    centity_t* cent;
    ...
    switch( weapon )
    {
        ...
        default:
            ...
            {
                maxDelay = ( float32 )bggame->Weapon( ( weapon_t )cent->currentState.weapon )->reloadTime;
...
==============
*/
void idCGameDraw::DrawPlayerBuildTimerRing( rectDef_t* rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader )
{
    playerState_t* ps = &cg.snap->ps;
    centity_t* cent;
    float32 buildTime = ps->stats[ STAT_MISC ];
...
==============
*/
void idCGameDraw::DrawPlayerBoosted( rectDef_t* rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader )
{
...
==============
*/
void idCGameDraw::DrawPlayerBoosterBolt( rectDef_t* rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader )
{
    vec4_t color;
    
    // Flash bolts when the boost is almost out
    if( ( cg.snap->ps.stats[STAT_STATE] & SS_BOOSTED ) && ( cg.snap->ps.stats[STAT_STATE] & SS_BOOSTEDWARNING ) )
    {
...
==============
*/
void idCGameDraw::DrawPlayerPoisonBarbs( rectDef_t* rect, vec4_t color, qhandle_t shader )
{
    bool vertical;
...
==============
*/
void idCGameDraw::DrawPlayerWallclimbing( rectDef_t* rect, vec4_t backColor, vec4_t foreColor, qhandle_t shader )
{
...
===============
*/
void idCGameDraw::DrawStack( rectDef_t* rect, vec4_t color, float32 fill, sint align, sint valign, float32 val, sint max )
{
    sint i;
    float32 each;
    sint ival;
    ...
    if( rect->h >= rect->w )  // vertical stack
    {
        ...
        if( each * cgs.screenYScale < 4.f )  // FIXME: magic number
        {
            ...
            idCGameDrawTools::DrawPic( rect->x, rect->y + offy, rect->w, h, cgs.media.whiteShader );
            trap_R_SetColor( nullptr );
        ...
        for( i = 0; i < ival; i++ )
        {
            ...
            idCGameDrawTools::DrawPic( rect->x, rect->y + rect->h * start, rect->w, each, cgs.media.whiteShader );
        }
        ...
        {
            case VALIGN_TOP:
                idCGameDrawTools::DrawPic( rect->x, rect->y - rect->h * ( 1 - frac ) / fmax, rect->w, each, cgs.media.whiteShader );
                break;
            ...
            default:
            case VALIGN_BOTTOM:
                idCGameDrawTools::DrawPic( rect->x, rect->y + rect->h * ( 1 + ( ( 1 - fill ) / fmax ) - frac / fmax ), rect->w, each, cgs.media.whiteShader );
        }
    }
    else // horizontal stack
    {
        ...
        if( each < 4.f )
        {
            ...
            idCGameDrawTools::DrawPic( rect->x + offx, rect->y, w, rect->h, cgs.media.whiteShader );
        ...
        for( i = 0; i < ival; i++ )
        {
            ...
            idCGameDrawTools::DrawPic( rect->x + rect->w * start, rect->y, each, rect->h, cgs.media.whiteShader );
        }
        ...
        {
            case ALIGN_LEFT:
                idCGameDrawTools::DrawPic( rect->x - ( 1 - frac ) * rect->w / fmax, rect->y, each, rect->h, cgs.media.whiteShader );
                break;
            ...
            default:
            case ALIGN_RIGHT:
                idCGameDrawTools::DrawPic( rect->x + rect->w * ( 1 + ( ( 1 - fill ) / fmax ) - frac / fmax ), rect->y, each, rect->h, cgs.media.whiteShader );
        }
    }
...
===============
*/
void idCGameDraw::DrawPlayerAmmoStack( rectDef_t* rect, vec4_t backColor, vec4_t foreColor, sint textalign, sint textvalign )
{
    float32 val;
    sint maxVal;
    ...
    switch( primary )
    {
        ...
        default:
            ...
            if( bggame->Weapon( primary )->usesEnergy && bggame->InventoryContainsUpgrade( UP_BATTPACK, ps->stats ) )
            {
    ...
    trap_R_SetColor( backColor );
    idCGameDrawTools::DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.whiteShader );
    trap_R_SetColor( nullptr );
...
===============
*/
void idCGameDraw::DrawPlayerClipsStack( rectDef_t* rect, vec4_t backColor, vec4_t foreColor, sint textalign, sint textvalign )
{
    float32 val;
    sint maxVal;
...
==============
*/
void idCGameDraw::DrawHumanScanner( rectDef_t* rect, qhandle_t shader, vec4_t color )
{
...
==============
*/
void idCGameDraw::DrawUsableBuildable( rectDef_t* rect, qhandle_t shader, vec4_t color )
{
    vec3_t view, point;
    trace_t trace;
    entityState_t* es;
    ...
    idCGamePredict::Trace( &trace, cg.refdef.vieworg, nullptr, nullptr, point, cg.predictedPlayerState.clientNum, MASK_SHOT );
            ...
            cg.predictedPlayerState.stats[ STAT_TEAM ] == bggame->Buildable( ( buildable_t )es->modelindex )->team )
    {
        ...
        if( ( es->modelindex == BA_H_REACTOR || es->modelindex == BA_H_REPEATER ) && ( !bggame->Weapon( ( weapon_t )cg.snap->ps.weapon )->usesEnergy ||
...
void idCGameDraw::DrawPlayerBuildTimer( rectDef_t* rect, vec4_t color )
{
    ...
    if( cg.time - cg.lastBuildAttempt <= BUILD_DELAY_TIME && ( ( cg.time - cg.lastBuildAttempt ) / 300 ) % 2 )
    {
        color[ 0 ] = 1.0f;
    ...
    trap_R_SetColor( color );
    idCGameDrawTools::DrawPic( rect->x, rect->y, rect->w, rect->h, cgs.media.buildWeaponTimerPie[ index ] );
    trap_R_SetColor( nullptr );
}
...
==============
*/
void idCGameDraw::DrawPlayerHealthCross( rectDef_t* rect, vec4_t ref_color )
{
    qhandle_t shader;
    vec4_t color;
    float32 ref_alpha;
    ...
    if( cg.snap->ps.stats[ STAT_TEAM ] == TEAM_HUMANS && cg.snap->ps.stats[ STAT_HEALTH ] < 10 )
    {
        color[ 0 ] = 1.0f;
        color[ 1 ] = color[ 2 ] = 0.0f;
    }
    ...
    if( cg.lastHealthCross != shader )
    {
        ...
        else
        {
            ...
            trap_R_SetColor( color );
            idCGameDrawTools::DrawPic( rect->x, rect->y, rect->w, rect->h, cg.lastHealthCross );
            trap_R_SetColor( nullptr );
            return;
        }
    }
...
float32 idCGameDraw::ChargeProgress( void )
{
    ...
    progress = ( ( float32 )cg.predictedPlayerState.stats[ STAT_MISC ] - min ) / ( max - min );
...
void idCGameDraw::DrawPlayerChargeBarBG( rectDef_t* rect, vec4_t ref_color, qhandle_t shader )
{
    vec4_t color;
...
void idCGameDraw::DrawPlayerChargeBar( rectDef_t* rect, vec4_t ref_color, qhandle_t shader )
{
    vec4_t color;
    ...
    if( cg.snap->ps.weapon == WP_LUCIFER_CANNON && cg.snap->ps.stats[ STAT_MISC ] >= LCANNON_CHARGE_TIME_WARN && ( cg.time & 128 ) )
    {
        color[ 0 ] = 1.f;
        color[ 1 ] = 0.f;
        color[ 2 ] = 0.f;
    ...
    if( rect->w >= rect->h )
    {
        ...
        trap_R_DrawStretchPic( x + width + cap_size, y, cap_size, height, 1, 0, 0, 1, shader );
    ...
    else
    {
        ...
        // Draw the meter
        trap_R_SetColor( color );
        trap_R_DrawStretchPic( x, y - cap_size, width, cap_size, 0, 1, 1, 0, shader );
        trap_R_DrawStretchPic( x, y - height - cap_size * 2, width, cap_size, 0, 0, 1, 1, shader );
        trap_R_DrawStretchPic( x, y - height - cap_size, width, height, 0, 1, 1, 1, shader );
        trap_R_SetColor( nullptr );
    }
}
...
===============
*/
void idCGameDraw::DrawProgressLabel( rectDef_t* rect, float32 text_x, float32 text_y, vec4_t color, float32 scale, sint textalign, sint textvalign, pointer s, float32 fraction )
{
    ...
    if( fraction < 1.0f )
    {
        UI_Text_Paint( text_x + tx, text_y + ty, scale, white, s, 0, 0, ITEM_TEXTSTYLE_NORMAL );
    }
    else
    {
        UI_Text_Paint( text_x + tx, text_y + ty, scale, color, s, 0, 0, ITEM_TEXTSTYLE_NEON );
    }
}
...
===============
*/
void idCGameDraw::DrawMediaProgress( rectDef_t* rect, vec4_t color, float32 scale, sint align, sint textalign, sint textStyle, sint special )
{
...
===============
*/
void idCGameDraw::DrawMediaProgressLabel( rectDef_t* rect, float32 text_x, float32 text_y, vec4_t color, float32 scale, sint textalign, sint textvalign )
{
...
===============
*/
void idCGameDraw::DrawBuildablesProgress( rectDef_t* rect, vec4_t color, float32 scale, sint align, sint textalign, sint textStyle, sint special )
{
...
===============
*/
void idCGameDraw::DrawBuildablesProgressLabel( rectDef_t* rect, float32 text_x, float32 text_y, vec4_t color, float32 scale, sint textalign, sint textvalign )
{
...
===============
*/
void idCGameDraw::DrawCharModelProgress( rectDef_t* rect, vec4_t color, float32 scale, sint align, sint textalign, sint textStyle, sint special )
{
...
===============
*/
void idCGameDraw::DrawCharModelProgressLabel( rectDef_t* rect, float32 text_x, float32 text_y, vec4_t color, float32 scale, sint textalign, sint textvalign )
{
...
===============
*/
void idCGameDraw::DrawOverallProgress( rectDef_t* rect, vec4_t color, float32 scale, sint align, sint textalign, sint textStyle, sint special )
{
    float32 total;
...
===============
*/
void idCGameDraw::DrawLevelName( rectDef_t* rect, float32 text_x, float32 text_y, vec4_t color, float32 scale, sint textalign, sint textvalign, sint textStyle )
{
    pointer s;
...
===============
*/
void idCGameDraw::DrawMOTD( rectDef_t* rect, float32 text_x, float32 text_y, vec4_t color, float32 scale, sint textalign, sint textvalign, sint textStyle )
{
    pointer s;
...
===============
*/
void idCGameDraw::DrawHostname( rectDef_t* rect, float32 text_x, float32 text_y, vec4_t color, float32 scale, sint textalign, sint textvalign, sint textStyle )
{
...
==============
*/
void idCGameDraw::DrawDemoPlayback( rectDef_t* rect, vec4_t color, qhandle_t shader )
{
...
===============
*/
void idCGameDraw::DrawKiller( rectDef_t* rect, float32 scale, vec4_t color, qhandle_t shader, sint textStyle )
{
    // fragged by ... line
    if( cg.killerName[ 0 ] )
    {
...
===============
*/
void idCGameDraw::DrawTeamSpectators( rectDef_t* rect, float32 scale, sint textvalign, vec4_t color, qhandle_t shader )
{
    float32 y;
    ...
    if( cg.spectatorLen )
    {
        ...
        AlignText( rect, nullptr, 0.0f, 0.0f, UI_Text_EmHeight( scale ), ALIGN_LEFT, textvalign, nullptr, &y );
        
        UI_Text_Paint_Limit( &maxX, cg.spectatorPaintX, y, scale, color, &cg.spectatorList[ cg.spectatorOffset ], 0, 0 );
        
        if( cg.spectatorPaintX2 >= 0 )
        {
            float32 maxX2 = rect->x + rect->w - 2;
            UI_Text_Paint_Limit( &maxX2, cg.spectatorPaintX2, y, scale, color, cg.spectatorList, 0, cg.spectatorOffset );
        }
...
==================
*/
void idCGameDraw::DrawTeamLabel( rectDef_t* rect, team_t team, float32 text_x, float32 text_y, vec4_t color, float32 scale, sint textalign, sint textvalign, sint textStyle )
{
    valueType* t;
...
==================
*/
void idCGameDraw::DrawStageReport( rectDef_t* rect, float32 text_x, float32 text_y, vec4_t color, float32 scale, sint textalign, sint textvalign, sint textStyle )
{
    ...
    if( cg.snap->ps.stats[ STAT_TEAM ] == TEAM_ALIENS )
    {
        sint frags = ceil( ( float32 )( cgs.alienNextStageThreshold - cgs.alienCredits ) / ALIEN_CREDITS_PER_FRAG );
...
void idCGameDraw::DrawFPS( rectDef_t* rect, float32 text_x, float32 text_y, float32 scale, vec4_t color, sint textalign, sint textvalign, sint textStyle, bool scalableText )
{
    valueType* s;
...
==================
*/
void idCGameDraw::DrawSpeed( rectDef_t* rect, float32 text_x, float32 text_y, float32 scale, vec4_t color, sint textalign, sint textvalign, sint textStyle )
{
    valueType* s;
...
=================
*/
void idCGameDraw::DrawTimer( rectDef_t* rect, float32 text_x, float32 text_y, float32 scale, vec4_t color, sint textalign, sint textvalign, sint textStyle )
{
    valueType* s;
    float32 tx, ty, w, h, totalWidth;
...
=================
*/
void idCGameDraw::DrawClock( rectDef_t* rect, float32 text_x, float32 text_y, float32 scale, vec4_t color, sint textalign, sint textvalign, sint textStyle )
{
    valueType* s;
    float32 tx, ty, w, h, totalWidth;
    sint i, t, strLength;
...
==================
*/
void idCGameDraw::DrawSnapshot( rectDef_t* rect, float32 text_x, float32 text_y, float32 scale, vec4_t color, sint textalign, sint textvalign, sint textStyle )
{
    valueType*    s;
    float32   tx, ty;
...
void idCGameDraw::DrawDisconnect( void )
{
    ...
    idCGameDrawTools::DrawPic( x, y, 48, 48, trap_R_RegisterShader( "gfx/2d/net.tga" ) );
}
...
void idCGameDraw::DrawLagometer( rectDef_t* rect, float32 text_x, float32 text_y, float32 scale, vec4_t textColor )
{
    sint a, x, y, i, color;
...
===================
*/
void idCGameDraw::DrawConsole( rectDef_t* rect, float32 text_x, float32 text_y, vec4_t color, float32 scale, sint textalign, sint textvalign, sint textStyle )
{
...
===================
*/
void idCGameDraw::DrawTutorial( rectDef_t* rect, float32 text_x, float32 text_y, vec4_t color, float32 scale, sint textalign, sint textvalign, sint textStyle )
{
    ...
    UI_DrawTextBlock( rect, text_x, text_y, color, scale, textalign, textvalign, textStyle, idCGameTutorial::TutorialText( ) );
}
...
void idCGameDraw::DrawWeaponIcon( rectDef_t* rect, vec4_t color )
{
    ...
    {
        float32 ammoPercent = ( float32 )ps->ammo / ( float32 )bggame->Weapon( weapon )->maxAmmo;
    ...
    if( cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_ALIENS && !bggame->AlienCanEvolve( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ],
    ...
    trap_R_SetColor( color );
    idCGameDrawTools::DrawPic( rect->x, rect->y, rect->w, rect->h, cg_weapons[ weapon ].weaponIcon );
    trap_R_SetColor( nullptr );
}
...
void idCGameDraw::DrawCrosshair( rectDef_t* rect, vec4_t color )
{
    ...
    if( cg_drawCrosshair.integer == CROSSHAIR_RANGEDONLY && !bggame->Weapon( weapon )->longRanged )
    {
        return;
    }
    
    if( ( cg.snap->ps.persistant[PERS_SPECSTATE] != SPECTATOR_NOT ) || ( cg.snap->ps.stats[STAT_STATE] & SS_HOVELING ) )
    {
        return;
    }
...
=================
*/
void idCGameDraw::ScanForCrosshairEntity( float32* zChange, bool* hitClient )
{
    trace_t trace;
    vec3_t start, end;
    sint content;
    ...
    *hitClient = false;
    ...
    idCGamePredict::Trace( &trace, start, vec3_origin, vec3_origin, end, cg.snap->ps.clientNum, CONTENTS_SOLID | CONTENTS_BODY );
    ...
    *zChange = trace.endpos[2] - start[2];
    ...
    if( trace.entityNum >= MAX_CLIENTS )
    {
        ...
        if( s->eType == ET_BUILDABLE && bggame->Buildable( ( buildable_t )s->modelindex )->team == cg.snap->ps.stats[STAT_TEAM] )
        {
...
=====================
*/
void idCGameDraw::DrawLocation( rectDef_t* rect, float32 scale, sint textalign, vec4_t color )
{
    pointer location;
    centity_t* locent;
    float32 maxX;
    ...
    if( locent )
    {
        location = idCGameMain::ConfigString( CS_LOCATIONS + locent->currentState.generic1 );
    }
    else
    {
...
=====================
*/
void idCGameDraw::DrawCrosshairNames( rectDef_t* rect, float32 scale, sint textStyle )
{
    ...
    color = idCGameDrawTools::FadeColor( cg.crosshairClientTime, CROSSHAIR_CLIENT_TIMEOUT );
    if( !color )
    {
        trap_R_SetColor( nullptr );
        return;
...
void idCGameDraw::DrawSquadMarkers( vec4_t color )
{
    ...
    vec3_t origin;
    qhandle_t shader;
    float32 x, y, w, h, distance, scale, u1 = 0.f, v1 = 0.f, u2 = 1.f, v2 = 1.f;
    sint i;
    bool vertical, flip;
    ...
    for( i = 0; i < cg.snap->numEntities; i++ )
    {
        ...
        if( cent->currentState.eType != ET_PLAYER || cgs.clientinfo[cg.snap->entities[i].number].team != cg.snap->ps.stats[STAT_TEAM] || !cent->pe.squadMarked )
        {
            continue;
        }
...
===============
*/
void idCGameDraw::OwnerDraw( float32 x, float32 y, float32 w, float32 h, float32 text_x, float32 text_y, sint ownerDraw, sint ownerDrawFlags, sint align, sint textalign, sint textvalign, float32 special, float32 scale, vec4_t foreColor, vec4_t backColor, qhandle_t shader, sint textStyle )
{
    rectDef_t rect;
...
void idCGameDraw::DrawLighting( void )
{
    ...
    if( ( cg.snap->ps.stats[ STAT_STAMINA ] < -800 ) && ( cg.snap->ps.stats[ STAT_TEAM ] == TEAM_HUMANS ) )
    {
        vec4_t black = { 0, 0, 0, 0 };
        black[ 3 ] = 1.0 - ( ( float32 )( cg.snap->ps.stats[ STAT_STAMINA ] + 1000 ) / 200.0f );
...
void idCGameDraw::DrawCenterString( void )
{
    ...
    color = idCGameDrawTools::FadeColor( cg.centerPrintTime, 1000 * cg_centertime.value );
...
void idCGameDraw::DrawVote( void )
{
    ...
    Q_strncpyz( yeskey, idCGameDrawTools::KeyBinding( "vote yes" ), sizeof( yeskey ) );
...
void idCGameDraw::DrawTeamVote( void )
{
    ...
    Q_strncpyz( yeskey, idCGameDrawTools::KeyBinding( "teamvote yes" ), sizeof( yeskey ) );
    Q_strncpyz( nokey, idCGameDrawTools::KeyBinding( "teamvote no" ), sizeof( nokey ) );
    
    s = va( "TEAMVOTE(%i): \"%s\"  [%s]Yes:%i   [%s]No:%i", sec, cgs.teamVoteString[ cs_offset ], yeskey, cgs.teamVoteYes[cs_offset], nokey, cgs.teamVoteNo[ cs_offset ] );
...
void idCGameDraw::Draw2D( void )
{
    ...
    if( menu && !( cg.snap->ps.stats[ STAT_STATE ] & SS_HOVELING ) && ( cg.snap->ps.stats[ STAT_HEALTH ] > 0 ) )
    {
...
===============
*/
void idCGameDraw::ScalePainBlendTCs( float32* s1, float32* t1, float32* s2, float32* t2 )
{
    *s1 -= 0.5f;
    *t1 -= 0.5f;
    *s2 -= 0.5f;
