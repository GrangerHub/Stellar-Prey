...
void idCGameSnapshot::SetInitialSnapshot(snapshot_t *snap) {
    ...
    bggame->PlayerStateToEntityState(&snap->ps,
                                     &cg_entities[ snap->ps.clientNum ].currentState, false);
...
void idCGameSnapshot::TransitionSnapshot(void) {
    ...
    bggame->PlayerStateToEntityState(&cg.snap->ps,
                                     &cg_entities[ cg.snap->ps.clientNum ].currentState, false);
...
void idCGameSnapshot::SetNextSnap(snapshot_t *snap) {
    ...
    bggame->PlayerStateToEntityState(&snap->ps,
                                     &cg_entities[ snap->ps.clientNum ].nextState, false);
...
snapshot_t *idCGameSnapshot::ReadNextSnapshot(void) {
    ...
    if(cg.latestSnapshotNum > cgs.processedSnapshotNum + 1000) {
        Printf("WARNING: idCGameSnapshot::ReadNextSnapshot: way out of range, %i > %i",
