...
void idCGameEvent::Obituary( entityState_t* ent )
{
    ...
    valueType targetName[ MAX_NAME_LENGTH ], attackerName[ MAX_NAME_LENGTH ], className[ 64 ];
    gender_t gender;
    clientInfo_t* ci;
    bool teamKill = false;
    ...
    if( attacker != ENTITYNUM_WORLD )
    {
        ...
        if( message )
        {
            Printf( "%s" S_COLOR_WHITE " %s %s%s" S_COLOR_WHITE "%s\n", targetName, message, ( teamKill ) ? S_COLOR_RED "TEAMMATE " S_COLOR_WHITE : "",
                    attackerName, message2 );
                    
            if( teamKill && attacker == cg.clientNum )
            {
                idCGameDraw::CenterPrint( va( "You killed " S_COLOR_RED "TEAMMATE " S_COLOR_WHITE "%s", targetName ), SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH );
            }
            return;
        }
    }
...
void idCGameEvent::PainEvent( centity_t* cent, sint health )
{
    ...
    trap_S_StartSound( nullptr, cent->currentState.number, CHAN_VOICE, idCGamePlayers::CustomSound( cent->currentState.number, snd ) );
...
void idCGameEvent::Level2Zap( entityState_t* es )
{
    ...
    for( i = 0; i <= 2; i++ )
    {
        ...
        {
            source->level2ZapTS[i] = idCGameTrails::SpawnNewTrailSystem( cgs.media.level2ZapTS );
        }
        
        if( idCGameTrails::IsTrailSystemValid( &source->level2ZapTS[ i ] ) )
        {
            idCGameAttachment::SetAttachmentCent( &source->level2ZapTS[ i ]->frontAttachment, source );
            idCGameAttachment::SetAttachmentCent( &source->level2ZapTS[ i ]->backAttachment, target );
            idCGameAttachment::AttachToCent( &source->level2ZapTS[ i ]->frontAttachment );
...
void idCGameEvent::EntityEvent( centity_t* cent, vec3_t position )
{
    ...
    {
        Printf( "ent:%3i  event:%3i %s\n", es->number, _event, bggame->EventName( _event ) );
    }
    ...
    switch( _event )
    {
        ...
        case EV_FALL_MEDIUM:
            ...
            trap_S_StartSound( nullptr, es->number, CHAN_VOICE, idCGamePlayers::CustomSound( es->number, "*pain100_1.ogg" ) );
        ...
        case EV_FALL_FAR:
            trap_S_StartSound( nullptr, es->number, CHAN_AUTO, idCGamePlayers::CustomSound( es->number, "*fall1.ogg" ) );
        ...
        case EV_FALLING:
            trap_S_StartSound( nullptr, es->number, CHAN_AUTO, idCGamePlayers::CustomSound( es->number, "*falling1.ogg" ) );
            break;
        ...
        case EV_JUMP:
            trap_S_StartSound( nullptr, es->number, CHAN_VOICE, idCGamePlayers::CustomSound( es->number, "*jump1.ogg" ) );
            
            if( bggame->ClassHasAbility( ( class_t )cg.predictedPlayerState.stats[ STAT_CLASS ], SCA_WALLJUMPER ) )
            {
        ...
        case EV_TAUNT:
            ...
            {
                trap_S_StartSound( nullptr, es->number, CHAN_VOICE, idCGamePlayers::CustomSound( es->number, "*taunt.ogg" ) );
            }
            break;
        ...
        case EV_WATER_CLEAR:
            trap_S_StartSound( nullptr, es->number, CHAN_AUTO, idCGamePlayers::CustomSound( es->number, "*gasp.ogg" ) );
            break;
        ...
        case EV_MISSILE_HIT:
            ...
            idCGameWeapons::MissileHitPlayer( ( weapon_t )es->weapon, ( weaponMode_t )es->generic1, position, dir, es->otherEntityNum, es->torsoAnim );
            break;
            
        case EV_MISSILE_MISS:
            ByteToDir( es->eventParm, dir );
            idCGameWeapons::MissileHitWall( ( weapon_t )es->weapon, ( weaponMode_t )es->generic1, 0, position, dir, IMPACTSOUND_DEFAULT, es->torsoAnim );
            break;
            
        case EV_MISSILE_MISS_METAL:
            ByteToDir( es->eventParm, dir );
            idCGameWeapons::MissileHitWall( ( weapon_t )es->weapon, ( weaponMode_t )es->generic1, 0, position, dir, IMPACTSOUND_METAL, es->torsoAnim );
            break;
        ...
        case EV_TESLATRAIL:
            cent->currentState.weapon = WP_TESLAGEN;
            {
                ...
                {
                    source->muzzleTS = idCGameTrails::SpawnNewTrailSystem( cgs.media.teslaZapTS );
                    
                    if( idCGameTrails::IsTrailSystemValid( &source->muzzleTS ) )
                    {
                        idCGameAttachment::SetAttachmentCent( &source->muzzleTS->frontAttachment, source );
                        idCGameAttachment::SetAttachmentCent( &source->muzzleTS->backAttachment, target );
                        ...
                        idCGameAttachment::SetAttachmentOffset( &source->muzzleTS->frontAttachment, sourceOffset );
        ...
        case EV_BULLET_HIT_WALL:
            ByteToDir( es->eventParm, dir );
            idCGameWeapons::Bullet( es->pos.trBase, es->otherEntityNum, dir, false, ENTITYNUM_WORLD );
            break;
            
        case EV_BULLET_HIT_FLESH:
            idCGameWeapons::Bullet( es->pos.trBase, es->otherEntityNum, dir, true, es->eventParm );
            break;
        ...
        case EV_GENERAL_SOUND:
            ...
            else
            {
                ...
                trap_S_StartSound( nullptr, es->number, CHAN_VOICE, idCGamePlayers::CustomSound( es->number, s ) );
            }
            break;
        ...
        case EV_GLOBAL_SOUND:
            ...
            {
                s = idCGameMain::ConfigString( CS_SOUNDS + es->eventParm );
                trap_S_StartSound( nullptr, cg.snap->ps.clientNum, CHAN_AUTO, idCGamePlayers::CustomSound( es->number, s ) );
            }
            break;
        ...
        case EV_DEATH2:
        case EV_DEATH3:
            trap_S_StartSound( nullptr, es->number, CHAN_VOICE, idCGamePlayers::CustomSound( es->number, va( "*death%i.ogg", _event - EV_DEATH1 + 1 ) ) );
            break;
        ...
        case EV_OVERMIND_ATTACK:
            if( cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_ALIENS )
            {
                ...
                idCGameDraw::CenterPrint( "The Overmind is under attack!", 200, GIANTCHAR_WIDTH * 4 );
            }
            break;
        ...
        case EV_OVERMIND_DYING:
            if( cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_ALIENS )
            {
                ...
                idCGameDraw::CenterPrint( "The Overmind is dying!", 200, GIANTCHAR_WIDTH * 4 );
            }
            break;
        ...
        case EV_DCC_ATTACK:
            if( cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_HUMANS )
            {
                ...
                idCGameDraw::CenterPrint( "Our base is under attack!", 200, GIANTCHAR_WIDTH * 4 );
            }
            break;
        ...
        case EV_OVERMIND_SPAWNS:
            if( cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_ALIENS )
            {
                ...
                idCGameDraw::CenterPrint( "The Overmind needs spawns!", 200, GIANTCHAR_WIDTH * 4 );
            }
            break;
        ...
        case EV_ALIEN_EVOLVE:
            ...
            {
                particleSystem_t* ps = idCGameParticles::SpawnNewParticleSystem( cgs.media.alienEvolvePS );
        ...
        {
            particleSystem_t* ps = idCGameParticles::SpawnNewParticleSystem( cgs.media.alienAcidTubePS );
...
void idCGameEvent::CheckEvents( centity_t* cent )
{
    ...
    bggame->EvaluateTrajectory( &cent->currentState.pos, cg.snap->serverTime, cent->lerpOrigin );
