...
void idCGameView::AddTestModel(void) {
    ...
    cg.testModelEntity.hModel = trap_R_RegisterModel(cg.testModelName);
    cg.testModelBarrelEntity.hModel = trap_R_RegisterModel(
                                          cg.testModelBarrelName);
    ...
    if(cg.testGun) {
        ...
        for(i = 0; i < 3; i++) {
            cg.testModelEntity.origin[ i ] += cg.refdef.viewaxis[ 0 ][ i ] *
                                              cg_gun_x.value;
            cg.testModelEntity.origin[ i ] += cg.refdef.viewaxis[ 1 ][ i ] *
                                              cg_gun_y.value;
            cg.testModelEntity.origin[ i ] += cg.refdef.viewaxis[ 2 ][ i ] *
                                              cg_gun_z.value;
        }
    }
    ...
    if(cg.testModelBarrelEntity.hModel) {
        idCGameEnts::PositionEntityOnTag(&cg.testModelBarrelEntity,
                                         &cg.testModelEntity, cg.testModelEntity.hModel, "tag_barrel");
...
void idCGameView::OffsetThirdPersonView(void) {
    ...
    VectorMA(cg.refdef.vieworg, cg.predictedPlayerState.viewheight, surfNormal,
             cg.refdef.vieworg);
    ...
    if(cg.demoPlayback || ((cg.snap->ps.pm_flags & PMF_FOLLOW) &&
        ...
        mouseInputAngles[ YAW ] = -1.0 * SHORT2ANGLE(
                                      cmd.angles[ YAW ]);   // yaw is inverted
        ...
        for(i = 0; i < 3; i++) {
            rotationAngles[ i ] = AngleNormalize180(cg.refdefViewAngles[ i ]) +
                                  mouseInputAngles[ i ];
        ...
        if(!(cg.snap->ps.stats[STAT_STATE] & SS_WALLCLIMBING) ||
                !bggame->RotateAxis(cg.snap->ps.grapplePoint, axis, rotaxis, false,
    ...
    if(!cg_cameraMode.integer) {
        ...
        idCGamePredict::Trace(&trace, cg.refdef.vieworg, mins, maxs, view,
...
void idCGameView::OffsetShoulderView(void) {
    ...
    if(!cg_shoulderViewOverride.integer) {
        ...
        VectorMA(cg.refdef.vieworg, cg_shoulderViewForward.value, forward,
                 cg.refdef.vieworg);
        VectorMA(cg.refdef.vieworg, cg_shoulderViewUp.value, up,
                 cg.refdef.vieworg);
        VectorMA(cg.refdef.vieworg, cg_shoulderViewRight.value, right,
                 cg.refdef.vieworg);
    }
    ...
    rotationAngles[ YAW ] = SHORT2ANGLE(cmd.angles[ YAW ]) +
...
#define LEVEL4_FEEDBACK  10.0f
    ...
    if(ps->weapon == WP_ALEVEL4) {
        if(ps->stats[ STAT_MISC ] > 0) {
            float32 fraction = (float32)ps->stats[ STAT_MISC ] /
...
#define LEVEL3_FEEDBACK  20.0f
    ...
    if((cg.predictedPlayerState.weapon == WP_ALEVEL3 ||
        ...
        fraction1 = (float32)cg.predictedPlayerState.stats[ STAT_MISC ] /
...
#define STRUGGLE_TIME 250
    ...
    if(cg.predictedPlayerState.pm_type == PM_GRABBED) {
        ...
        usercmd_t cmd;
        sint cmdNum;
        float32 fFraction, rFraction, uFraction, fFraction2, rFraction2,
                uFraction2;
    ...
    if((cg.predictedPlayerEntity.currentState.eFlags & EF_POISONCLOUDED) &&
        ...
        scale = 1.0f - (float32)(cg.time - cg.poisonedTime) /
                bggame->PlayerPoisonCloudTime(&cg.predictedPlayerState);
    ...
    if(cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_HUMANS &&
        ...
        if(cg.predictedPlayerState.stats[ STAT_STAMINA ] <
                STAMINA_BREATHING_LEVEL) {
            float32 deltaBreath = (cg.predictedPlayerState.stats[ STAT_STAMINA ] -
    ...
    if(timeDelta < DUCK_TIME) {
        cg.refdef.vieworg[ 2 ] -= cg.duckChange * (DUCK_TIME - timeDelta) /
                                  DUCK_TIME;
    }
...
sint idCGameView::CalcFov(void) {
    ...
    if(cmd.buttons & BUTTON_USE_HOLDABLE &&
            !(oldcmd.buttons & BUTTON_USE_HOLDABLE)) {
    ...
    if(cg.predictedPlayerState.pm_type == PM_INTERMISSION ||
        ...
        attribFov = bggame->Class((class_t)
                                  cg.predictedPlayerState.stats[ STAT_CLASS ])->fov;
        fov_x = attribFov;
        ...
        if(cg.spawnTime > (cg.time - FOVWARPTIME) &&
                bggame->ClassHasAbility((class_t)
                                        cg.predictedPlayerState.stats[ STAT_CLASS ], SCA_FOVWARPS)) {
            float32 temp, temp2;
        ...
        // account for zooms
        zoomFov = bggame->Weapon((weapon_t)
                                 cg.predictedPlayerState.weapon)->zoomFov;
    ...
    if((cg.predictedPlayerEntity.currentState.eFlags & EF_POISONCLOUDED) &&
            cg.predictedPlayerState.stats[ STAT_HEALTH ] > 0 &&
            !(cg.snap->ps.pm_flags & PMF_FOLLOW)) {
        float32 scale = 1.0f - (float32)(cg.time - cg.poisonedTime) /
                        bggame->PlayerPoisonCloudTime(&cg.predictedPlayerState);
...
void idCGameView::DrawSurfNormal(void) {
    ...
    idCGamePredict::Trace(&tr, cg.refdef.vieworg, nullptr, nullptr, end,
                          cg.predictedPlayerState.clientNum, MASK_SOLID);
...
===============
*/
void idCGameView::addSmoothOp(vec3_t rotAxis, float32 rotAngle,
                              float32 timeMod) {
    sint i;
...
===============
*/
void idCGameView::smoothWWTransitions(playerState_t *ps, const vec3_t in,
                                      vec3_t out) {
    ...
    for(i = MAXSMOOTHS - 1; i >= 0; i--) {
        ...
        if(cg.time < cg.sList[ i ].time + smoothTime) {
            stLocal = 1.0f - (((cg.sList[ i ].time + smoothTime) - cg.time) /
                              smoothTime);
            sFraction = -(cos(stLocal * M_PI) + 1.0f) / 2.0f;

            RotatePointAroundVector(outAxis[ 0 ], cg.sList[ i ].rotAxis, inAxis[ 0 ],
                                    sFraction * cg.sList[ i ].rotAngle);
            RotatePointAroundVector(outAxis[ 1 ], cg.sList[ i ].rotAxis, inAxis[ 1 ],
                                    sFraction * cg.sList[ i ].rotAngle);
            RotatePointAroundVector(outAxis[ 2 ], cg.sList[ i ].rotAxis, inAxis[ 2 ],
                                    sFraction * cg.sList[ i ].rotAngle);
...
===============
*/
void idCGameView::smoothWJTransitions(playerState_t *ps, const vec3_t in,
                                      vec3_t out) {
    sint i;
...
sint idCGameView::CalcViewValues(void) {
    ...
    cg.xyspeed = sqrt(ps->velocity[ 0 ] * ps->velocity[ 0 ] + ps->velocity[ 1 ]
                      * ps->velocity[ 1 ]);
    ...
    if(bggame->ClassHasAbility((class_t)ps->stats[STAT_CLASS],
                               SCA_WALLCLIMBER)) {
        smoothWWTransitions(ps, ps->viewangles, cg.refdefViewAngles);
    } else if(bggame->ClassHasAbility((class_t)ps->stats[STAT_CLASS],
                                      SCA_WALLJUMPER)) {
...
=================
*/
void idCGameLocal::DrawActiveFrame(sint serverTime,
                                   stereoFrame_t stereoView, bool demoPlayback) {
    sint inwater;
    ...
    trap_SetUserCmdValue(cg.weaponSelect, cg.zoomSensitivity ? 0x01 : 0x00,
    ...
    cg.renderingThirdPerson = (cg_thirdPerson.integer ||
                               (cg.snap->ps.stats[ STAT_HEALTH ] <= 0) || (cg.chaseFollow &&
                                       cg.snap->ps.pm_flags & PMF_FOLLOW));
