...
void idCGameServerCmds::SetConfigValues( void )
{
    ...
    sscanf( idCGameMain::ConfigString( CS_STAGES ), "%d %d %d %d %d %d", &cgs.alienStage, &cgs.humanStage,
...
================
*/
void idCGameServerCmds::AnnounceAlienStageTransistion( stage_t from, stage_t to )
{
...
================
*/
void idCGameServerCmds::AnnounceHumanStageTransistion( stage_t from, stage_t to )
{
    ...
    if( to > from )
    {
        idCGameDraw::CenterPrint( "Reinforcements have arrived!", 200, GIANTCHAR_WIDTH * 4 );
    }
    else
    {
        idCGameDraw::CenterPrint( "Reinforcements have been withdrawn!", 200, GIANTCHAR_WIDTH * 4 );
    }
}
...
void idCGameServerCmds::ConfigStringModified( void )
{
    ...
    {
        sscanf( str, "%d %d %d %d", &cgs.alienBuildPoints, &cgs.alienBuildPointsTotal, &cgs.humanBuildPoints, &cgs.humanBuildPointsTotal );
    }
    ...
    {
        Q_strncpyz( cgs.teamVoteString[ num - CS_TEAMVOTE_STRING ], str, sizeof( cgs.teamVoteString[ num - CS_TEAMVOTE_STRING ] ) );
    }
    ...
    {
        cgs.gameParticleSystems[num - CS_PARTICLE_SYSTEMS] = idCGameParticles::RegisterParticleSystem( ( valueType* )str );
    }
...
void idCGameServerCmds::Menu( sint menu, sint arg )
{
    ...
    pointer dialog;
    cgameDialogType_t type = ( cgameDialogType_t )0; // controls which cg_disable var will switch it off
    ...
    switch( menu )
    {
        ...
        case MN_A_INFEST:
            trap_Cvar_Set( "ui_currentClass", va( "%d %d",  cg.snap->ps.stats[ STAT_CLASS ], cg.snap->ps.persistant[ PERS_CREDIT ] ) );
            cmd = "menu tremulous_alienupgrade\n";
            type = DT_INTERACTIVE;
            break;
            
        case MN_A_CANTEVOLVE:
            shortMsg = va( "You cannot evolve into a %s", bggame->ClassConfig( ( class_t )arg )->humanName );
            type = DT_ARMOURYEVOLVE;
            break;
        ...
        case MN_A_CLASSNOTSPAWN:
            shortMsg = va( "You cannot spawn as a %s", bggame->ClassConfig( ( class_t )arg )->humanName );
            type = DT_ARMOURYEVOLVE;
            break;
            
        case MN_A_CLASSNOTALLOWED:
            shortMsg = va( "The %s is not allowed", bggame->ClassConfig( ( class_t )arg )->humanName );
            type = DT_ARMOURYEVOLVE;
            break;
            
        case MN_A_CLASSNOTATSTAGE:
            shortMsg = va( "The %s is not allowed at Stage %d", bggame->ClassConfig( ( class_t )arg )->humanName, cgs.alienStage + 1 );
            type = DT_ARMOURYEVOLVE;
            break;
...
=================
*/
voiceTrack_t* idCGameServerCmds::VoiceTrack( valueType* voice, sint cmd, sint track )
{
    voice_t* v;
    voiceCmd_t* c;
    voiceTrack_t* t;
...
void idCGameServerCmds::ParseVoice( void )
{
    ...
    track = VoiceTrack( ci->voice, atoi( idCGameMain::Argv( 3 ) ), atoi( idCGameMain::Argv( 4 ) ) );
...
{
    idCGameDraw::CenterPrint( idCGameMain::Argv( 1 ), SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH );
}
...
void idCGameServerCmds::PoisonCloud_f( void )
{
    ...
    {
        cg.poisonCloudPS = idCGameParticles::SpawnNewParticleSystem( cgs.media.poisonCloudPS );
        idCGameAttachment::SetAttachmentCent( &cg.poisonCloudPS->attachment, &cg.predictedPlayerEntity );
...
void idCGameServerCmds::PlaySound( void )
{
    ...
    VectorSet( origin, atof( idCGameMain::Argv( 2 ) ), atof( idCGameMain::Argv( 3 ) ), atof( idCGameMain::Argv( 4 ) ) );
