...
======================
*/
bool idCGameWeapons::ParseWeaponAnimationFile(pointer filename,
        weaponInfo_t *weapon) {
    valueType *text_p;
    sint len, i;
    valueType *token;
...
===============
*/
bool idCGameWeapons::ParseWeaponModeSection(weaponInfoMode_t *wim,
        valueType **text_p) {
    valueType *token;
    sint i;
    ...
    while(1) {
        ...
        if(!Q_stricmp(token, "missileModel")) {
            ...
            wim->missileParticleSystem = idCGameParticles::RegisterParticleSystem(
                                             token);
            ...
            wim->muzzleParticleSystem = idCGameParticles::RegisterParticleSystem(
                                            token);
            ...
            wim->impactParticleSystem = idCGameParticles::RegisterParticleSystem(
                                            token);
...
void idCGameWeapons::RegisterWeapon(sint weaponNum) {
    ...
    Q_vsprintf_s(path, MAX_QPATH, MAX_QPATH, "models/weapons/%s/weapon.cfg",
                 bggame->Weapon((weapon_t)weaponNum)->name);
    ...
    Q_vsprintf_s(path, MAX_QPATH, MAX_QPATH, "models/weapons/%s/animation.cfg",
                 bggame->Weapon((weapon_t)weaponNum)->name);

    if(!cg_suppressWAnimWarnings.integer &&
            !ParseWeaponAnimationFile(path, weaponInfo)) {
        Com_Printf(S_COLOR_RED "ERROR: failed to parse %s\n", path);
    }
...
void idCGameWeapons::InitWeapons(void) {
    ...
    cgs.media.level2ZapTS =
        idCGameTrails::RegisterTrailSystem("models/weapons/lev2zap/lightning");
    cgs.media.massDriverTS =
        idCGameTrails::RegisterTrailSystem("models/weapons/mdriver/fireTS");
}
...
===============
*/
void idCGameWeapons::SetWeaponLerpFrameAnimation(weapon_t weapon,
...
===============
*/
void idCGameWeapons::WeaponAnimation(centity_t *cent, sint *old, sint *now,
                                     float32 *backLerp) {
...
==============
*/
void idCGameWeapons::CalculateWeaponPosition(vec3_t origin,
        vec3_t angles) {
    ...
    bob = bggame->Class((class_t)
                        cg.predictedPlayerState.stats[ STAT_CLASS ])->bob;
    ...
    if(weapon->noDrift) {
        ...
        if(delta < LAND_DEFLECT_TIME) {
            ...
            origin[2] += cg.landChange * 0.25 * (LAND_DEFLECT_TIME + LAND_RETURN_TIME -
...
=============
*/
void idCGameWeapons::AddPlayerWeapon(refEntity_t *parent,
    ...
    if(((cent->currentState.eFlags & EF_FIRING) &&
            weaponMode == WPM_PRIMARY) || ((cent->currentState.eFlags & EF_FIRING2) &&
                                           weaponMode == WPM_SECONDARY) ||
    ...
    if(!ps) {
        ...
        if(firing && weapon->wim[ weaponMode ].firingSound) {
            trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                                   vec3_origin, weapon->wim[weaponMode].firingSound);
    ...
    if(!noGunModel) {
        idCGameEnts::PositionEntityOnTag(&gun, parent, parent->hModel,
                                         "tag_weapon");
        ...
        if(barrel.hModel) {
            ...
            idCGameEnts::PositionRotatedEntityOnTag(&barrel, &gun, gun.hModel,
                                                    "tag_barrel");
    ...
    if(idCGameParticles::IsParticleSystemValid(&cent->muzzlePS)) {
        if(ps || cg.renderingThirdPerson ||
            ...
            if(noGunModel) {
                idCGameAttachment::SetAttachmentTag(&cent->muzzlePS->attachment, *parent,
                                                    parent->hModel, "tag_weapon");
            } else {
                idCGameAttachment::SetAttachmentTag(&cent->muzzlePS->attachment, gun,
                                                    gun.hModel, "tag_flash");
            }
        }
    ...
    if(flash.hModel) {
        ...
        if(noGunModel) {
            idCGameEnts::PositionRotatedEntityOnTag(&flash, parent, parent->hModel,
                                                    "tag_weapon");
        } else {
            idCGameEnts::PositionRotatedEntityOnTag(&flash, &gun, gun.hModel,
                                                    "tag_flash");
        }
    ...
    if(ps || cg.renderingThirdPerson ||
        ...
        if(weapon->wim[ weaponMode ].muzzleParticleSystem &&
            ...
            cent->muzzlePS = idCGameParticles::SpawnNewParticleSystem(
...
void idCGameWeapons::AddViewWeapon(playerState_t *ps) {
    ...
    if((ps->stats[STAT_BUILDABLE] & ~SB_VALID_TOGGLEBIT) > BA_NONE) {
        idCGameBuildable::GhostBuildable((buildable_t)(ps->stats[STAT_BUILDABLE] &
                                         ~SB_VALID_TOGGLEBIT));
    }
    ...
    if(!cg_drawGun.integer) {
        ...
        if(wi->wim[ weaponMode ].muzzleParticleSystem && cent->muzzlePsTrigger) {
            cent->muzzlePS = idCGameParticles::SpawnNewParticleSystem(
                                 wi->wim[ weaponMode ].muzzleParticleSystem);
...
void idCGameWeapons::DrawItemSelect(rectDef_t *rect, vec4_t color) {
    ...
    if(!(cg.snap->ps.pm_flags & PMF_FOLLOW)) {
        ...
        if(cg.weaponSelect <= 32 && !WeaponSelectable((weapon_t)cg.weaponSelect)) {
            NextWeapon_f();
                  ...
                  !UpgradeSelectable((upgrade_t)(cg.weaponSelect - 32))) {
            NextWeapon_f();
        }
    }
    ...
    for(i = 0; i < length; i++) {
        ...
        if(item >= 0 && item < numItems) {
            ...
            if(items[item] <= 32) {
                idCGameDrawTools::DrawPic(x, y, iconWidth, iconHeight,
...
===================
*/
void idCGameWeapons::DrawItemSelectText(rectDef_t *rect, float32 scale,
                                        sint textStyle) {
    sint x, w;
    valueType *name;
    float32 *color;

    color = idCGameDrawTools::FadeColor(cg.weaponSelectTime,
                                        WEAPON_SELECT_TIME);
    ...
    if(cg.weaponSelect <= 32) {
        if(cg_weapons[ cg.weaponSelect ].registered &&
                bggame->InventoryContainsWeapon(cg.weaponSelect, cg.snap->ps.stats)) {
        ...
        if(cg_upgrades[ cg.weaponSelect - 32 ].registered &&
                bggame->InventoryContainsUpgrade(cg.weaponSelect - 32,
                        cg.snap->ps.stats)) {
...
void idCGameWeapons::FireWeapon(centity_t *cent, weaponMode_t weaponMode) {
    ...
    if(wi->wim[ weaponMode ].muzzleParticleSystem) {
        if(!idCGameParticles::IsParticleSystemValid(&cent->muzzlePS) ||
                !idCGameParticles::IsParticleSystemInfinite(cent->muzzlePS)) {
...
=================
*/
void idCGameWeapons::HandleAlienFeedback(centity_t *cent,
...
=================
*/
void idCGameWeapons::MissileHitWall(weapon_t weaponNum,
                                    weaponMode_t weaponMode, sint clientNum, vec3_t origin, vec3_t dir,
                                    impactSound_t soundType, sint charge) {
    ...
    if(soundType == IMPACTSOUND_FLESH) {
        ...
        if(c > 0) {
            ...
            if(weapon->wim[weaponMode].impactFleshSound[c]) {
                trap_S_StartSound(origin, ENTITYNUM_WORLD, CHAN_AUTO,
                                  weapon->wim[weaponMode].impactFleshSound[c]);
            }
        }
    } else {
        //normal sound
        ...
        if(c > 0) {
            ...
            if(weapon->wim[weaponMode].impactSound[c]) {
                trap_S_StartSound(origin, ENTITYNUM_WORLD, CHAN_AUTO,
                                  weapon->wim[weaponMode].impactSound[c]);
            }
        }
    }
    ...
    if(ps) {
        particleSystem_t *partSystem = idCGameParticles::SpawnNewParticleSystem(
                                           ps);
    ...
    // impact mark
    if(radius > 0.0f) {
        idCGameMarks::ImpactMark(mark, origin, dir, random() * 360, 1, 1, 1, 1,
                                 false, radius, false);
    }
}
...
=================
*/
void idCGameWeapons::MissileHitPlayer(weapon_t weaponNum,
                                      weaponMode_t weaponMode, vec3_t origin, vec3_t dir, sint entityNum,
                                      sint charge) {
    vec3_t normal;
    weaponInfo_t *weapon = &cg_weapons[ weaponNum ];
    ...
    if(weapon->wim[ weaponMode ].alwaysImpact) {
        ...
        if(cg_entities[entityNum].currentState.eType == ET_PLAYER ||
                (cg_entities[entityNum].currentState.eType == ET_BUILDABLE &&
                 bggame->Buildable((buildable_t)
                                   cg_entities[entityNum].currentState.modelindex)->team == TEAM_ALIENS)) {
            sound = IMPACTSOUND_FLESH;
        }

        MissileHitWall(weaponNum, weaponMode, 0, origin, dir, (impactSound_t)sound,
                       charge);
    }
}
...
======================
*/
void idCGameWeapons::Bullet(vec3_t end, sint sourceEntityNum,
                            vec3_t normal, bool flesh, sint fleshEntityNum) {
    vec3_t start;
    ...
    if(flesh) {
        ...
        MissileHitWall(WP_MACHINEGUN, WPM_PRIMARY, 0, end, normal,
...
================
*/
void idCGameWeapons::ShotgunPattern(vec3_t origin, vec3_t origin2,
                                    sint seed, sint otherEntNum) {
    sint i;
    float32 r, u;
    ...
    for(i = 0; i < SHOTGUN_PELLETS; i++) {
        ...
        idCGamePredict::Trace(&tr, origin, nullptr, nullptr, end, otherEntNum,
                              MASK_SHOT);

        if(!(tr.surfaceFlags & SURF_NOIMPACT)) {
            if(cg_entities[tr.entityNum].currentState.eType == ET_PLAYER) {
                MissileHitPlayer(WP_SHOTGUN, WPM_PRIMARY, tr.endpos, tr.plane.normal,
                                 tr.entityNum, 0);
            } else if(tr.surfaceFlags & SURF_METALSTEPS) {
                MissileHitWall(WP_SHOTGUN, WPM_PRIMARY, 0, tr.endpos, tr.plane.normal,
                               IMPACTSOUND_METAL, 0);
            } else {
                MissileHitWall(WP_SHOTGUN, WPM_PRIMARY, 0, tr.endpos, tr.plane.normal,
                               IMPACTSOUND_DEFAULT, 0);
...
void idCGameWeapons::ShotgunFire(entityState_t *es) {
    ...
    ShotgunPattern(es->pos.trBase, es->origin2, es->eventParm,
                   es->otherEntityNum);
}
