...
void idCGameTrails::CalculateBeamNodeProperties( trailBeam_t* tb )
{
    ...
    for( j = 0, i = tb->nodes; i; i = i->next, j++ )
    {
        ...
        {
            i->textureCoord = tb->_class->frontTextureCoord + ( ( position / totalDistance ) * TCRange );
        }
        ...
        i->halfWidth = ( tb->_class->frontWidth + ( ( position / totalDistance ) * widthRange ) ) / 2.0f;
        i->alpha = ( uchar8 )( ( float32 )0xFF * ( tb->_class->frontAlpha + ( ( position / totalDistance ) * alphaRange ) ) * fadeAlpha );
        
        VectorMA( tb->_class->frontColor, ( position / totalDistance ), colorRange, i->color );
...
void idCGameTrails::RenderBeam( trailBeam_t* tb )
{
    ...
    if( bts->thirdPersonOnly && ( idCGameAttachment::AttachmentCentNum( &ts->frontAttachment ) == cg.snap->ps.clientNum ||
                                  idCGameAttachment::AttachmentCentNum( &ts->backAttachment ) == cg.snap->ps.clientNum ) && !cg.renderingThirdPerson )
    {
        return;
    }
...
void idCGameTrails::UpdateBeam( trailBeam_t* tb )
{
    ...
    if( ts->destroyTime <= 0 )
    {
        ...
        while( nodesToAdd-- )
        {
            ...
            if( !tb->nodes->next && idCGameAttachment::Attached( &ts->frontAttachment ) )
            {
                ...
                if( !idCGameAttachment::AttachmentPoint( &ts->frontAttachment, i->refPosition ) )
                {
    ...
    if( idCGameAttachment::Attached( &ts->frontAttachment ) && idCGameAttachment::Attached( &ts->backAttachment ) )
    {
    ...
    else if( idCGameAttachment::Attached( &ts->frontAttachment ) )
    {
        ...
        if( i && i->timeLeft >= 0 )
        {
            ...
            else if( i->timeLeft >= 0 && i->prev )
            {
                ...
                length = VectorNormalize( dir ) * ( ( float32 )i->timeLeft / ( float32 )tb->_class->segmentTime );
        ...
        if( tb->nodes )
        {
            if( !idCGameAttachment::AttachmentPoint( &ts->frontAttachment, tb->nodes->refPosition ) )
            {
                DestroyTrailSystem( &ts );
            }
...
===============
*/
bool idCGameTrails::ParseTrailBeam( baseTrailBeam_t* btb, valueType** text_p )
{
    valueType* token;
...
===============
*/
bool idCGameTrails::ParseTrailSystem( baseTrailSystem_t* bts, valueType** text_p, pointer name )
{
    valueType* token;
...
===============
*/
trailBeam_t* idCGameTrails::SpawnNewTrailBeam( baseTrailBeam_t* btb, trailSystem_t* parent )
{
    sint i;
...
void idCGameTrails::GarbageCollectTrailSystems( void )
{
    ...
    for( i = 0; i < MAX_TRAIL_SYSTEMS; i++ )
    {
        ...
        if( ( centNum = idCGameAttachment::AttachmentCentNum( &ts->frontAttachment ) ) >= 0 && centNum != cg.snap->ps.clientNum )
        {
        ...
        if( ( centNum = idCGameAttachment::AttachmentCentNum( &ts->backAttachment ) ) >= 0 && centNum != cg.snap->ps.clientNum )
        {
            trailSystem_t* tempTS = ts;
        ...
        // lifetime expired
        if( ts->destroyTime <= 0 && ts->_class->lifeTime && ts->birthTime + ts->_class->lifeTime < cg.time )
        {
            trailSystem_t* tempTS = ts;
            ...
            if( cg_debugTrails.integer >= 1 )
            {
                Printf( "TS %s expired (born %d, lives %d, now %d)\n", ts->_class->name, ts->birthTime, ts->_class->lifeTime, cg.time );
            }
        }
...
void idCGameTrails::TestTS_f( void )
{
    ...
    if( testTSHandle )
    {
        ...
        if( IsTrailSystemValid( &testTS ) )
        {
            idCGameAttachment::SetAttachmentCent( &testTS->frontAttachment, &cg_entities[ 0 ] );
