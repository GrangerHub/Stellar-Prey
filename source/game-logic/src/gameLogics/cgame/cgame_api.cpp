...
void trap_Cvar_Register(vmConvar_t *vmCvar, pointer varName,
    ...
    imports->cvarSystem->Register(vmCvar, varName, defaultValue, flags,
                                  description);
}
...
void trap_Cvar_LatchedVariableStringBuffer(pointer var_name,
    ...
    imports->cvarSystem->LatchedVariableStringBuffer(var_name, buffer,
            bufsize);
}
...
sint trap_CM_TransformedPointContents(const vec3_t p, clipHandle_t model,
    ...
    return imports->collisionModelManager->TransformedPointContents(p, model,
            origin, angles);
}
...
void trap_CM_BoxTrace(trace_t *results, const vec3_t start,
                      ...
                      sint brushmask) {
    imports->collisionModelManager->BoxTrace(results, start, end, mins, maxs,
            model, brushmask, TT_AABB);
}
...
void trap_CM_TransformedBoxTrace(trace_t *results, const vec3_t start,
    ...
    imports->collisionModelManager->TransformedBoxTrace(results, start, end,
            mins, maxs, model, brushmask, origin, angles, TT_AABB);
}
...
void trap_CM_TransformedCapsuleTrace(trace_t *results, const vec3_t start,
                                     ...
                                     sint brushmask, const vec3_t origin, const vec3_t angles) {
    imports->collisionModelManager->TransformedBoxTrace(results, start, end,
            mins, maxs, model, brushmask, origin, angles, TT_CAPSULE);
}
...
void trap_CM_BiSphereTrace(trace_t *results, const vec3_t start,
                           ...
                           sint mask) {
    imports->collisionModelManager->BiSphereTrace(results, start, end,
...
void trap_CM_TransformedBiSphereTrace(trace_t *results, const vec3_t start,
    ...
    imports->collisionModelManager->TransformedBiSphereTrace(results, start,
            end, PASSFLOAT(startRad), PASSFLOAT(endRad), model, mask, origin);
}
...
sint trap_CM_MarkFragments(sint numPoints, const vec3_t *points,
    ...
    return imports->renderSystem->MarkFragments(numPoints, points, projection,
...
void trap_S_AddRealLoopingSound(sint entityNum, const vec3_t origin,
                                const vec3_t velocity, sfxHandle_t sfx) {
    imports->soundSystem->AddRealLoopingSound(entityNum, origin, velocity,
            sfx);
}
...
void trap_R_DrawStretchPic(float32 x, float32 y, float32 w, float32 h,
    ...
    imports->renderSystem->DrawStretchPic(PASSFLOAT(x), PASSFLOAT(y),
                                          PASSFLOAT(w), PASSFLOAT(h), PASSFLOAT(s1), PASSFLOAT(t1), PASSFLOAT(s2),
...
void trap_GetCurrentSnapshotNumber(sint *snapshotNumber,
                                   sint *serverTime) {
    imports->clientGameSystem->GetCurrentSnapshotNumber(snapshotNumber,
            serverTime);
}
...
void trap_SetUserCmdValue(sint stateValue, sint flags,
    ...
    imports->clientGameSystem->SetUserCmdValue(stateValue, flags,
            PASSFLOAT(sensitivityScale), mpIdentClient);
}

void trap_SetClientLerpOrigin(float32 x, float32 y, float32 z) {
    imports->clientGameSystem->SetClientLerpOrigin(PASSFLOAT(x), PASSFLOAT(y),
            PASSFLOAT(z));
}
...
sint trap_R_LightForPoint(vec3_t point, vec3_t ambientLight,
    ...
    return imports->renderSystem->LightForPoint(point, ambientLight,
