...
====================
*/
void idCGamePredict::ClipMoveToEntities(const vec3_t start,
                                        const vec3_t mins, const vec3_t maxs, const vec3_t end, sint skipNumber,
                                        sint mask, trace_t *tr, traceType_t collisionType) {
    sint i, j, x, zd, zu;
    trace_t trace;
    entityState_t *ent;
    clipHandle_t cmodel;
    ...
    for(i = 0; i < j; i++) {
        ...
        if(ent->solid == SOLID_BMODEL) {
            ...
            bggame->EvaluateTrajectory(&cent->currentState.pos, cg.physicsTime,
                                       origin);
        } else {
            // encoded bbox
            x = (ent->solid & 255);
            ...
            if(i == cg_numSolidEntities) {
                bggame->ClassBoundingBox((class_t)((ent->misc >> 8) & 0xFF), bmins, bmaxs,
        ...
        if(collisionType == TT_CAPSULE) {
            ...
            trap_CM_TransformedBiSphereTrace(&trace, start, end, mins[ 0 ], maxs[ 0 ],
                                             cmodel, mask, origin);
        }
...
================
*/
void idCGamePredict::Trace(trace_t *result, const vec3_t start,
                           const vec3_t mins, const vec3_t maxs, const vec3_t end, sint skipNumber,
                           sint mask) {
    trace_t t;
...
================
*/
void idCGamePredict::CapTrace(trace_t *result, const vec3_t start,
                              const vec3_t mins, const vec3_t maxs, const vec3_t end, sint skipNumber,
                              sint mask) {
    trace_t t;
...
================
*/
void idCGamePredict::BiSphereTrace(trace_t *result, const vec3_t start,
...
================
*/
sint idCGamePredict::PointContents(const vec3_t point,
                                   sint passEntityNum) {
    sint i, contents;
    entityState_t *ent;
    centity_t *cent;
...
void idCGamePredict::InterpolatePlayerState(bool grabAngles) {
    ...
    *out = cg.snap->ps;
    ...
    f = (float32)(cg.time - prev->serverTime) / (next->serverTime -
            prev->serverTime);
    ...
    for(i = 0; i < 3; i++) {
        out->origin[ i ] = prev->ps.origin[ i ] + f * (next->ps.origin[ i ] -
                           prev->ps.origin[ i ]);

        if(!grabAngles) {
            out->viewangles[i] = LerpAngle(prev->ps.viewangles[i],
                                           next->ps.viewangles[i], f);
        }

        out->velocity[ i ] = prev->ps.velocity[ i ] + f * (next->ps.velocity[ i ] -
                             prev->ps.velocity[ i ]);
    }
}
...
void idCGamePredict::TouchTriggerPrediction(void) {
    ...
    for(i = 0; i < cg_numTriggerEntities; i++) {
        ...
        trap_CM_BoxTrace(&trace, cg.predictedPlayerState.origin,
                         cg.predictedPlayerState.origin, cg_pmove.mins, cg_pmove.maxs, cmodel, -1);
...
=========================
*/
sint idCGamePredict::IsUnacceptableError(playerState_t *ps,
        playerState_t *pps) {
    vec3_t delta;
    sint i;
    ...
    if(pps->damageEvent != ps->damageEvent ||
            pps->damageYaw != ps->damageYaw || pps->damagePitch != ps->damagePitch ||
...
void idCGamePredict::PredictPlayerState(void) {
    ...
    if(cg_optimizePrediction.integer) {
        ...
        // as the last one
            ...
            for(i = cg.stateHead; i != cg.stateTail; i = (i + 1) % NUM_SAVED_STATES) {
                ...
                errorcode = IsUnacceptableError(&cg.predictedPlayerState,
    ...
    for(cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++) {
        ...
        if(cg.predictedPlayerState.commandTime == oldPlayerState.commandTime) {
            ...
            if(cg.thisFrameTeleport) {
                ...
                vec3_t adjusted;
                idCGameEnts::AdjustPositionForMover(cg.predictedPlayerState.origin,
    ...
    idCGameEnts::AdjustPositionForMover(cg.predictedPlayerState.origin,
                                        cg.predictedPlayerState.groundEntityNum, cg.physicsTime, cg.time,
                                        cg.predictedPlayerState.origin);


    // fire events and other transition triggered things
    idCGamePlayerState::TransitionPlayerState(&cg.predictedPlayerState,
            &oldPlayerState);
}
