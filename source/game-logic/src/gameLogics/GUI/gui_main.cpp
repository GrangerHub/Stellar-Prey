...
void UI_DrawSides( float32 x, float32 y, float32 w, float32 h, float32 size )
{
...
void UI_DrawTopBottom( float32 x, float32 y, float32 w, float32 h, float32 size )
{
    UI_AdjustFrom640( &x, &y, &w, &h );
...
static void UI_DrawCorners( float32 x, float32 y, float32 w, float32 h, float32 size, qhandle_t pic )
{
    float32 hs, vs;
    UI_AdjustFrom640( &x, &y, &w, &h );
...
=================
*/
void UI_DrawRect( float32 x, float32 y, float32 width, float32 height, float32 size, const float32* color )
{
...
=================
*/
void UI_DrawRoundedRect( float32 x, float32 y, float32 width, float32 height, float32 size, const float32* color )
{
    trap_R_SetColor( color );
...
=================
*/
void UI_FillRect( float32 x, float32 y, float32 width, float32 height, const float32* color )
{
    trap_R_SetColor( color );
...
=================
*/
void UI_FillRoundedRect( float32 x, float32 y, float32 width, float32 height, float32 size, const float32* color )
{
...
==================
*/
static sint UI_GetServerStatusInfo( valueType* serverAddress, serverStatusInfo_t* info )
{
        ...
        if( info->numLines < MAX_SERVERSTATUS_LINES - 3 )
        {
            ...
            while( p && *p )
            {
                ...
                *p++ = '\0';
                ...
                Q_vsprintf_s( &info->pings[len], sizeof( info->pings ) - len, sizeof( info->pings ) - len, "%d", i );
...
static void UI_BuildFindPlayerList( bool force )
{
    ...
    else
    {
        ...
        Q_vsprintf_s( uiInfo.foundPlayerServerNames[uiInfo.numFoundPlayerServers - 1], sizeof( uiInfo.foundPlayerServerNames[uiInfo.numFoundPlayerServers - 1] ),
    ...
    else
    {
        ...
        {
            Q_vsprintf_s( uiInfo.foundPlayerServerNames[uiInfo.numFoundPlayerServers - 1], sizeof( uiInfo.foundPlayerServerAddresses[0] ),
...
===============
*/
static void UI_DrawInfoPane( menuItem_t* item, rectDef_t* rect, float32 text_x, float32 text_y,
                             float32 scale, sint textalign, sint textvalign, vec4_t color, sint textStyle )
{
    sint         value = 0;
    pointer  s = "";
    valueType*        string = "";
...
static void UI_DrawServerMapPreview( rectDef_t* rect, float32 scale, vec4_t color )
{
...
static void UI_DrawSelectedMapPreview( rectDef_t* rect, float32 scale, vec4_t color )
{
...
static void UI_DrawSelectedMapName( rectDef_t* rect, float32 scale, vec4_t color, sint textStyle )
{
    sint map = ui_selectedMap.integer;
...
static void UI_DrawGLInfo( rectDef_t* rect, float32 scale, sint textalign, sint textvalign,
                           vec4_t color, sint textStyle, float32 text_x, float32 text_y )
{
    valueType      buffer[ 4096 ];
    
    Q_vsprintf_s( buffer, sizeof( buffer ), sizeof( buffer ), "VENDOR: %s\nVERSION: %s\n"
...
static void UI_DrawCredits( rectDef_t* rect, float32 scale, sint textalign, sint textvalign,
                            vec4_t color, sint textStyle, float32 text_x, float32 text_y )
{
...
static bool UI_NetSource_HandleKey( sint flags, float32* special, sint key )
{
...
static bool UI_OwnerDrawHandleKey( sint ownerDraw, sint flags, float32* special, sint key )
{
    switch( ownerDraw )
    {
        case UI_NETSOURCE:
...
static void UI_LoadHumanArmouryBuys( void )
{
    ...
    for( i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++ )
    {
                ...
                i != uiInfo.weapon )
        {
            ...
            {
                Q_vsprintf_s( buffer, sizeof( buffer ), sizeof( buffer ), "cmd sell %s;", bggame->Weapon( ( weapon_t )uiInfo.weapon )->name );
    ...
    for( i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++ )
    {
                ...
                !( uiInfo.upgrades & ( 1 << i ) ) )
        {
            ...
            for( i2 = UP_NONE + 1; i2 < UP_NUM_UPGRADES; i2++ )
            {
                ...
                {
                    Q_vsprintf_s( buffer, sizeof( buffer ), sizeof( buffer ), "%scmd sell %s;", buffer, bggame->Upgrade( ( upgrade_t )i2 )->name );
        ...
        if( dirptr[0] && Q_stricmp( dirptr, "." ) && Q_stricmp( dirptr, ".." ) )
        {
            ...
            if( uiInfo.profileIndex == -1 )
            {
                ...
                if( !Q_stricmp( token.string, cl_profile.string ) )
                {
                    ...
                    {
                        Menu_SetFeederSelection( Menu_Get( j ), FEEDER_PROFILES, uiInfo.profileIndex, nullptr );
                    }
                }
            }
...
static void UI_LoadDemos( void )
{
    ...
    Q_vsprintf_s( demoExt, sizeof( demoExt ), sizeof( demoExt ), "dm_%d", ( sint )trap_Cvar_VariableValue( "protocol" ) );
...
void UI_ScreenChange( valueType** args )
{
    ...
    // Sort the list
    qsort( screenshots, maxscreens, MAX_QPATH, ( sint(* )( const void*, const void* ) )strcmp );
...
static pointer UI_FeederItemText( float32 feederID, sint index, sint column, qhandle_t* handle )
{
                ...
                case SORT_HOST:
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            Q_vsprintf_s( hostname, sizeof( hostname ), sizeof( hostname ), "%s", Info_ValueForKey( info, "hostname" ) );
    ...
    else if( feederID == FEEDER_RESOLUTIONS )
    {
        if( uiInfo.oldResolutions )
        {
            ...
            {
                Q_vsprintf_s( resolution, sizeof( resolution ), sizeof( resolution ), "Custom (%dx%d)",
        ...
        else
        {
            ...
            for( i = 0; i < uiInfo.numResolutions; i++ )
            {
                ...
                {
                    Q_vsprintf_s( resolution, sizeof( resolution ), sizeof( resolution ), "%dx%d", w, h );
                    return resolution;
                }
            }
            
            Q_vsprintf_s( resolution, sizeof( resolution ), sizeof( resolution ), "Custom (%dx%d)", w, h );
            return resolution;
        }
    }
    else if( feederID == FEEDER_PROFILES )
...
static sint UI_PlayCinematic( pointer name, float32 x, float32 y, float32 w, float32 h )
{
...
static void UI_DrawCinematic( sint handle, float32 x, float32 y, float32 w, float32 h )
{
...
static void UI_ReadableSize( valueType* buf, sint bufsize, sint value )
{
    if( value > 1024 * 1024 * 1024 )
    {
        ...
        Q_vsprintf_s( buf + strlen( buf ), bufsize - strlen( buf ), bufsize - strlen( buf ), ".%02d GB",
    ...
    else if( value > 1024 * 1024 )
    {
        ...
        Q_vsprintf_s( buf + strlen( buf ), bufsize - strlen( buf ), bufsize - strlen( buf ), ".%02d MB",
...
static void UI_PrintTime( valueType* buf, sint bufsize, sint time )
{
    ...
    {
        // in the hours range
        Q_vsprintf_s( buf, bufsize, bufsize, "%d hr %d min", time / 3600, ( time % 3600 ) / 60 );
    }
    else if( time > 60 )
    {
        // mins
...
void Text_PaintCenter( float32 x, float32 y, float32 scale, vec4_t color, pointer text, float32 adjust )
{
    sint len = UI_Text_Width( text, scale, 0 );
    UI_Text_Paint( x - len / 2, y, scale, color, text, 0, 0, ITEM_TEXTSTYLE_SHADOWEDMORE );
}

void Text_PaintCenter_AutoWrapped( float32 x, float32 y, float32 xmax, float32 ystep, float32 scale, vec4_t color, pointer str, float32 adjust )
{
    sint width;
    valueType* s1, *s2, *s3;
    valueType c_bcp;
...
static void UI_DisplayDownloadInfo( pointer downloadName, float32 centerPoint, float32 yStart, float32 scale )
{
    ...
    if( downloadSize > 0 )
        s = va( "%s (%d%%)", downloadName, ( sint )( ( float32 )downloadCount * 100.0f / downloadSize ) );
    else
        s = downloadName;
