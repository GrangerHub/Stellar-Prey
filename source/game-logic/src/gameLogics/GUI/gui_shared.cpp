...
void Fade( sint* flags, float32* f, float32 clamp, sint* nextTime, sint offsetTime, bool bFlags, float32 fadeAmount )
{
...
static void Window_Paint( WinDow* w, float32 fadeAmount, float32 fadeClamp, float32 fadeCycle )
{
    vec4_t color;
...
itemDef_t* Menu_GetMatchingItemByNumber( menuDef_t* menu, sint index, pointer name )
{
    sint i;
    sint count = 0;
        ...
        switch( testtype )
        {
            ...
            case 2:
                ...
                else if( !Q_stricmp( cvar, "systemrestartIsRequired" ) )
                {
                    ...
                    sint				ui_com_soundmegs = DC->getCVarValue( "ui_com_soundmegs" );
                ...
                else if( !Q_stricmp( cvar, "vidrestartIsRequired" ) )
                {
                    ...
                    sint             ui_r_ext_compressed_textures = DC->getCVarValue( "ui_r_ext_compressed_textures" );
                    sint             ui_r_allowextensions = DC->getCVarValue( "ui_r_allowextensions" );
                    sint             ui_r_detailtextures = DC->getCVarValue( "ui_r_detailtextures" );
                    ...
                    sint				ui_r_hdrrendering = DC->getCVarValue( "ui_r_hdrrendering" );
                    sint				ui_r_bloom = DC->getCVarValue( "ui_r_bloom" );
                    sint				ui_r_normalmapping = DC->getCVarValue( "ui_r_normalmapping" );
                    sint				ui_r_parallaxmapping = DC->getCVarValue( "ui_r_parallaxmapping" );
                    ...
                    sint             r_ext_compressed_textures = DC->getCVarValue( "r_ext_compressed_textures" );
                    ...
                    sint				r_parallaxmapping = DC->getCVarValue( "r_parallaxmapping" );
...
void Menu_TransitionItemByName( menuDef_t* menu, pointer p, rectDef_t rectFrom, rectDef_t rectTo,
...
void Menu_OrbitItemByName( menuDef_t* menu, pointer p, float32 x, float32 y, float32 cx, float32 cy, sint time )
{
    itemDef_t* item;
    sint i;
    sint count = Menu_ItemsMatchingGroup( menu, p );
...
void Menu_SetItemBackground( const menuDef_t* menu, pointer itemName, pointer background )
{
    sint j, count;
    itemDef_t* item;
...
static bool UI_Text_Emoticon( pointer s, bool* escaped, sint* length, qhandle_t* h, sint* width )
{
...
float32 UI_Text_Width( pointer text, float32 scale, sint limit )
{
    ...
    if( text )
    {
        ...
        while( s && *s && count < len )
        {
            ...
            else if( UI_Text_Emoticon( s, &emoticonEscaped, &emoticonLen, nullptr, nullptr ) )
            {
                if( emoticonEscaped )
                {
                    s++;
...
static void UI_Text_PaintChar( float32 x, float32 y, float32 width, float32 height,
                               float32 scale, float32 s, float32 t, float32 s2, float32 t2, qhandle_t hShader )
{
    float32 w, h;
    w = width * scale;
    h = height * scale;
...
void UI_Text_Paint( float32 x, float32 y, float32 scale, vec4_t color, pointer text,
                    float32 adjust, sint limit, sint style )
{
    ...
    if( text )
    {
        ...
        while( s && *s && count < len )
        {
            ...
            {
                sint ofs = style == ITEM_TEXTSTYLE_SHADOWED ? 1 : 2;
...
void UI_Text_PaintWithCursor( float32 x, float32 y, float32 scale, vec4_t color, pointer text,
...
void Item_ListBox_MouseEnter( itemDef_t* item, float32 x, float32 y )
{
    ...
    if( item->window.flags & WINDOW_HORIZONTAL )
    {
                                      ...
                                      WINDOW_LB_PGUP | WINDOW_LB_PGDN ) ) )
        {
            ...
            if( listPtr->elementStyle == LISTBOX_IMAGE )
            {
                ...
                if( Rect_ContainsPoint( &r, x, y ) )
                {
                    listPtr->cursorPos = ( sint )( ( x - r.x ) / listPtr->elementWidth )  + listPtr->startPos;
                                       ...
                                       WINDOW_LB_THUMB | WINDOW_LB_PGUP | WINDOW_LB_PGDN ) ) )
    {
        ...
        if( Rect_ContainsPoint( &r, x, y ) )
        {
            listPtr->cursorPos = ( sint )( ( y - 2 - r.y ) / listPtr->elementHeight )  + listPtr->startPos;
...
bool Item_ListBox_HandleKey( itemDef_t* item, sint key, bool down, bool force )
{
    listBoxDef_t* listPtr = ( listBoxDef_t* )item->typeData;
    sint count = DC->feederCount( item->special );
    sint max, viewmax;
...
static void Item_TextField_CalcPaintOffset( itemDef_t* item, valueType* buff )
{
...
bool Item_HandleKey( itemDef_t* item, sint key, bool down )
{
    ...
    switch( item->type )
    {
        ...
        case ITEM_TYPE_EDITFIELD:
        case ITEM_TYPE_NUMERICFIELD:
            //Dushan - the stupidiest idea ever
...
void Menu_HandleKey( menuDef_t* menu, sint key, bool down )
{
    ...
    if( key == K_MOUSE3 && down && Rect_ContainsPoint( &menu->window.rect, DC->cursorx, DC->cursory ) &&
...
void Item_SetTextExtents( itemDef_t* item, sint* width, sint* height, pointer text )
{
...
void Item_TextColor( itemDef_t* item, vec4_t* newColor )
{
    ...
    if( item->enableCvar != nullptr && *item->enableCvar && item->cvarTest != nullptr && *item->cvarTest )
    {
        if( item->cvarFlags & ( CVAR_ENABLE | CVAR_DISABLE ) && !Item_EnableShowViaCvar( item, CVAR_ENABLE ) )
    ...
    while( *p )
    {
        ...
        while( UI_Text_Width( p, scale, testLength ) < width )
        {
            ...
            for( i = 0; i < testLength; )
            {
                ...
                while( UI_Text_Emoticon( q, &emoticonEscaped, &emoticonLen, nullptr, nullptr ) )
                {
                    if( emoticonEscaped )
                        q++;
                    else
...
typedef struct
{
    valueType      text[ MAX_WRAP_TEXT* MAX_WRAP_LINES ];  //FIXME: augment with hash?
    rectDef_t rect;
    float32     scale;
...
static void UI_CreateCacheEntry( pointer text, rectDef_t* rect, float32 scale )
{
...
static bool UI_CheckWrapCache( pointer text, rectDef_t* rect, float32 scale )
{
    sint i;
...
void Item_TextField_Paint( itemDef_t* item )
{
    valueType            buff[1024];
    vec4_t          newColor;
    sint             offset = ( item->text && *item->text ) ? ITEM_VALUE_OFFSET : 0;
    menuDef_t*       parent = ( menuDef_t* )item->parent;
    editFieldDef_t*  editPtr = ( editFieldDef_t* )item->typeData;
...
void Item_Model_Paint( itemDef_t* item )
{
    ...
    if( modelPtr->frameTime )	// don't advance on the first frame
        modelPtr->backlerp += ( ( ( DC->realTime - modelPtr->frameTime ) / 1000.0f ) * ( float32 )modelPtr->fps );
...
keywordHash_t* KeywordHash_Find( keywordHash_t* table[], valueType* keyword )
{
    keywordHash_t* key;
    sint hash;
