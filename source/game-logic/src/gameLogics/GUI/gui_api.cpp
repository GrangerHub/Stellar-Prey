...
#else
Q_EXPORT idUserInterfaceManager* dllEntry( guiImports_t* guiimports )
#endif
{
    imports = guiimports;
...
sint trap_CM_LerpTag( orientation_t* tag, clipHandle_t mod, sint startFrame, sint endFrame, float32 frac, pointer tagName )
{
...
void trap_LAN_GetServerAddressString( sint source, sint n, valueType* buf, sint buflen )
{
    imports->idLANSystem->GetServerAddressString( source, n, buf, buflen );
}

void trap_LAN_GetServerInfo( sint source, sint n, valueType* buf, sint buflen )
{
...
sint trap_LAN_ServerStatus( valueType* serverAddress, valueType* serverStatus, sint maxLen )
{
    return imports->idLANSystem->GetServerStatus( serverAddress, serverStatus, maxLen );
}
...
sint trap_LAN_CompareServers( sint source, sint sortKey, sint sortDir, sint s1, sint s2 )
{
    return imports->idLANSystem->CompareServers( source, sortKey, sortDir, s1, s2 );
}
